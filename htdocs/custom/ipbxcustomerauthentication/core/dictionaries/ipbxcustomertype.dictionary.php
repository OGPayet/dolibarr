 ...
 */
class IpbxCustomerTypeDictionary extends Dictionary
{
    /**
     ...
     */
    public $langs = array('ipbxcustomerauthentication@ipbxcustomerauthentication');

    /**
     * @var string      Family name of which this dictionary belongs
     */
    public $family = 'ipbxCustomerAuthentication';

    /**
     * @var string      Family label for show in the list, translated if key found
     */
    public $familyLabel = 'ModuleIpbxCustomerAuthenticationName';
     ...
     */
    public $module = 'ipbxcustomerauthentication';

    /**
     * @var string      Module name for show in the list, translated if key found
     */
    public $moduleLabel = 'ModuleIpbxCustomerAuthenticationName';

    /**
     * @var string      Name of this dictionary for show in the list, translated if key found
     */
    public $nameLabel = 'IpbxCustomerCustomerTypeDictionaryName';

    /**
     * @var string      Name of the dictionary table without prefix (ex: c_country)
     */
    public $table_name = 'c_ipbxcustomerauthentication_customertype';

    /**
     * @var string[] Cache of Thirdparty tags
     */
    public static $cache_thirdparty_tags;

    /**
     * @var int[] Categorie type ids
     */
    const CATEGORY_TYPES = array(Categorie::TYPE_CUSTOMER, Categorie::TYPE_SUPPLIER);
    ...
    public $fields = array(
        'label' => array(
            'name'       => 'label',
            'label'      => 'IpbxCustomerAuthenticationCustomerTypeLabel',
            'type'       => 'varchar',
            'database'   => array(),
            'is_require' => true,
        ),
        'ipbxvalue' => array(
            'name'       => 'ipbxvalue',
            'label'      => 'IpbxCustomerAuthenticationCustomerTypeIpbxValue',
            'type'       => 'varchar',
            'database'   => array(),
            'is_require' => true,
        ),
        'thirdparty_tag' => array()
    );
    ...
    {
        $this->fields['thirdparty_tag'] = array(
            'name'       => 'thirdparty_tag',
            'label'      => 'IpbxCustomerAuthenticationCustomerThirdpartyTag',
            'type'       => 'checkbox',
            'options'    => $this->getThirdpartiesTags()
        );
    }

    /**
     * Function to get cached tags label on third parties
     * @return string[]
     */
    private function getThirdpartiesTags()
    {
        if (!self::$cache_thirdparty_tags) {
            $this->loadThirdpartyTags();
        }
        return self::$cache_thirdparty_tags;
    }

    /**
     * Function to load tags label on third parties
     * @return string[]
     */
    private function loadThirdpartyTags()
    {
        $categoryStatic = new Categorie($this->db);
        $result = array();
        foreach (self::CATEGORY_TYPES as $categoryType) {
            $cate_arbo = $categoryStatic->get_full_arbo($categoryType);
            foreach ($cate_arbo as $cat) {
                $result[$cat['id']] = $cat['fulllabel'];
            }
        }
        self::$cache_thirdparty_tags = $result;
        return $result;
    }
}
