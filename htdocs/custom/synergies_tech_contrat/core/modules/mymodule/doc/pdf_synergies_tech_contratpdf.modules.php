...
/**
 *	\file       htdocs/core/modules/synergies_tech_contrat/doc/pdf_beluga.modules.php
 *	\ingroup    synergies_tech_contrat
 *	\brief      Fichier de la classe permettant de generer les synergies_tech_contrats au modele beluga
...
/**
 *	Classe permettant de generer les synergies_tech_contrats au modele Baleine
 */

class pdf_synergies_tech_contratpdf extends Modelesynergies_tech_contrat
{
	...
	/**
	 *	Fonction generant le synergies_tech_contrat sur le disque
	 *
	 *	@param	synergies_tech_contrat		$object   		Object synergies_tech_contrat a generer
	...
	function write_file($object,$outputlangs)
	{
		...
		$formsynergies_tech_contrat=new Formsynergies_tech_contrat($this->db);

		if (! is_object($outputlangs)) $outputlangs=$langs;
		// For backward compatibility with FPDF, force output charset to ISO, because FPDF expect text to be encoded in ISO
		if (! empty($conf->global->MAIN_USE_FPDF)) $outputlangs->charset_output='ISO-8859-1';

		$outputlangs->load("main");		$outputlangs->load("dict");
		$outputlangs->load("companies");
		$outputlangs->load("synergies_tech_contrat");

		if ($conf->synergies_tech_contrat->dir_output)
		{
			//$nblignes = count($object->lines);  // This is set later with array of tasks

			$objectref = dol_sanitizeFileName($object->ref);
			$dir = $conf->synergies_tech_contrat->dir_output;
			...
			if (file_exists($dir))
			{
				...
				$task = new Taskin($this->db);
				$tasksarray = $task->getTasksArray('synergies_tech_contrat',0,0,$object->id);
				...
				$pdf->SetSubject($outputlangs->transnoentities("synergies_tech_contrat"));
				$pdf->SetCreator("Dolibarr ".DOL_VERSION);
				$pdf->SetAuthor($outputlangs->convToOutputCharset($user->getFullName($outputlangs)));
				$pdf->SetKeyWords($outputlangs->convToOutputCharset($object->ref)." ".$outputlangs->transnoentities("synergies_tech_contrat"));
                ...
                $listofreferent=array(
                    'propal'=>array(
			'name'=>"Proposals",
			'title'=>"ListProposalsAssociatedsynergies_tech_contrat",
			'class'=>'Propal',
			'table'=>'propal',
                        'datefieldname'=>'datep',
			...
			'title'=>"ListOrdersAssociatedsynergies_tech_contrat",
			'class'=>'Commande',
			'table'=>'commande',
			...
			'title'=>"ListInvoicesAssociatedsynergies_tech_contrat",
			'class'=>'Facture',
			'margin'=>'add',
			'table'=>'facture',
			'datefieldname'=>'datef',
			...
			'title'=>"ListPredefinedInvoicesAssociatedsynergies_tech_contrat",
			'class'=>'FactureRec',
			'table'=>'facture_rec',
			'datefieldname'=>'datec',
			'test'=>$conf->facture->enabled && $user->rights->facture->lire,
			...
			'title'=>"ListSupplierOrdersAssociatedsynergies_tech_contrat",
			...
			'name'=>"BillsSuppliers",
			'title'=>"ListSupplierInvoicesAssociatedsynergies_tech_contrat",
			...
			'test'=>$conf->fournisseur->enabled && $user->rights->fournisseur->facture->lire,
                    ...
                    'contract'=>array(
			'name'=>"Contracts",
			'title'=>"ListContractAssociatedsynergies_tech_contrat",
			'class'=>'Contrat',
			'table'=>'contrat',
			...
			'test'=>$conf->contrat->enabled && $user->rights->contrat->lire,
                    ...
                    'intervention'=>array(
			'name'=>"Interventions",
			'title'=>"ListFichinterAssociatedsynergies_tech_contrat",
			'class'=>'Fichinter',
			'table'=>'fichinter',
			...
			'title'=>"ListExpenseReportsAssociatedsynergies_tech_contrat",
			'class'=>'Deplacement',
			'table'=>'deplacement',
			'datefieldname'=>'dated',
			'margin'=>'minus',
			...
			'test'=>$conf->expensereport->enabled && $user->rights->expensereport->lire,
                    ...
                    'agenda'=>array(
			'name'=>"Agenda",
			'title'=>"ListActionsAssociatedsynergies_tech_contrat",
			'class'=>'ActionComm',
			'table'=>'actioncomm',
			'datefieldname'=>'datep',
			'disableamount'=>1,
			...
			$langs->load($langstoload);
                    ...
                    if ($qualified)
                    {
                        ...
                        if ($num >= 0)
                        {
                            ...
                            $selectList = $formsynergies_tech_contrat->select_element($tablename, $synergies_tech_contrat->thirdparty->id);
                            $nexY = $pdf->GetY() + 1;
                            $curY = $nexY;
                            $pdf->SetXY($this->posxref, $curY);
	 ...
	 *
	 *  @param	PDF			$pdf     		Object PDF
	 *  @param  synergies_tech_contrat		$object     	Object to show
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell(100, 4, $outputlangs->transnoentities("synergies_tech_contrat")." ".$outputlangs->convToOutputCharset($object->ref), '', 'R');
	...
	function _pagefoot(&$pdf,$object,$outputlangs,$hidefreetext=0)
	{
		...
		return pdf_pagefoot($pdf,$outputlangs,'SYNERGIES_TECH_CONTRAT_FREE_TEXT',$this->emetteur,$this->marge_basse,$this->marge_gauche,$this->page_hauteur,$object,$showdetails,$hidefreetext);
	}
}
