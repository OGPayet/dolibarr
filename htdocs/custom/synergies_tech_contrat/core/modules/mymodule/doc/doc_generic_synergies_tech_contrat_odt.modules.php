...
/**
 *	\file       htdocs/core/modules/synergies_tech_contrat/doc/doc_generic_synergies_tech_contrat_odt.modules.php
 *	\ingroup    synergies_tech_contrat
 *	\brief      File of class to build ODT documents for third parties
 */
dol_include_once('/synergies_tech_contrat/core/modules/synergies_tech_contrat/modules_synergies_tech_contrat.php');
dol_include_once('/synergies_tech_contrat/class/synergies_tech_contrat.class.php');
dol_include_once('/taskin/class/taskin.class.php');
 ...
 */
class doc_generic_synergies_tech_contrat_odt extends Modelesynergies_tech_contrat
{
	...
	function __construct($db)
	{
		...
		$langs->load("synergies_tech_contrat");

		$this->db = $db;
		$this->name = "ODT templates";
		$this->description = $langs->trans("DocumentModelOdt");
		$this->scandir = 'SYNERGIES_TECH_CONTRAT_ADDON_PDF_ODT_PATH';	// Name of constant that is used to save list of directories to scan
	...
	function get_substitutionarray_tasks($task,$outputlangs)
	{
		...
		return array(
		'task_ref'=>$task->ref,
		'task_fk_synergies_tech_contrat'=>$task->fk_synergies_tech_contrat,
		'task_synergies_tech_contratref'=>$task->synergies_tech_contratref,
		'task_synergies_tech_contratlabel'=>$task->synergies_tech_contratlabel,
	 ...
	 *  @return	array								Return a substitution array
	 */
	function get_substitutionarray_synergies_tech_contrat_contacts($contact,$outputlangs)
	{
		global $conf;
	 ...
	 *  @return	array								Return a substitution array
	 */
	function get_substitutionarray_synergies_tech_contrat_file($file,$outputlangs)
	{
		global $conf;
	 ...
	 *  @return	array								Return a substitution array
	 */
	function get_substitutionarray_synergies_tech_contrat_reference($refdetail,$outputlangs)
	{
		global $conf;
	...
	function info($langs)
	{
		...
		$texte.= '<form action="'.$_SERVER["PHP_SELF"].'?page='.GETPOST('page').'" method="POST">';
		$texte.= '<input type="hidden" name="token" value="'.$_SESSION['newtoken'].'">';
		$texte.= '<input type="hidden" name="action" value="setModuleOptions">';
		$texte.= '<input type="hidden" name="param1" value="SYNERGIES_TECH_CONTRAT_ADDON_PDF_ODT_PATH">';
		$texte.= '<table class="nobordernopadding" width="100%">';

		// List of directories area
		$texte.= '<tr><td>';
		$texttitle=$langs->trans("ListOfDirectories");
		$listofdir=explode(',',preg_replace('/[\r\n]+/',',',trim($conf->global->SYNERGIES_TECH_CONTRAT_ADDON_PDF_ODT_PATH)));
		$listoffiles=array();
		...
		$texte.=$conf->global->SYNERGIES_TECH_CONTRAT_ADDON_PDF_ODT_PATH;
		$texte.= '</textarea>';
		...
		if (! empty($conf->global->SYNERGIES_TECH_CONTRAT_ADDON_PDF_ODT_PATH))
		{
	...
	function write_file($object,$outputlangs,$srctemplatepath)
	{
		...
		$outputlangs->load("synergies_tech_contrats");
		...
		if ($conf->projet->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				// If SYNERGIES_TECH_CONTRATLEADER contact defined on synergies_tech_contrat, we use it
				$usecontact=false;
				$arrayidcontact=$object->getIdContact('external','SYNERGIES_TECH_CONTRATLEADER');
				...
				if (! empty($usecontact))
				{
				// if we have a SYNERGIES_TECH_CONTRATLEADER contact and we dont use it as recipient we store the contact object for later use
                ...
                // retrieve contact information for use in synergies_tech_contrat as contact_xxx tags
			$array_synergies_tech_contrat_contact = array();
			if ($usecontact)
				$array_synergies_tech_contrat_contact=$this->get_substitutionarray_contact($contactobject,$outputlangs,'contact');

				$tmparray = array_merge($array_user,$array_soc,$array_thirdparty,$array_objet,$array_other,$array_synergies_tech_contrat_contact);
				...
				try
				{
					...
					foreach ($tasksarray as $task)
					{
						...
						// Replace tags of synergies_tech_contrat files
				...
				try
				{
					$listlines = $odfHandler->setSegment('synergies_tech_contratfiles');
					...
					foreach ($filearray as $filedetail)
					{
						...
						$tmparray=$this->get_substitutionarray_synergies_tech_contrat_file($filedetail,$outputlangs);
				...
				if ((is_array($contact_arrray) && count($contact_arrray) > 0))
				{
					try
					{
						$listlines = $odfHandler->setSegment('synergies_tech_contratcontacts');
						...
						foreach ($contact_arrray as $contact)
						{
							...
							$tmparray=$this->get_substitutionarray_synergies_tech_contrat_contacts($contact,$outputlangs);
							foreach($tmparray as $key => $val)
							{
								try
								{
				...
				try
				{
					$listlines = $odfHandler->setSegment('synergies_tech_contratrefs');
						...
						if ($qualified)
						{
							...
							if (count($elementarray)>0 && is_array($elementarray))
							{
								...
								for ($i = 0; $i < $num; $i++)
								{
									...
									$tmparray=$this->get_substitutionarray_synergies_tech_contrat_reference($ref_array,$outputlangs);
