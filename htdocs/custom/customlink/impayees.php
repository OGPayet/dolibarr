...
if (! $user->rights->societe->thirdparty->voir || $socid)
...
if ($action == 'presend' && GETPOST('cancel')) {
	$action='';
	// If we made a cancel from submit email form, this means we must be into mode=sendmassremind
	if (GETPOST('models')=='facture_relance')
		$mode='sendmassremind';
}
				...
				if (dol_is_file($file)) {
					...
					if (dol_strlen($sendto)) {
						...
						$mailfile = new CMailFile(
										$subject, $sendto, $from, $message,
										$filepath, $mimetype, $filename,
										$sendtocc, '', $deliveryreceipt, -1
						);
						if ($mailfile->error)
							...
							if ($result) {
								...
								if ($mailfile->error) {
									...
									$resultmasssend.='<div class="warning">';
									$resultmasssend.='No mail sent. Feature is disabled by option MAIN_DISABLE_ALL_MAILS</div>';
								}
							}
						}
					}
...
if ($action == "builddoc"
	...
	&& ! GETPOST('button_search')
	&& !empty($builddoc_generatebutton)) {
	if (is_array($_POST['toGenerate'])) {
		$arrayofinclusion=array();
		foreach ($_POST['toGenerate'] as $tmppdf)
			$arrayofinclusion[] = preg_quote($tmppdf.'.pdf', '/');
		$factures = dol_dir_list(
						$conf->facture->dir_output, 'all', 1,
						implode('|', $arrayofinclusion), '\.meta$|\.png',
						'date', SORT_DESC
		);
...
$sql.= ", f.rowid as facid, f.ref as facnumber, f.ref_client, f.increment, f.total as total_ht";
$sql.= ", f.tva as total_tva, f.total_ttc, f.localtax1, f.localtax2, f.revenuestamp";
...
$sql.= " AND f.paye = 0";
if ($option == 'late')
	$sql.=" AND f.date_lim_reglement < '".$db->idate(dol_now() - $conf->facture->thirdparty->warning_delay)."'";
if (! $user->rights->societe->thirdparty->voir && ! $socid)
...
if (GETPOST('sf_ref'))   $sql .= " AND f.ref LIKE '%".$db->escape(GETPOST('sf_ref'))."%'";
$sql.= " GROUP BY s.nom, s.rowid, s.email, f.rowid, f.ref asfacnumber, f.increment, f.total, f.tva, f.total_ttc, ";
$sql.= " f.localtax1, f.localtax2, f.revenuestamp, f.datef, f.date_lim_reglement, f.paye, f.fk_statut, f.type";
if (! $user->rights->societe->thirdparty->voir && ! $socid)
	$sql .= ", sc.fk_soc, sc.fk_user ";
$sql.= " ORDER BY ";
...
if ($resql) {
	...
	if(! empty($socid))
		$titre=$langs->trans("BillsCustomersUnpaidForCompany", $soc->nom);
	else
		$titre=$langs->trans("BillsCustomersUnpaid");
	...
	$link='';
	if (empty($option))
		$link='<a href="'.$_SERVER["PHP_SELF"].'?option=late">'.$langs->trans("ShowUnpaidLateOnly").'</a>';
	elseif ($option == 'late')
		$link='<a href="'.$_SERVER["PHP_SELF"].'">'.$langs->trans("ShowUnpaidAll").'</a>';
	print_fiche_titre($titre, $link);

	dol_htmloutput_mesg($mesg);

	print '<form id="form_unpaid" method="POST" action="';
	print $_SERVER["PHP_SELF"].'?sortfield='.$sortfield.'&sortorder='.$sortorder.'">';
	...
	print_liste_field_titre(
					$langs->trans("Ref"), $_SERVER["PHP_SELF"], "f.facnumber",
					"", $param, "", $sortfield, $sortorder
	);
	print_liste_field_titre(
					$langs->trans('RefCustomer'), $_SERVER["PHP_SELF"], 'f.ref_client',
					'', $param, '', $sortfield, $sortorder
	);
	print_liste_field_titre(
					$langs->trans("Date"), $_SERVER["PHP_SELF"], "f.datef",
					"", $param, 'align="center"', $sortfield, $sortorder
	);
	print_liste_field_titre(
					$langs->trans("DateDue"), $_SERVER["PHP_SELF"], "f.date_lim_reglement",
					"", $param, 'align="center"', $sortfield, $sortorder
	);
	print_liste_field_titre(
	...
	print_liste_field_titre(
					$langs->trans("AmountHT"), $_SERVER["PHP_SELF"], "f.total",
					"", $param, 'align="right"', $sortfield, $sortorder
	);
	print_liste_field_titre(
					$langs->trans("Taxes"), $_SERVER["PHP_SELF"], "f.tva",
					"", $param, 'align="right"', $sortfield, $sortorder
	);
	print_liste_field_titre(
					$langs->trans("AmountTTC"), $_SERVER["PHP_SELF"], "f.total_ttc",
					"", $param, 'align="right"', $sortfield, $sortorder
	);
	print_liste_field_titre(
					$langs->trans("Received"), $_SERVER["PHP_SELF"], "am",
					"", $param, 'align="right"', $sortfield, $sortorder
	);
	print_liste_field_titre(
					$langs->trans("Rest"), $_SERVER["PHP_SELF"], "am",
					"", $param, 'align="right"', $sortfield, $sortorder
	);
	print_liste_field_titre(
					$langs->trans("Status"), $_SERVER["PHP_SELF"], "fk_statut,paye,am",
					"", $param, 'align="right"', $sortfield, $sortorder
	);
	if (empty($mode))
		print_liste_field_titre(
		...
		print_liste_field_titre(
						$langs->trans("Remind"), $_SERVER["PHP_SELF"], "",
						"", $param, 'align="center"', $sortfield, $sortorder
		);
	...
	print '<input class="flat" type="text" size="10" name="search_societe" value="';
	print dol_escape_htmltag($search_societe).'"></td>';
	print '<td class="liste_titre" align="right">';
	print '<input class="flat" type="text" size="8" name="search_montant_ht" value="';
	print dol_escape_htmltag($search_montant_ht).'"></td>';
	print '<td class="liste_titre">&nbsp;</td>';
	print '<td class="liste_titre" align="right">';
	print '<input class="flat" type="text" size="8" name="search_montant_ttc" value="';
	print dol_escape_htmltag($search_montant_ttc).'"></td>';
	print '<td class="liste_titre">&nbsp;</td>';
	print '<td class="liste_titre">&nbsp;</td>';
	print '<td class="liste_titre" align="right">';
	...
	if (empty($mode)) {
		...
		if ($conf->use_javascript_ajax) {
			print '<a href="#" id="checkall">'.$langs->trans("All").'</a>';
			print ' / <a href="#" id="checknone">'.$langs->trans("None").'</a>';
		}
		print '</td>';
	} else {
		print '<td class="liste_titre" align="center">';
		if ($conf->use_javascript_ajax) {
			print '<a href="#" id="checkallsend">'.$langs->trans("All").'</a>';
			print ' / <a href="#" id="checknonesend">'.$langs->trans("None").'</a>';
		}
		print '</td>';
	}
	print "</tr>\n";
	...
	if ($num > 0) {
		...
		while ($i < $num) {
			...
			if ($date_limit < ($now - $conf->facture->thirdparty->warning_delay) && ! $objp->paye && $objp->fk_statut == 1)
			...
			print '<td><a href="'.DOL_URL_ROOT.'/comm/fiche.php?socid='.$objp->socid.'">';
			print img_object($langs->trans("ShowCompany"), "company").' '.dol_trunc($objp->nom, 28).'</a></td>';
			...
			// Remain to receive
			print '<td align="right">';
			print ((! empty($objp->am) || ! empty($cn))?price($objp->total_ttc-$objp->am-$cn):'&nbsp;').'</td>';
			...
			if (empty($mode)) {
				...
				if ($objp->email)
					...
					print img_picto($langs->trans("NoEMail"), 'warning.png');
				print '</td>' ;
			}
	...
	if (empty($mode)) {
		...
		$formfile->show_documents(
						'customlink', '', $filedir, $urlsource, $genallowed,
						$delallowed, '', 1, 1, 0, 48, 1,
						$param, $langs->trans("PDFMerge"), $langs->trans("PDFMerge")
		);
	} else {
		if ($action != 'presend') {
			print '<div class="tabsAction">';
			print '<a href="'.$_SERVER["PHP_SELF"].'?mode=sendremind&action=presend" class="butAction" ';
			print 'name="buttonsendremind" value="'.dol_escape_htmltag($langs->trans("SendRemind")).'">';
			print $langs->trans("SendRemind").'</a>';
			print '</div>';
			print '<br>';
		}
	}
