...
if ($action == 'confirm_paiement' && $confirm == 'yes') {
	...
	if (! $error) {
		...
		if ($result < 0) {
			...
			// mise à jour du nom du payeur pour être propre
	...
	if (! $error) {
		...
		if ($invoiceid > 0) {
			if (DOL_VERSION < '6.0.0')
				$loc = DOL_URL_ROOT.'/compta/facture.php?facid='.$invoiceid;
			else
...
if ($object->fetch($id, $ref) > 0) {
	...
	dol_fiche_head($head, 'externalbill', $langs->trans("ThirdParty"), 0, 'company');
...
if ($action == '' || $action == 'confirm_paiement' || $action == 'add_paiement') {
	...
	if ($conf->use_javascript_ajax) {
		...
		if (! empty($conf->global->MAIN_JS_ON_PAYMENT) && $facture->type != 2) {
			...
			print '	$("#amountpayment").change(function() {
						callForResult();
					});';
		}
	...
	if ($conf->use_javascript_ajax && !empty($conf->global->MAIN_JS_ON_PAYMENT)) {
		...
		if ($action == 'add_paiement') {
			...
			print '<input name="amountpayment" type="hidden" ';
			print ' value="'.(empty($_POST['amountpayment'])?'':$_POST['amountpayment']).'">';
		} else {
	...
	// on récupère d'abord la liste des clients liés à ce tiers
	...
	// ensuite la liste des factures impayés
	$sql = 'SELECT  f.rowid as facid, s.nom, f.facnumber, f.total_ttc, f.type, f.datef as df';
...
if (! GETPOST('action')) {
	...
	if ($id) {
		...
		$sql.= ' (sp.rowid= ec.fk_socpeople AND ec.element_id = s.rowid AND ec.fk_c_type_contact=192 AND sp.fk_soc='.$id.'))';
	}
//print $sql;
	...
	if ($resql) {
		...
		while ($i < min($num, $limit)) {
			...
			if (DOL_VERSION < "6.0.0")
				...
				print '<td><a href="'.DOL_URL_ROOT.'/compta/facture/card.php?facid='.$objp->facid.'">';
