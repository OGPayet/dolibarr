...
class RequestManagerMessageTemplateDictionary extends Dictionary
{
	...
	protected function initialize()
    {
        ...
        $hookmanager->initHooks(array('requestmanagermessagetemplates'));
        ...
        $templateTypeList = array(
            ...
            'message_template_user' => $langs->trans('RequestManagerTemplateTypeMessageTemplateUser'),
//            'message_template_customer' => $langs->trans('RequestManagerTemplateTypeMessageTemplateCustomer'), prevoir option pour client avoir ses templates ?
        );

        $parameters = array('templateTypeList' => $templateTypeList);
        $reshook = $hookmanager->executeHooks('messageTemplateTypeList', $parameters);    // Note that $action and $object may have been modified by some hooks
        if ($reshook >= 0) {
        ...
        $this->fields['template_type'] = array(
            'name' => 'template_type',
            'label' => 'TypeOfTemplate',
            'type' => 'select',
            'options' => $templateTypeList,
            'is_require' => true,
            'help' => 'RequestManagerTemplateForType',
        );
...
class RequestManagerMessageTemplateDictionaryLine extends DictionaryLine
{
    public function checkFieldsValues($fieldsValue)
    {
        ...
        if ($fieldsValue['template_type'] == 'notify_status_modified' ||
            $fieldsValue['template_type'] == 'notify_message_added') {
            $template = $this->dictionary->getCodeFromFilter('{{rowid}}', array('template_type' => array($fieldsValue['template_type']), 'request_type'=> explode(',', $fieldsValue['request_type'])));
            if (($template > 0 && $template != $this->id) || $template == -1) {
                $this->errors[] = $langs->trans('RequestManagerErrorOnlyOneTemplateForThisTypeForEachRequestType');
                return -1;
            } elseif ($template == -2) {
                return $template;
            }
