...
$out .= '   var ajaxData = {';
$out .= '       actioncomm_id: ' . json_encode($selectedActionCommId) . ',';
...
if (!empty($selectedDescription)) {
    $out .= '   description: ' . json_encode($selectedDescription) . ',';
}
if ($conf->equipement->enabled) {
    $out .= '   equipement_id: ' . json_encode($selectedEquipementId) . ',';
}
if (!empty($selectedLabel)) {
    $out .= '   label: ' . json_encode($selectedLabel) . ',';
}
$out .= '       socid_origin: ' . json_encode($selectedSocIdOrigin) . ',';
$out .= '       socid: ' . json_encode($selectedSocId) . ',';
$out .= '       socid_benefactor: ' . json_encode($selectedSocIdBenefactor) . ',';
$out .= '       socid_watcher: ' . json_encode($selectedSocIdWatcher) . ',';
$out .= '       source: ' . json_encode($selectedFkSource) . ',';
$out .= '       type: ' . json_encode($selectedFkType) . ',';
$out .= '       urgency: ' . json_encode($selectedFkUrgency) . ',';
$out .= '       notify_assigned: ' . json_encode($notify_assigned) . ',';
$out .= '       notify_requesters: ' . json_encode($notify_requesters) . ',';
$out .= '       notify_watchers: ' . json_encode($notify_watchers) . ',';
$out .= '       message_type: ' . json_encode($message_type) . ',';
$out .= '       message_subject: ' . json_encode($message_subject) . ',';
$out .= '       message: ' . json_encode($message) . ',';
$out .= '       zone: 1';
$out .= '   };';
