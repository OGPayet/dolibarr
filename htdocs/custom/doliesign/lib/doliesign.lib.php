 ...
 *
 * @return mixed 0 = OK, array with errors if NOK
 */

function ysInit($user, $object, $dir, $emailInitTemplate = '', $emailEndTemplate = '', $authMode = "sms")
{
	...
	if (is_object($object) && is_array($configIds)) {
		...
		foreach($configIds as $configId) {
			...
			if ($res == 0) {
				$errors[] = "DoliEsignContactMissing";
				return $errors;
			}
				...
				foreach ($contactIds as $key => $contactId) {
					...
					if ($result = $contact->fetch($contactId) > 0){
						if (empty($contact->email)) {
							$errors[] = "DoliEsignContactEmailMissing";
							return $errors;
						}
						if ($authMode == 'sms' && empty($contact->phone_mobile)) {
							$errors[] = "DoliEsignContactPhoneMobileMissing";
							return $errors;
						} else if ($authMode == 'sms') {
							$contact->phone_mobile = ysFixMobile($contact->phone_mobile, $contact->country_code);
							if (empty($contact->phone_mobile)) {
								$errors[] = "DoliEsignContactPhoneMobileWrongFormat";
								return $errors;
							}
						} else {
			...
			if (count($userIds) > 0) {
				foreach ($userIds as $key => $userId) {
					...
					if ($result = $user->fetch($userId) > 0){
						if (empty($user->email)) {
							$errors[] = "DoliEsignUserEmailMissing";
							return $errors;
						}
						if ($authMode == 'sms' && empty($user->user_mobile)) {
							...
							$user->user_mobile = ysFixMobile($user->user_mobile, $user->country_code);
							if (empty($user->user_mobile)) {
								$errors[] = "DoliEsignUserPhoneMobileWrongFormat";
								return $errors;
							}
						} else {
							$user->user_mobile = '';
	...
	else
	{
		...
		foreach($refs as $key=>$ref) {
			// create dolisign object
			$doliEsign = new DoliEsign($object->db);
			$doliEsign->ref = $ref;
			$doliEsign->id_file = $ysResult['fileInfos']['idFile'];
			$doliEsign->hash_file = $ysResult['fileInfos']['sha1'];
			$doliEsign->path_file = $file;
			$doliEsign->sign_id = $ysResult['idDemand'];
			$doliEsign->sign_status = 'COSIGNATURE_FILE_REQUEST_PENDING';
			$doliEsign->status = DoliEsign::STATUS_WAITING;
			$doliEsign->date_creation = dol_now();
			$doliEsign->fk_object = $object->id;
			$doliEsign->object_type = $object->element;
			if ($listId[$key]->type == 'contact') $doliEsign->fk_contact_sign = $listId[$key]->id;
			if ($listId[$key]->type == 'user') $doliEsign->fk_user_sign = $listId[$key]->id;
			$doliEsign->fk_user_creat = $user->id;
			$doliEsign->fk_soc = $object->socid;
			$res = $doliEsign->create($user);
			if ($res < 0) {
		...
		$ysResult = ysInfo($user, $object, 'info', $ys);
 ...
 *
 * @return mixed int with null = no info, lowest status = OK, array with errors if NOK
 */
function ysInfo($user, $object, $mode='sync', $ys = null)
{
	global $langs;
	...
	$status = null;
	$doliEsign = new DoliEsign($object->db);
	$tokens = $doliEsign->fetchTokens($objectId, $objectType);
	if (is_array($tokens) && count($tokens) > 0) {
		foreach ($tokens as $token) {
			if ($token->status == DoliEsign::STATUS_WAITING || $token->status == DoliEsign::STATUS_ERROR || $mode == 'info') {
				...
				$res = $doliEsign->fetch($token->id);
				if ($res < 0) {
					$errors = $doliEsign->errors;
					return $errors;
				} else {
					$doliEsign->sign_status = $ysResult['status'];
					$dateCreate = new DateTime($ysResult['dateCreation']);
					$doliEsign->date_creation = $dateCreate->getTimestamp();
					...
					if (isset($dateSign)) {
						$doliEsign->date_sign = $dateSign->getTimestamp();
						$dateSign = null;
					}
					if (isset($statusSign)) {
						$doliEsign->sign_status = $statusSign;
						$statusSign = null;
					}
					if ($ysResult['status'] == 'COSIGNATURE_EVENT_REQUEST_PENDING' || $ysResult['status'] == 'COSIGNATURE_EVENT_PROCESSING') {
						...
						// signed
						$doliEsign->status = DoliEsign::STATUS_SIGNED;
					} elseif ($ysResult['status'] == 'COSIGNATURE_EVENT_CANCELLED') {
						// canceled
						$doliEsign->status = DoliEsign::STATUS_CANCELED;
					} elseif ($ysResult['status'] == 'COSIGNATURE_EVENT_PARTIAL_ERROR') {
						// Error
						$doliEsign->status = DoliEsign::STATUS_ERROR;
					}
					$status = $doliEsign->status;
					$doliEsign->fk_user_modif=$user->id;
					$res = $doliEsign->update($user);
					if ($res < 0) {
						$errors = $doliEsign->errors;
						return $errors;
					}
 ...
 * @param object	$doliEsign			doliesign object containing yousign id
 *
 * @return mixed 0 = OK, array with errors if NOK
 */
function ysCancel($user, $doliEsign)
{
	$idDemand = $doliEsign->sign_id;
 ...
 * @return mixed 0 = OK, array with errors if NOK
 */
function ysFetch($user, $object)
{
	$result = 0;
	$ysResult = null;
	$doliEsign = new DoliEsign($object->db);
	$tokens = $doliEsign->fetchTokens($object->id, $object->element);
	if (is_array($tokens) && count($tokens) > 0) {
		foreach ($tokens as $token) {
			if ($token->status == DoliEsign::STATUS_SIGNED || $token->status == DoliEsign::STATUS_FILE_FETCHED) {
				...
				if ($res < 0) {
					$errors = $doliEsign->errors;
					return $errors;
				} elseif (! empty($token->idDemand) && ! empty($doliEsign->id_file)) {
					if (! isset($ysResult)) {
						$ys = ysConnect();
						if (! $ys->isAuthenticated()) {
							$errors = $ys->getErrors();
						...
						$ysResult = $ys->getCosignedFileFromIdDemand($token->idDemand, $doliEsign->id_file);
						if($ysResult === false) {
							$errors = $ys->getErrors();
						} else {
							$file = $doliEsign->path_file;
							$res = file_put_contents($file, base64_decode($ysResult['file']));
							if ($res > 0) {
								$result = 1;
							} else {
								$errors[] = "DoliEsignCreateFileError";
								return $errors;
							}
						}
					}
					if ($result == 1) {
						$doliEsign->status = DoliEsign::STATUS_FILE_FETCHED;
						$doliEsign->fk_user_modif=$user->id;
						$res = $doliEsign->update($user);
						if ($res < 0) {
							$errors = $doliEsign->errors;
...
function ysFixMobile($mobile, $countryCode)
{
