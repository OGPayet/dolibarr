...
if (empty($action) && empty($id)) $action='create';
...
if (empty($reshook))
{
	...
	$backurlforlist = dol_buildpath('/doliesign/config_list.php',1);
	...
	if ($action == 'update' && ! empty($permissiontoadd))
	{
		...
		foreach ($object->fields as $key => $val)
		{
			if (null === GETPOST($key)) continue;		// The field was not submited to be edited
			if (in_array($key, array('rowid', 'entity', 'date_creation', 'tms', 'fk_user_creat', 'fk_user_modif', 'import_key'))) continue;	// Ignore special fields
...
llxHeader('','Config','EN:Module_DoliEsign_EN|FR:Module_DoliEsign_FR|ES:Module_DoliEsign_EN');
...
if ($action == 'create')
{
	print load_fiche_titre($langs->trans("NewObject", $langs->transnoentitiesnoconv("Config")));
	...
	foreach($object->fields as $key => $val)
	{
		...
		if ($key == 'fk_c_type_contact') continue;	// We want this fields only in edit mode
		if ($key == 'rowid') continue;	// We don't want this field
		...
		print $object->showInputField($val, $key, $value, $action, '', '', '', 0);
		print '</td>';
		print '</tr>';
	}
...
if ($id && $action == 'edit')
{
	...
	foreach($object->fields as $key => $val)
	{
		...
		print '<td>';
		if (in_array($val['type'], array('int', 'integer'))) $value = GETPOST($key, 'int')?GETPOST($key, 'int'):$object->$key;
		elseif ($val['type'] == 'text' || $val['type'] == 'html') $value = GETPOST($key,'none')?GETPOST($key,'none'):$object->$key;
		else $value = GETPOST($key,'none')?GETPOST($key, 'alpha'):$object->$key;
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create')))
{
	...
	if ($action == 'delete') {
	    $formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $object->id, $langs->trans('DeleteConfig'), $langs->trans('ConfirmDeleteConfig'), 'confirm_delete', '', 0, 1);
	}
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="' .dol_buildpath('/doliesign/config_list.php',1) . '?restore_lastsearch_values=1' . (! empty($socid) ? '&socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';

	$morehtmlref='<div class="refidno">';
	/*
	// Ref bis
	$morehtmlref.=$form->editfieldkey("RefBis", 'ref_client', $object->ref_client, $object, $user->rights->doliesign->creer, 'string', '', 0, 1);
	$morehtmlref.=$form->editfieldval("RefBis", 'ref_client', $object->ref_client, $object, $user->rights->doliesign->creer, 'string', '', null, null, '', 1);
	// Thirdparty
	...
	if (! empty($conf->projet->enabled))
	{
	    ...
	    if ($user->rights->doliesign->creer)
	    {
	        if ($action != 'classify')
	        {
	...
	*/
	...
	dol_banner_tab($object, 'rowid', $linkback, 0, 'rowid', 'rowid', $morehtmlref);
	...
	if (empty($reshook))
	{
		...
		if ($user->rights->doliesign->delete)
		{
	...
	print '<div class="link"><a href="https://placeit.yousign.fr/" target="_blank">'.$langs->trans('URLtoDefineCoordinates').'</a></div>';
}
