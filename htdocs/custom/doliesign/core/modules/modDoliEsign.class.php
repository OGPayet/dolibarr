...
/**
 * 	\defgroup   doliesign     Module DoliEsign
 *  \brief      DoliEsign module descriptor.
 *
 *  \file       htdocs/doliesign/core/modules/modDoliEsign.class.php
 *  \ingroup    doliesign
 *  \brief      Description and activation file for module DoliEsign
 */
...
/**
 *  Description and activation class for module DoliEsign
 */
class modDoliEsign extends DolibarrModules
{
	...
	public function __construct($db)
	{
		...
		$this->numero = 202110;		// TODO Go on page https://wiki.dolibarr.org/index.php/List_of_modules_id to reserve id number for your module
		// Key text used to identify module (for permissions, menus, etc...)
		$this->rights_class = 'doliesign';
		...
		// for default path (eg: /doliesign/core/xxxxx) (0=disable, 1=enable)
		...
		// Example: this->dirs = array("/doliesign/temp","/doliesign/subdir");
		$this->dirs = array("/doliesign/");

		// Config pages. Put here list of php page, stored into doliesign/admin directory, to use to setup module.
		$this->config_page_url = array("setup.php@doliesign");
		...
		$this->langfiles = array("doliesign@doliesign");
		$this->phpmin = array(5,3);					// Minimum version of PHP required by module
		$this->need_dolibarr_version = array(4,0);	// Minimum version of Dolibarr required by module
		$this->warnings_activation = array();                     // Warning to show when we activate module. array('always'='text') or array('FR'='textfr','ES'='textes'...)
		$this->warnings_activation_ext = array();                 // Warning to show when we activate an external module. array('always'='text') or array('FR'='textfr','ES'='textes'...)
		//$this->automatic_activation = array('FR'=>'DoliEsignWasAutomaticallyActivatedBecauseOfYourCountryChoice');
		//$this->always_enabled = true;								// If true, can't be disabled

		// Constants
		// List of particular constants to add when module is enabled (key, 'chaine', value, desc, visible, 'current' or 'allentities', deleteonunactive)
		// Example: $this->const=array(0=>array('DOLIESIGN_MYNEWCONST1','chaine','myvalue','This is a constant to add',1),
		//                             1=>array('DOLIESIGN_MYNEWCONST2','chaine','myvalue','This is another constant to add',0, 'current', 1)
		// );
		$this->const = array(
		...
		{
			$conf->doliesign=new stdClass();
			$conf->doliesign->enabled=0;
		}
        ...
        $this->tabs = array();
		// Example:
		// $this->tabs[] = array('data'=>'objecttype:+tabname1:Title1:mylangfile@doliesign:$user->rights->doliesign->read:/doliesign/mynewtab1.php?id=__ID__');  					// To add a new tab identified by code tabname1
        // $this->tabs[] = array('data'=>'objecttype:+tabname2:SUBSTITUTION_Title2:mylangfile@doliesign:$user->rights->othermodule->read:/doliesign/mynewtab2.php?id=__ID__',  	// To add another new tab identified by code tabname2. Label will be result of calling all substitution functions on 'Title2' key.
        ...
        $this->dictionaries=array(
            ...
            'tabcond'=>array($conf->doliesign->enabled,$conf->doliesign->enabled,$conf->doliesign->enabled)												// Condition to show each dictionary
        );
        */
		...
		$this->cronjobs = array(
			0=>array('label'=>'MyJob label', 'jobtype'=>'method', 'class'=>'/doliesign/class/doliesign.class.php', 'objectname'=>'doliesign', 'method'=>'doScheduledJob', 'parameters'=>'', 'comment'=>'Comment', 'frequency'=>2, 'unitfrequency'=>3600, 'status'=>0, 'test'=>true)
		);
		...
		$this->rights[$r][1] = 'ReadDoliEsign';	// Permission label
		$this->rights[$r][3] = 1; 					// Permission by default for new user (0/1)
		$this->rights[$r][4] = 'read';				// In php code, permission will be checked by test if ($user->rights->doliesign->level1->level2)
		$this->rights[$r][5] = '';				    // In php code, permission will be checked by test if ($user->rights->doliesign->level1->level2)

		$r++;
		$this->rights[$r][0] = $this->numero + $r;	// Permission id (must not be already used)
		$this->rights[$r][1] = 'CreateUpdateDoliEsign';	// Permission label
		$this->rights[$r][3] = 1; 					// Permission by default for new user (0/1)
		$this->rights[$r][4] = 'create';				// In php code, permission will be checked by test if ($user->rights->doliesign->level1->level2)
		$this->rights[$r][5] = '';				    // In php code, permission will be checked by test if ($user->rights->doliesign->level1->level2)

		$r++;
		$this->rights[$r][0] = $this->numero + $r;	// Permission id (must not be already used)
		$this->rights[$r][1] = 'DeleteDoliEsign';	// Permission label
		$this->rights[$r][3] = 0; 					// Permission by default for new user (0/1)
		$this->rights[$r][4] = 'delete';				// In php code, permission will be checked by test if ($user->rights->doliesign->level1->level2)
		$this->rights[$r][5] = '';				    // In php code, permission will be checked by test if ($user->rights->doliesign->level1->level2)
		...
		$this->menu[$r++]=array('fk_menu'=>'fk_mainmenu='.$mainmenu,			                // '' if this is a top menu. For left menu, use 'fk_mainmenu=xxx' or 'fk_mainmenu=xxx,fk_leftmenu=yyy' where xxx is mainmenucode and yyy is a leftmenucode
								...
								'position'=>1000+$r,
								'enabled'=>'$conf->doliesign->enabled',	// Define condition to show or hide menu entry. Use '$conf->doliesign->enabled' if entry must be visible if module is enabled.
		...
		$this->menu[$r++]=array('fk_menu'=>'fk_mainmenu='.$mainmenu.',fk_leftmenu=doliesign',	    // '' if this is a top menu. For left menu, use 'fk_mainmenu=xxx' or 'fk_mainmenu=xxx,fk_leftmenu=yyy' where xxx is mainmenucode and yyy is a leftmenucode
								...
								'langs'=>'doliesign@doliesign',	        // Lang file to use (without .lang) by module. File must be in langs/code_CODE/ directory.
								'position'=>1100+$r,
								'enabled'=>'$conf->doliesign->enabled',  // Define condition to show or hide menu entry. Use '$conf->doliesign->enabled' if entry must be visible if module is enabled. Use '$leftmenu==\'system\'' to show if leftmenu system is selected.
								'perms'=>'$user->rights->doliesign->read',			                // Use 'perms'=>'$user->rights->doliesign->level1->level2' if you want your menu with a permission rules
								'target'=>'',
								'user'=>0);				                // 0=Menu for internal users, 1=external users, 2=both
		$this->menu[$r++]=array('fk_menu'=>'fk_mainmenu='.$mainmenu.',fk_leftmenu=doliesign',	    // '' if this is a top menu. For left menu, use 'fk_mainmenu=xxx' or 'fk_mainmenu=xxx,fk_leftmenu=yyy' where xxx is mainmenucode and yyy is a leftmenucode
		...
		$this->export_label[$r]='doliesignLines';	// Translation key (used only if key ExportDataset_xxx_z not found)
		$this->export_icon[$r]='doliesign@doliesign';
		$keyforclass = 'doliesign'; $keyforclassfile='/mymobule/class/doliesign.class.php'; $keyforelement='doliesign';
		include DOL_DOCUMENT_ROOT.'/core/commonfieldsinexport.inc.php';
		$keyforselect='doliesign'; $keyforaliasextra='extra'; $keyforelement='doliesign';
		include DOL_DOCUMENT_ROOT.'/core/extrafieldsinexport.inc.php';
		//$this->export_dependencies_array[$r]=array('mysubobject'=>'ts.rowid', 't.myfield'=>array('t.myfield2','t.myfield3')); // To force to activate one or several fields if we select some fields that need same (like to select a unique key if we ask a field of a child to avoid the DISTINCT to discard them, or for computed field than need several other fields)
		$this->export_sql_start[$r]='SELECT DISTINCT ';
		$this->export_sql_end[$r]  =' FROM '.MAIN_DB_PREFIX.'doliesign as t';
		$this->export_sql_end[$r] .=' WHERE 1 = 1';
		$this->export_sql_end[$r] .=' AND t.entity IN ('.getEntity('doliesign').')';
		$r++; */
	...
	{
		$this->_load_tables('/doliesign/sql/');

		// Create extrafields
		include_once DOL_DOCUMENT_ROOT.'/core/class/extrafields.class.php';
		$extrafields = new ExtraFields($this->db);

		//$result1=$extrafields->addExtraField('myattr1', "New Attr 1 label", 'boolean', 1,  3, 'thirdparty',   0, 0, '', '', 1, '', 0, 0, '', '', 'doliesign@doliesign');
		//$result2=$extrafields->addExtraField('myattr2', "New Attr 2 label", 'varchar', 1, 10, 'project',      0, 0, '', '', 1, '', 0, 0, '', '', 'doliesign@doliesign');
		//$result3=$extrafields->addExtraField('myattr3', "New Attr 3 label", 'varchar', 1, 10, 'bank_account', 0, 0, '', '', 1, '', 0, 0, '', '', 'doliesign@doliesign');
		...
		//$result5=$extrafields->addExtraField('myattr5', "New Attr 5 label", 'text',    1, 10, 'user',         0, 0, '', '', 1, '', 0, 0, '', '', 'doliesign@doliesign');
