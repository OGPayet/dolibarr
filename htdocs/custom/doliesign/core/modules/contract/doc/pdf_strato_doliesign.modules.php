...
/**
 *	\file       htdocs/doliesign/core/modules/contract/doc/pdf_strato_doliesign.modules.php
 *	\ingroup    ficheinter
 ...
 */
class pdf_strato_doliesign extends ModelePDFContract
{
	var $db;
	var $name;
	var $description;
...
*/
		...
		if (empty($hidebottom))
		{
			...
			$hookmanager->initHooks(array('pdfgeneration'));
			$parameters=array('pdf'=>$pdf, 'outputlangs'=>$outputlangs, 'posy'=>$posy,
				'tab'=>7, 'posx'=>$this->marge_gauche, 'fontSize'=>$default_font_size, 'largcol'=>$posmiddle - $this->marge_gauche - 5, 'sourceContact'=>'internal');
			$reshook=$hookmanager->executeHooks('changeSignatureArea', $parameters, $object, $action);
			...
			$parameters['posx']=$posmiddle + 5;
			$parameters['largcol']=$this->page_largeur-$this->marge_droite - $posmiddle - 5;
			$parameters['sourceContact']='external';
			$reshook=$hookmanager->executeHooks('changeSignatureArea', $parameters, $object, $action);
