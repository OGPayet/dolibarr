...
$(document).on("change", 'input#productid, input#serviceid', function (e) {
	var id = $(this).val();
	setTimeout(function(){
		var text =  $('#search_'+e.currentTarget.id).val();

		// Check si le produit est déjà dans l'ouvrage
		if (id > 0 && $( "#sortable input[type=hidden][name='products[]'][value="+id+"]" ).length == 0) {
			var newelt = $('<li class="ui-state-default"></li>');
			$(newelt).append('<span class="icon-drag-drop"></span>');
			$(newelt).append('<input type="hidden" name="products[]" value="'+id+'" />');
			$(newelt).append(text);
			$(newelt).append('<input type="number" name="quantity[]" value="1" length="4" />');
			$(newelt).append('<a href="#" class="delete">X</a>');
			$( "#sortable" ).append($(newelt));

			setTimeout(function(){ $('#productid, #serviceid').val(0); }, 20);
			setTimeout(function(){ $('#search_productid, #search_serviceid').val(''); }, 20);
		}
	}, 20);
});
