...
?>
/* Ouvrage Script for Dolibarr Version upper than 10 */
$().ready(function () {
    $("#addProduct").show();

    $(document).on("select2:selecting", '#serviceid', function (e) {
    ...
    $(document).on("autocompleteselect", '#search_serviceid', function (event, ui) {
    ...
    $("#addProduct").click(function () {
        $.getJSON("<?php echo dol_buildpath('/variants/ajax/getCombinations.php', 1) ?>", {
            id: $("#productid").val()
        }, function (data) {
            var productREF = [];
            var nbNull = 0;
            ...
            for (i = data.length - 1; i >= 0; i--) {
                productREF[i] = $("select[name='combinations[" + (i + 1) + "]']").val();
                if (productREF[i] == "") {
                    nbNull++;
                }
            }
            ...
            $.ajax({
                ...
                if (data.error != 0) {
                    ...
                    addProductOuvrage(data.productID, data.productLabel);
                }
            }).fail(function () {
