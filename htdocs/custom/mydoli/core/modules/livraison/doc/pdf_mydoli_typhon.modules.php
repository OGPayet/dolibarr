 ...
 */
class pdf_mydoli_typhon extends ModelePDFDeliveryOrder
{
    var $db;
    var $name;
    var $description;
	...
	function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0)
	{
		...
		if ($conf->expedition->dir_output)
		{
			...
			$pdf->commitTransaction();
					...
					$pdf->MultiCell($this->page_largeur-$this->marge_droite - $this->posxremainingqty, 3, $object->lines[$i]->qty_shipped, 0, 'C');
	...
	function _tableau_info(&$pdf, $object, $posy, $outputlangs)
	{
		...
		if ($conf->expedition->dir_output)
		{
			...
			if (file_exists($file))
			{
				$pdf->Image($file, $this->page_largeur-70, $this->page_hauteur-47, 0, 28);	// width=0 (auto)
			}
		}

		$pdf->Rect(2*$larg_sign+$this->marge_gauche, $posy + 4, $larg_sign, 30);
		$pdf->SetXY(2*$larg_sign+$this->marge_gauche + 2, $posy );
		$pdf->MultiCell($larg_sign,2, $outputlangs->trans("Signature du client").':','','L');
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
		...
		if (empty($hidetop)) {
			...
			$pdf->MultiCell($this->posxremainingqty - $this->posxcomm,2, $outputlangs->transnoentities("Comments"),'','L');
		}
		...
		if (empty($hidetop)) {
			...
			$pdf->MultiCell($this->page_largeur-$this->marge_droite-$this->posxremainingqty, 2, $outputlangs->transnoentities("Quantité"),'','R');
		}
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		$pdf->SetXY($posx,$posy);
		$pdf->SetFont('','',$default_font_size - 1);
		$pdf->MultiCell(100, 2, $outputlangs->transnoentities("Réf:")." ".$outputlangs->convToOutputCharset($object->ref_customer), '', 'R');
		...
		if ($object->client)
			$clientBL=$object->client;
		else
			$clientBL=$object->thirdparty;
		if ($clientBL->code_client)
		{
			$posy+=5;
			$pdf->SetXY($posx,$posy);
			$pdf->SetTextColor(0,0,60);
			$pdf->MultiCell(100, 3, $outputlangs->transnoentities("CustomerCode")." : " . $outputlangs->transnoentities($clientBL->code_client), '', 'R');
		}
		...
		if ($showaddress)
		{
			...
			$carac_client=pdf_build_address($outputlangs,$this->emetteur,$clientBL,($usecontact?$object->contact:''),$usecontact,'target');
