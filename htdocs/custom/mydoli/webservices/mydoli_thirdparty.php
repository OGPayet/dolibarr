...
$thirdparty_fields= array(
	...
	'remise_client' => array('name'=>'remise_client','type'=>'xsd:double'));
...
$server->register(
    'getClientsGPS',
    // Entry values
    array('authentication'=>'tns:authentication','latitude'=>'xsd:string','longitude'=>'xsd:string'),
    // Exit values
    array('result'=>'tns:result','coords'=>'tns:clientGPSArray2'),
    $ns,
    $ns.'#getClientsGPS',
    $styledoc,
    $styleuse,
    'WS to get GPS from clients'
);
			...
			if ($result > 0)
			{
				...
				$thirdparty_result_fields=array(
					...
					'date_modification' => dol_print_date($thirdparty->date_modification,'dayhourrfc'),
						...
						'remise_client' => $thirdparty->remise_percent
						);
...
function createThirdParty($authentication,$thirdparty)
{
    ...
    if (! $error)
    {
		...
		{
			$tabCountry=getCountry('','all',0,'',1,$thirdparty['country']);
			if (ISSET($tabCountry['id']))
			{
				$newobject->country_id=$tabCountry['id'];
				$newobject->country_code=$tabCountry['code'];
				$newobject->country=$tabCountry['label'];
			}
			else
			{
				$newobject->country=$thirdparty['country'];
			}
		}
        ...
        {
            $db->commit();
            if ($newobject->client==2)
            {
				$newobject->stcomm_id=$thirdparty['status'];
				$newobject->set_commnucation_level($fuser);
			}
            // Patch to add capability to associate (one) sale representative
            //if($thirdparty['commid'] && $thirdparty['commid']>0)
            if ($fuser->admin==0) $newobject->add_commercial($fuser, $fuser->id);
	...
	if (! $error)
	{
		...
		if (!empty($object->id)) {
			...
			if ($thirdparty['country'])
			{
				...
				if (ISSET($tabCountry['id']))
				{
					$object->country_id=$tabCountry['id'];
					$object->country_code=$tabCountry['code'];
					$object->country=$tabCountry['label'];
				}
				else
				{
					$object->country=$thirdparty['country'];
				}
			}
		...
		{
			$db->commit();
			$object->stcomm_id=$thirdparty['status'];
			$object->set_commnucation_level($fuser);

			$object->update_note(dol_html_entity_decode($thirdparty['note_public'], ENT_QUOTES),'_public');
			$object->update_note(dol_html_entity_decode($thirdparty['note_private'], ENT_QUOTES),'_private');

			if ($fuser->admin==0) $object->add_commercial($fuser,$fuser->id);
...
function getListOfThirdParties($authentication,$filterthirdparty)
{
    ...
    if (! $error)
    {
        $sql  = "SELECT s.rowid as socRowid, s.nom as ref, s.ref_ext, s.address, s.zip, s.town, c.label as country, s.phone, s.fax, s.email, s.url, s.client, s.fournisseur, s.remise_client, s.code_compta, s.fk_stcomm, s.note_public, s.note_private, extra.*";
        ...
        foreach($filterthirdparty as $key => $val)
        {
            ...
            if ($key == 'client'   && $val != '')  $tc=intval($val);
            if ($key == 'supplier'   && $val != '')  $tf=intval($val);
        ...
        {
		if ($tf!=0) $sql.=" AND s.fournisseur = ".$tf;
        }
        ...
        if ($resql)
        {
            ...
            while ($i < $num)
            {
                ...
                if ($fuser->admin==0)
                {
			$ok=isCommercial($obj->socRowid,$fuser->id);
                }
                if ($ok==1)
                {
			$arraythirdparties[]=array('id'=>$obj->socRowid,
                    ...
                    'url'=>$obj->url,
                    'email' => $obj->email,
                    'remise_client' => $obj->remise_client,
                    'note_public' => $obj->note_public,
                    'note_private' => $obj->note_private,
                    'status' => $obj->fk_stcomm,
                    'customer_code_accountancy' => $obj->code_compta
			);
			$arraythirdparties[$c] = array_merge($arraythirdparties[$c],$extrafieldsOptions);
			$c++;
				}
                $i++;
            }
...
function getClientsGPS($authentication,$latitude,$longitude)
{
	global $db,$conf,$langs,$mysoc;

	dol_syslog("Function: getClientsGPS login=".$authentication['login']);
	...
	if (! $error)
	{
		$sql = "SELECT fk_soc,latitude,longitude,111.045*DEGREES(ACOS(COS(RADIANS(".$latitude."))*COS(RADIANS(latitude))*COS(RADIANS(".$longitude.")-RADIANS(longitude))+SIN(RADIANS(".$latitude."))*SIN(RADIANS(latitude)))) AS distance FROM ".MAIN_DB_PREFIX."societe_gps WHERE entity=".$conf->entity." ORDER BY distance";
		$resql=$db->query($sql);
		if ($resql)
		{
			$lines=array();
			...
			while ($i < $num)
			{
				...
				$obj=$db->fetch_object($resql);
		if ($obj->distance>20) break;

				$sql="SELECT rowid,nom FROM ".MAIN_DB_PREFIX."societe WHERE rowid=".$obj->fk_soc;
				$resql2=$db->query($sql);
				if ($resql2)
				{
					$cli=$db->fetch_object($resql2);
					$lines[]=array(
					'id' => $obj->fk_soc,
					'client' => $cli->nom,
