...
function getContact($authentication,$id,$ref_ext)
{
    ...
    if (! $error)
    {
		...
		// Or for external user who have contact read permission, with restrict on societe_id
			...
			$sql = "SELECT rowid,label FROM ".MAIN_DB_PREFIX."c_country WHERE rowid=".$contact->country_id;
				$resql=$db->query($sql);
				if ($resql)
				{
...
function getContactsForThirdParty($authentication,$idthirdparty)
{
	...
	if (! $error)
	{
		...
		if ($idthirdparty!='all') $sql.= " WHERE c.fk_soc=$idthirdparty";
...
function deleteContact($authentication,$id)
{
    global $db,$conf,$langs,$mysoc;

    dol_syslog("Function: deleteContact login=".$authentication['login']." id=".$id);
