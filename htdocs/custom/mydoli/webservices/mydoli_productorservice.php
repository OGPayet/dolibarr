...
$productorservice_fields = array(
	...
	'accountancy_code_sell' => array('name'=>'accountancy_code_sell','type'=>'xsd:string'),
    ...
    'stock_maxi' => array('name'=>'stock_maxi','type'=>'xsd:string'),
...
function getProductOrService($authentication,$id='',$ref='',$ref_ext='',$lang='')
{
	...
	$langs->setDefaultLang($langcode);
		...
		$productorservice_result_fields = array(
		    ...
		    'stock_maxi' => $product->desiredstock,
...
function createProductOrService($authentication,$product)
{
    ...
    if (! $error)
    {
        ...
        $newobject->type=intval($product['type']);
        ...
        $newobject->accountancy_code_sell=$product['accountancy_code_sell'];
        $newobject->date_creation=$now;
		if (ISSET($product['url'])) $newobject->url=$product['url'];
	...
	if ($product['barcode'])
	{
        ...
        $newobject->desiredstock=$product['stock_maxi'];
...
function updateProductOrService($authentication,$product)
{
	...
	foreach($extrafields->attribute_label as $key=>$label)
	{
        ...
        {
            $db->commit();
            $sql="UPDATE ".MAIN_DB_PREFIX."product SET fk_product_type=".intval($product['type'])." WHERE rowid=".$product['id'];
            $resql=$db->query($sql);
...
function getListOfProductsOrServices($authentication,$filterproduct)
{
    ...
    if (! $error)
    {
        $sql ="SELECT rowid, ref, ref_ext,label,description,note,barcode,fk_barcode_type,price,price_ttc,tva_tx,stock,seuil_stock_alerte,fk_product_type,accountancy_code_sell";
		...
		while ($i < $num)
		{
			...
			$arrayproducts[]=array(
						...
						'type' => $obj->fk_product_type,
								...
								'stock_real' => $obj->stock,
						'stock_alert' => $obj->seuil_stock_alerte,
						        'stock_maxi' => $obj->desiredstock,
