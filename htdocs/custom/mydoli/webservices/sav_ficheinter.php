...
$server->register(
    ...
    $styledoc,
    $styleuse,
    'WS to genere PDF of an intervention'
);
//**** Test structure BDD
function testBDD()
{
	...
	//*** Test controles effectues
	$sql='SELECT *';
	$sql.=' FROM '.MAIN_DB_PREFIX.'extrafields';
	$sql.=" WHERE elementtype = 'fichinter' AND name LIKE 'c0%'";
	$resql=$db->query($sql);
	...
	if ($num<1)
	{
		...
		$extrafields->addExtraField("c001","Vérification de l'état nature et géométrie du conduit", "int",1,"10","fichinter",0,0,"",0,1,"",0);
		//echo $db->lasterror();

		$extrafields->addExtraField("c002","Nettoyage du corps de chauffe, veilleuse et extracteur", "int",2,"10","fichinter",0,0,"",0,1,"",0);
		$extrafields->addExtraField("c003","Démontage et nettoyage du bruleur", "int",3,"10","fichinter",0,0,"",0,1,"",0);
		$extrafields->addExtraField("c004","Vérification fonctionnelle de la sécurité", "int",4,"10","fichinter",0,0,"",0,1,"",0);
		$extrafields->addExtraField("c005","Vérification et réglage du débit de gaz ou fuel", "int",5,"10","fichinter",0,0,"",0,1,"",0);
		$extrafields->addExtraField("c006","Vérification fonctionnelle du circulateur de chauffage", "int",6,"10","fichinter",0,0,"",0,1,"",0);
		$extrafields->addExtraField("c007","Vérification et réglage des organes de circulation", "int",7,"10","fichinter",0,0,"",0,1,"",0);
		$extrafields->addExtraField("c008","Vérification anti-refoulement des fumées", "int",8,"10","fichinter",0,0,"",0,1,"",0);
		$extrafields->addExtraField("c009","Chaudière + ballon accumulation, vérification anode", "int",9,"10","fichinter",0,0,"",0,1,"",0);
		$extrafields->addExtraField("c010","Vérification de la pression d'eau", "int",10,"10","fichinter",0,0,"",0,1,"",0);
	}

	//*** Test controles effectues
	$sql='SELECT *';
	$sql.=' FROM '.MAIN_DB_PREFIX.'extrafields';
	$sql.=" WHERE elementtype = 'fichinter' AND name LIKE 'm0%'";
	$resql=$db->query($sql);
	$num=$db->num_rows($resql);
	if ($num<1)
	{
		...
		$extrafields->addExtraField("m001a","Température des fumées", "int",11,"10","fichinter",0,0,"",0,1,"",0);
		$extrafields->addExtraField("m001b","Température des fumées", "int",12,"10","fichinter",0,0,"",0,1,"",0);
		$extrafields->addExtraField("m002a","Teneur en O²", "int",13,"10","fichinter",0,0,"",0,1,"",0);
		$extrafields->addExtraField("m002b","Teneur en O²", "int",14,"10","fichinter",0,0,"",0,1,"",0);
		$extrafields->addExtraField("m003a","Teneur en CO²", "int",15,"10","fichinter",0,0,"",0,1,"",0);
		$extrafields->addExtraField("m003b","Teneur en CO²", "int",16,"10","fichinter",0,0,"",0,1,"",0);
		$extrafields->addExtraField("m004a","Teneur en CO", "int",17,"10","fichinter",0,0,"",0,1,"",0);
		$extrafields->addExtraField("m004b","Teneur en CO", "int",18,"10","fichinter",0,0,"",0,1,"",0);
		$extrafields->addExtraField("m005a","Teneur en CO ambiant", "int",19,"10","fichinter",0,0,"",0,1,"",0);
		$extrafields->addExtraField("m005b","Teneur en CO ambiant", "int",20,"10","fichinter",0,0,"",0,1,"",0);
		$extrafields->addExtraField("m006a","Température ambiante", "int",21,"10","fichinter",0,0,"",0,1,"",0);
		$extrafields->addExtraField("m006b","Température ambiante", "int",22,"10","fichinter",0,0,"",0,1,"",0);
		$extrafields->addExtraField("m007a","Opacité des fumées", "int",23,"10","fichinter",0,0,"",0,1,"",0);
		$extrafields->addExtraField("m007b","Opacité des fumées", "int",24,"10","fichinter",0,0,"",0,1,"",0);
		$extrafields->addExtraField("m008a","Rendement chaudière", "int",25,"10","fichinter",0,0,"",0,1,"",0);
		$extrafields->addExtraField("m008b","Rendement chaudière", "int",26,"10","fichinter",0,0,"",0,1,"",0);
		$extrafields->addExtraField("m009a","Rendement de référence 2009", "int",27,"10","fichinter",0,0,"",0,1,"",0);
		$extrafields->addExtraField("m009b","Rendement de référence 2009", "int",28,"10","fichinter",0,0,"",0,1,"",0);
		$extrafields->addExtraField("m010a","Facteur d'air", "int",29,"10","fichinter",0,0,"",0,1,"",0);
		$extrafields->addExtraField("m010b","Facteur d'air", "int",30,"10","fichinter",0,0,"",0,1,"",0);
	}
}
...
function getFicheinter($authentication,$id='',$ref='',$ref_ext='')
{
	...
	if (! $error)
	{
		...
		//if ($fuser->rights->fichinter->lire)
		//{
			$linesFiches=array();
...
function getExtras($authentication)
{
	...
	if (! $error)
	{
		...
		$sql = "SELECT name,label,elementtype,entity FROM ".MAIN_DB_PREFIX."extrafields WHERE elementtype='fichinter' AND entity=".$conf->entity." ORDER BY name";
		$resql=$db->query($sql);
		if ($resql)
		{
			$num=$db->num_rows($resql);
...
function createFicheinter($authentication,$ficheinter)
{
 ...
 //           	$newobject->generateDocument("", $langs);
			...
			//tester si Extra ? si oui alors 'mydolisav_soleil'
...
function genereFicheinter($authentication,$id)
{
    global $db,$conf,$langs,$mysoc;

    $now=dol_now();

    dol_syslog("Function: genereFicheinter login=".$authentication['login']);
