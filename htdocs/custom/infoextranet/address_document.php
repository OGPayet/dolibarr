...
/**
 *  \file       htdocs/modulebuilder/template/address_document.php
 *  \ingroup    infoextranet
 *  \brief      Tab for documents linked to AddressExtraﬁ
 */
...
$hookmanager->initHooks(array('addressdocument'));     // Note that conf->hooks_modules contains array
...
if ($id > 0 || ! empty($ref)) $upload_dir = $conf->infoextranet->multidir_output[$object->entity] . "/address/" . dol_sanitizeFileName($object->ref);
...
$title=$langs->trans("AddressExtra").' - '.$langs->trans("Files");
$help_url='';
...
if ($object->id)
{
	...
	dol_fiche_head($head, 'document', $langs->trans("Address"), -1, 'address@infoextranet');
    ...
    //$relativepathwithnofile='address/' . dol_sanitizeFileName($object->id).'/';
    $relativepathwithnofile='address/' . dol_sanitizeFileName($object->ref).'/';
