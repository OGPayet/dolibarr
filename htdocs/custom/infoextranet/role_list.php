...
$contextpage= GETPOST('contextpage','aZ')?GETPOST('contextpage','aZ'):'rolelist';   // To manage different context of search
...
$hookmanager->initHooks(array('rolelist'));     // Note that conf->hooks_modules contains array
...
//$help_url="EN:Module_Role|FR:Module_Role_FR|ES:Módulo_Role";
$help_url='';
$title = $langs->trans('ListOfRoles');
...
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."role_extrafields as ef on (t.rowid = ef.fk_object)";
if ($object->ismultientitymanaged == 1) $sql.= " WHERE t.entity IN (".getEntity('role').")";
else $sql.=" WHERE 1 = 1";
...
if ($num == 1 && ! empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/infoextranet/role_card.php?id='.$id);
	exit;
}
...
$topicmail="SendRoleRef";
$modelmail="role";
$objecttmp=new Role($db);
