...
/**
 *  \file       role_document.php
 *  \ingroup    infoextranet
 *  \brief      Tab for documents linked to Role
 */
...
$hookmanager->initHooks(array('roledocument'));     // Note that conf->hooks_modules contains array
...
if ($id > 0 || ! empty($ref)) $upload_dir = $conf->infoextranet->multidir_output[$object->entity] . "/role/" . dol_sanitizeFileName($object->ref);
...
$title=$langs->trans("Role").' - '.$langs->trans("Files");
$help_url='';
...
if ($object->id)
{
	...
	dol_fiche_head($head, 'document', $langs->trans("Role"), -1, 'role@infoextranet');
	...
	//$relativepathwithnofile='role/' . dol_sanitizeFileName($object->id).'/';
	$relativepathwithnofile='role/' . dol_sanitizeFileName($object->ref).'/';
