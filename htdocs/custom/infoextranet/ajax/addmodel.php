...
$inputModel = GETPOST('modelName');
$ret = 0;


if(!empty($inputModel)){
    $sql = 'INSERT INTO `llx_infoextranet_devicelabels` (`rowid`, `code`, `label`, `active`)
            VALUES (NULL, NULL, \''.$inputModel.'\', \'1\');';

    $result = $db->query($sql);
    if ($result){
        $lastID = $db->last_insert_id('llx_infoextranet_devicemodels');
        $ret = $lastID;
