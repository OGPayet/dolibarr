...
if (empty($selectedfields))
    $selectedfields = $conf->global->INFOEXTRANET_SELECTED_P;
...
// Code 42 : Change for section
$sql.= " FROM ".MAIN_DB_PREFIX."societe AS s INNER JOIN ".MAIN_DB_PREFIX."societe_extrafields AS e ON s.rowid = e.fk_object WHERE e.c42P_contract >= 0";
foreach ($extra as $key => $field)
{
...
if ($resql)
{
    ...
    // Code 42 : Change for section
    print_barre_liste('<i class="fa fa-wrench"></i> '.$langs->trans('TitleP'), $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder,'',$num,$totalnboflines,'',0,'','',$limit);
