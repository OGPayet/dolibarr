...
$hookmanager->initHooks(array('devicedocument'));     // Note that conf->hooks_modules contains array
...
if ($id > 0 || ! empty($ref)) $upload_dir = $conf->infoextranet->multidir_output[$object->entity] . "/device/" . dol_sanitizeFileName($object->ref);
...
if ($object->id)
{
	...
	dol_fiche_head($head, 'document', $langs->trans("Device"), -1, 'device@infoextranet');
