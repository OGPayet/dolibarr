...
/**
 *   	\file       sepamandat_list.php
 *		\ingroup    sepamandatmanager
 *		\brief      List page for sepamandat
 */
...
require_once __DIR__.'/class/sepamandat.class.php';
...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'sepamandatlist'; // To manage different context of search
...
$hookmanager->initHooks(array('sepamandatlist')); // Note that conf->hooks_modules contains array
...
$permissiontoadd = $user->rights->sepamandatmanager->sepamandat->write;
$permissiontodelete = $user->rights->sepamandatmanager->sepamandat->delete;
...
if (empty($reshook))
{
	...
	// Mass actions
	$objectclass = 'Sepamandat';
	$objectlabel = 'Sepamandat';
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page)
{
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/sepamandatmanager/sepamandat_card.php', 1).'?id='.$id);
	exit;
}
...
// Output page
// --------------------------------------------------------------------
//$help_url="EN:Module_Sepamandat|FR:Module_Sepamandat_FR|ES:Módulo_Sepamandat";
$help_url = '';
$title = $langs->trans('ListOfSepaMandat');
if ($socid > 0) {
	$societe = new Societe($db);
	$societe->fetch($socid);
	$title = $langs->trans('ListOfSepaMandatForCompany', $societe->name);
...
if ($socid > 0) {
    ...
    $active_tab = 'sepamandat_companytab';
...
$topicmail = "SendSepamandatRef";
$modelmail = "sepamandat";
$objecttmp = new Sepamandat($db);
