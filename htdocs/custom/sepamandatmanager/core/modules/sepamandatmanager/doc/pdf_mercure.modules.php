...
class pdf_mercure extends ModelePDFSepamandat
{
	...
	function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!empty($directoryPath)) {
			...
			if (file_exists($directoryPath)) {
				...
				if (!empty($object->iban)) $iban = $object->iban;
				$posY = $pdf->GetY();
				$posY += 1;
				$pdf->SetXY($this->marge_gauche, $posY);
				...
				$pdf->SetXY(80, $posY);
				if ($object->type == $object::TYPE_RECURRENT) $pdf->MultiCell(5, 3, 'X', 0, 'L');
				$pdf->SetXY(86, $posY);
				...
				$pdf->Rect(80, $posY, 5, 5);
				$pdf->SetXY(80, $posY);
				if ($object->type == $object::TYPE_PUNCTUAL) $pdf->MultiCell(5, 3, 'X', 0, 'L');
				$pdf->SetXY(86, $posY);
	...
	function _signature_area(&$pdf, $object, $posy, $outputlangs)
	{
		...
		if(!empty($object->date_rum)) {
			...
			$textDate .= '___________________________________';
		}
		$pdf->MultiCell(100, 3, $textDate, 0, 'L', 0);
