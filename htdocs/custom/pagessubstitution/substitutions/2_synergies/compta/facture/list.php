    ...
    else
    {
        ...
        foreach($arrayofselected as $toselectid)
        {
            ...
            if ($result > 0)
            {
				...
				//On vérifie que la facture n'a pas déjà une demande de prélèvement
				...
				//On vérifie que la facture n'a pas déjà un bon de prélèvement en attente ou envoyé
				$sql = "SELECT pb.rowid, pb.ref";
				$sql .= " FROM ".MAIN_DB_PREFIX."prelevement_lignes as pl,".MAIN_DB_PREFIX."prelevement_facture as pf, ".MAIN_DB_PREFIX."prelevement_bons as pb";
				$sql .= " WHERE pf.fk_prelevement_lignes = pl.rowid";
				$sql .= " AND pb.rowid = pl.fk_prelevement_bons";
				$sql .= " AND pb.statut IN (0,1)";
				$sql .= " AND pf.fk_facture=".$objecttmp->id;
                ...
                if($numprlv>0){
					$row = $db->fetch_object($result_sql);
					$url = '<a href="'.DOL_URL_ROOT.'/compta/prelevement/card.php?id='.$row->rowid.'">'.$row->ref.'</a>';
					$error++;
                    setEventMessages("La facture ".$objecttmp->ref." est déjà lié au bon de prélèvement ".$url, $objecttmp->errors, 'errors');
                }
				...
				$account->fetch(0,$objecttmp->socid);
				if(empty($account->date_signature)) {
					$soc = new Societe($db);
					$soc->fetch($objecttmp->socid);
					$langs->load("banks");

					$error++;
                    setEventMessages($objecttmp->ref .' '.$langs->trans("ErrorDateSignatureWithDraw",$soc->getNomUrl(0)), $objecttmp->errors, 'errors');
				}

				if(empty($error)) {
					$db->begin();
					$result = $objecttmp->demande_prelevement($user, $objecttmp->resteapayer);
					if ($result > 0) {
						$db->commit();
						$nbwithdrawrequestok++;
					} else {
...
if ($resql)
{
    ...
    if ($socid > 0)
	{
		$soc = new Societe($db);
		$soc->fetch($socid);
		$title = $langs->trans('BillsCustomers') . ' - '.$soc->name;
	}
	else
	{
	...
	print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'title_accountancy.png', 0, '', '', $limit);
	...
	if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label))
	{
	    foreach($extrafields->attribute_label as $key => $val)
	    {
	        if (! empty($arrayfields["ef.".$key]['checked']))
	        {
                ...
                //-------------------------------------------------------------------------------
                // Modification - Open-DSI - Begin
                } elseif (in_array($typeofextrafield, array('sellist'))) {
                    $tmpkey = preg_replace('/search_options_/', '', $key);
                    print $extrafields->showInputField($tmpkey, $search_array_options['search_options_' . $tmpkey], '', '', 'search_');
                // Modification - Open-DSI - End
                //-------------------------------------------------------------------------------
	            }
	            print '</td>';
