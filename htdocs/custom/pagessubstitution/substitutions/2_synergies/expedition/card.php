...
if (empty($reshook)) {
            ...
            if (!empty($conf->productbatch->enabled) && $objectsrc->lines[$i]->product_tobatch) {      // If product need a batch number
                ...
                while (GETPOSTISSET($stockLocation)) {
                    ...
                    if (GETPOST($productToSerializeFieldName, 'int') == 1) {
                        ...
                        if (!isset($equipementLineList[$sourceLineId])) {
                            $equipementLineList[$sourceLineId] = array();
                        }
                ...
                foreach ($object->lines as $expeditionLine) {
                    ...
                    if ($expeditionLine->fk_product > 0) {
                        ...
                        if ($productStatic->array_options['options_synergiestech_to_serialize'] == 1) {
                            ...
                            } elseif (count($expeditionLine->details_entrepot) > 1) {
                                // multi warehouses
    ...
    elseif ($action == 'update_equipement' && $user->rights->expedition->creer && $user->rights->equipement->creer) {
        $error = 0;
                ...
                if (empty($reshook)) {
                    ...
                    if ($warehouse_id > 0 || !($line->fk_product > 0) || empty($conf->stock->enabled)) {     // If warehouse was already selected or if product is not a predefined, we go into this part with no multiwarehouse selection
                        ...
                        if (empty($conf->productbatch->enabled) || !$product->hasbatch()) {
                            ...
                            // Product need lot
                            print '<td></td><td></td>'; // end line and start a new one for lot/serial
                            if (!empty($conf->equipement->enabled)) {
                                print '<td></td>';
                            }
