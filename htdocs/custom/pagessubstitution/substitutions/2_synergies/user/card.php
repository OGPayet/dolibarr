...
if(! empty($conf->multicompany->enabled) && ! empty($conf->global->MULTICOMPANY_TRANSVERSE_MODE)) {
	...
	$caneditgroup = $conf->entity == 1 && ! empty($user->admin);
...
if ($action == 'create' || $action == 'adduserldap')
{
    ...
    print '<td>';
    print $form->select_dolusers($object->fk_user, 'fk_user', 1, array($object->id), 0, '', 0, $conf->entity, 0, 0, ' AND u.fk_soc IS NULL', 0, '', 'maxwidth300');
    print '</td>';
    print "</tr>\n";
...
else
{
    ...
    if ($id > 0)
    {
		...
		print '</td></tr>';
            ...
            if ($caneditfield)
            {
		print $form->select_dolusers($object->fk_user, 'fk_user', 1, array($object->id), 0, '', 0, $object->entity, 0, 0, ' AND u.fk_soc IS NULL', 0, '', 'maxwidth300');
            }
            else
            {
