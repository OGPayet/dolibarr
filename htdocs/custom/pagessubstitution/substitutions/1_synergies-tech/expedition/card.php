...
if (!empty($conf->equipement->enabled)) $langs->load('equipement@equipement');
...
if (empty($reshook))
{
	...
	if ($action == 'add' && $user->rights->expedition->creer)
	{
		...
		$qty = "qtyl".$i;
            $productToSerializeFieldName = 'product_serializel' . $i . '_' . $j;
		$equipementFieldName = 'equipementl' . $i . '_' . $j;
			...
			else if (isset($_POST[$stockLocation]))
			{
			    ...
			    while (isset($_POST[$stockLocation]))
			    {
                    ...
                    // equipment list
                    if (GETPOST($productToSerializeFieldName, 'int') == 1) {
                        $stockLine[$i][$j]['equipement_list'] = GETPOST($equipementFieldName, 'array');

                        // check qty with list of equipments
                        if (count($stockLine[$i][$j]['equipement_list']) != intval($stockLine[$i][$j]['qty'])) {
                            $error++;
                            $object->error    = $langs->trans('Line') . ' ' . ($i+1) . ' - ' . ($j+1) . ' : ' . $langs->trans('EquipementErrorQtyToShip');
                            $object->errors[] = $object->error;
                        }

                        // equipment list by lines and warehouse
                        $sourceLineId = $objectsrc->lines[$i]->id;
                        $warehouseId  = $stockLine[$i][$j]['warehouse_id'];
                        if (!isset($equipementLineList[$sourceLineId]))  $equipementLineList[$sourceLineId] = array();
                        $equipementLineList[$sourceLineId][$warehouseId] = $stockLine[$i][$j]['equipement_list'];
                    }
	    ...
	    // check total qty to ship
	    if ($totalqty <= 0) {
            $error++;
            $object->error    = $langs->trans("ErrorFieldRequired",$langs->transnoentitiesnoconv("QtyToShip").'/'.$langs->transnoentitiesnoconv("Warehouse"));
            $object->errors[] = $object->error;
        }
        ...
        if (!$error && !empty($conf->equipement->enabled)) {
            ...
            // join all equipments to shipment
            foreach ($object->lines as $expeditionLine) {
                // only for products
                if ($expeditionLine->fk_product > 0) {
                    $productStatic = new Product($db);
                    $productStatic->fetch($expeditionLine->fk_product);

                    if ($productStatic->array_options['options_synergiestech_to_serialize'] == 1) {
                        $sourceLineId = $expeditionLine->fk_origin_line;

                        $expeditionEquipementLineList = array();
                        if ($expeditionLine->entrepot_id > 0) {
                            // one warehouse
                            $expeditionEquipementLineList[] = array('equipement_id_list' => $equipementLineList[$sourceLineId][$expeditionLine->entrepot_id], 'expedition_line_id' => $expeditionLine->id);
                        } else if (count($expeditionLine->details_entrepot) > 1) {
                            // multi warehouses
                            foreach ($expeditionLine->details_entrepot as $detail_entrepot) {
                                if ($detail_entrepot->entrepot_id > 0) {
                                    $expeditionEquipementLineList[] = array('equipement_id_list' => $equipementLineList[$sourceLineId][$detail_entrepot->entrepot_id], 'expedition_line_id' => $detail_entrepot->line_id);
                                }
                            }
                        }

                        // join all equipemnts lines in each warehouse
                        foreach ($expeditionEquipementLineList as $expeditionEquipementLineArray) {
                            ...
                            foreach ($expeditionEquipementLineArray['equipement_id_list'] as $equipementId) {
                                ...
                                if (!$error) {
                                    ...
                                    $sql .= " FROM " . MAIN_DB_PREFIX . "equipementassociation";
                                    $sql .= " WHERE fk_equipement_pere = " . $equipementId;

                                    $resql = $db->query($sql);
                                    if ($resql) {
                                        // add children components lines (events)
	    ...
	    if (! $error) {
	        ...
	        // Modification - Open-DSI - Begin - Hack for the redirect to attach equipments if has products serializable
            //if (!isset($object->context['workflow_to_serialize'])) {
            //    header("Location: card.php?id=".$object->id);
            //}
            // Modification - Open-DSI - End - Hack for the redirect to attach equipments if has products serializable
	        //exit;
            header("Location: card.php?id=" . $object->id);
            exit();
	    } else {
	...
	// update equipments lines
    else if ($action == 'update_equipement' && $user->rights->expedition->creer && $user->rights->equipement->creer) {
        $error = 0;

        $fkExpedition = GETPOST('id', 'int');
        $expeditionLineSuffixList = GETPOST('expedition_line_suffix_list', 'array') ? GETPOST('expedition_line_suffix_list', 'array') : array();

        if (!$error) {
            $db->begin();

            // for all equipments lines
            foreach ($expeditionLineSuffixList as $expeditionLineSuffix) {
                $expeditionTab = explode('_', $expeditionLineSuffix);
                $expeditionLineId = $expeditionTab[0];

                $sql  = "SELECT ee.rowid";
                $sql .= " FROM " . MAIN_DB_PREFIX . "equipementevt ee";
                $sql .= " WHERE ee.fk_expedition = " . $fkExpedition;
                ...
                if (!$error) {
                    // delete all equipments lines linked to this shipment line
                    while ($obj = $db->fetch_object($resql)) {
                        $equipementEvt = new Equipementevt($db);

                        $ret = $equipementEvt->fetch($obj->rowid);
                        if ($ret < 0) {
                            $error++;
                ...
                if (!$error) {
                    $equipementIdList    = GETPOST('equipementl_' . $expeditionLineSuffix, 'array');
                    $equipementQtyToShip = count($equipementIdList);

                    // add all equipments lines linked to this shipment line
                    foreach ($equipementIdList as $equipementId) {
                        // join equipment to this shipment
                        $equipement = new Equipement($db);
                        $ret = $equipement->fetch($equipementId);
                        if ($ret < 0) {
                            $error++;
                    ...
                    if (!$error) {
                        // update expedition line qty to ship
                        $expeditionLigne = new ExpeditionLigne($db);
                        $expeditionLigne->fetch($expeditionLineId);
                        $expeditionLigne->qty = $equipementQtyToShip;
                        $ret = $expeditionLigne->update($user);

                        if ($ret < 0) {
                            $error++;
                            $object->error = $expeditionLigne->error;
                            $object->errors[] = $object->error;
                        }
                    }
                }
	...
	)
	{
        ...
        // check all equipments quantities
        if (!$error && !empty($conf->equipement->enabled)) {
            $product_static = new Product($db);

            $num_prod = count($object->lines);
            for ($i = 0; $i < $num_prod; $i++) {
                $expeditionLine = $object->lines[$i];

                if ($expeditionLine->fk_product > 0) {
                    $product_static->fetch($expeditionLine->fk_product);
                    ...
                    if ($product_static->id > 0 && $product_static->array_options['options_synergiestech_to_serialize'] == 1) {
                        ...
                        $entrepotIdList = array();

                        if ($expeditionLine->entrepot_id > 0) {
                            $entrepotIdList[] = $expeditionLine->entrepot_id;
                        } else if (count($expeditionLine->details_entrepot) > 1) {
                            foreach ($expeditionLine->details_entrepot as $detail_entrepot) {
                                $entrepotIdList[] = $detail_entrepot->entrepot_id;
                            }
                        }

                        // find all equipments in shipment for this product and in warehouse
                        $sql  = "SELECT e.rowid, e.ref, e.fk_entrepot";
                        $sql .= " FROM " . MAIN_DB_PREFIX . "equipement as e";
                        $sql .= " LEFT JOIN " . MAIN_DB_PREFIX . "equipementevt as ee ON ee.fk_equipement = e.rowid";
                        $sql .= " WHERE ee.fk_expedition = " .$expeditionLine->fk_expedition;
                        $sql .= " AND e.fk_product = " . $product_static->id;
                        if (count($entrepotIdList) > 0) {
                            $sql .= " AND e.fk_entrepot IN (" . implode(',', $entrepotIdList) . ")";
                        }
                        ...
                        if (count($equipementList) != $expeditionLine->qty_shipped) {
                            $error++;
                            $object->error    = $langs->trans('EquipementErrorQtyToShip');
                            $object->errors[] = $object->error;
                        }
                    }
                }
        ...
        if (!$error) {
            ...
            if (!$error && !empty($conf->equipement->enabled)) {
                // find all equipments in shipment
                $sql  = "SELECT e.rowid";
                $sql .= " FROM " . MAIN_DB_PREFIX . "equipement as e";
                $sql .= " LEFT JOIN " . MAIN_DB_PREFIX . "equipementevt as ee ON ee.fk_equipement = e.rowid";
                ...
                if (!$error) {
                    while ($obj = $db->fetch_object($resql)) {
                        ...
                        // fetch equipment
                        $ret = $equipement->fetch($obj->rowid);
                        if ($ret < 0) {
                            $error++;
                            $object->error    = $equipement->error;
                            $object->errors[] = $object->error;
                        ...
                        if (!$error) {
                            ...
                            $ret = $equipement->set_entrepot($user, -1);
                            if ($ret < 0) {
                                $error++;
                                $object->error    = $equipement->error;
                                $object->errors[] = $object->error;
    ...
    if ($origin)
    {
		...
		while($i < $numAsked)
		{
			...
			print '<td align="left">'.$langs->trans("Warehouse").' ('.$langs->trans("Stock").')</td>';
                ...
                if (!empty($conf->equipement->enabled)) {
                    print '<td align="center">' . $langs->trans("Equipement") . '</td>';
                }
                print "</tr>\n";
            }
			...
			print $text.' '.nl2br($line->desc);
				...
				if ($warehouse_id > 0 || ! ($line->fk_product > 0) || empty($conf->stock->enabled))     // If warehouse was already selected or if product is not a predefined, we go into this part with no multiwarehouse selection
				{
					...
					if (empty($conf->productbatch->enabled) || ! $product->hasbatch())
					{
						...
						// Equipement
                        if (!empty($conf->equipement->enabled)) print '<td></td>';
					...
					// Define nb of lines suggested for this order line
						...
						foreach ($product->stock_warehouse as $warehouse_id=>$stock_warehouse)    // $stock_warehouse is product_stock
						{
							...
							if ($stock_warehouse->real > 0)
							{
								...
								// Equipement
                                if (!empty($conf->equipement->enabled)) {
                                    $productToSerialize          = 0;
                                    $productToSerializeFieldName = 'product_serializel' . $indiceAsked . '_' . $subj;

                                    $equipementFieldName   = 'equipementl' . $indiceAsked . '_' . $subj;
                                    $equipementList        = array();
                                    $idEquipementList      = GETPOST($equipementFieldName, 'array') ? GETPOST($equipementFieldName, 'array') : array();
                                    $multiSelectEquipement = '';

                                    if ($product->array_options['options_synergiestech_to_serialize'] == 1) {
                                        $productToSerialize = 1;

                                        // find all equipments for a product and warehouse
                                        $resql = $equipementStatic->findAllByFkProductAndFkEntrepot($product->id, $warehouse_id);
                                        if ($resql) {
                                            while ($obj = $db->fetch_object($resql)) {
                                                $equipementList[$obj->rowid] = $obj->ref;
                                            }
                                        ...
                                        $multiSelectEquipement = Form::multiselectarray($equipementFieldName, $equipementList, $idEquipementList, 0, 0, '', 0, 200);
                                    }

                                    print '<td>';
                                    print '<input type="hidden" name="' . $productToSerializeFieldName . '" value="' . $productToSerialize . '" />';
                                    print $multiSelectEquipement;
                                    print '</td>';
                                }
...
/* *************************************************************************** */
{
	...
	print '<div class="underbanner clearboth"></div>';
		...
		print '<form name="form_expedition_lines" action="' . $_SERVER['PHP_SELF'] . '?id=' . $object->id . '" method="post">';
        print '<input type="hidden" name="token" value="' . $_SESSION['newtoken'] . '" />';
		if ($action == 'edit_equipement' && $user->rights->expedition->creer && $user->rights->equipement->creer) {
            print '<input type="hidden" name="action" value="update_equipement" />';
        } else {
            print '<input type="hidden" name="action" value="edit_equipement" />';
        }
		...
		if ($origin && $origin_id > 0)
		{
			...
			if (! empty($conf->stock->enabled))
			{
				...
				if ($lines[$i]->entrepot_id > 0)
				{
                    ...
                    $entrepotIdList[] = $lines[$i]->entrepot_id;
                    $entrepotList[$entrepot->id] = array('entrepot' => $entrepot, 'expedition_line_id' => $lines[$i]->id);
				...
				else if (count($lines[$i]->details_entrepot) > 1)
				{
					...
					foreach ($lines[$i]->details_entrepot as $detail_entrepot)
					{
						if ($detail_entrepot->entrepot_id > 0)
						{
                            ...
                            $entrepotIdList[] = $detail_entrepot->entrepot_id;
                            $entrepotList[$detail_entrepot->entrepot_id] =  array('entrepot' => $entrepot, 'expedition_line_id' => $detail_entrepot->line_id);
            ...
            if (!empty($conf->equipement->enabled) && $lines[$i]->fk_product>0 && $product_static->id>0) {
                ...
                if ($product_static->array_options['options_synergiestech_to_serialize'] == 1) {
                    ...
                    foreach ($entrepotIdList as $entrepotId) {
                        $expeditionLineId     = $entrepotList[$entrepotId]['expedition_line_id'];
                        $expeditionLineSuffix = $expeditionLineId . '_' . $entrepotId;
                        $equipementFieldName  = 'equipementl_' . $expeditionLineSuffix;
                        $equipementIdList     = array();
                        $equipementList       = array();

                        $equipementWarehouseList = array();
                        if ($action == 'edit_equipement' && $user->rights->expedition->creer && $user->rights->equipement->creer) {
                            // find all equipments for a product and warehouse
                            $resql = $equipementStatic->findAllByFkProductAndFkEntrepot($product_static->id, $entrepotId);
                            if ($resql) {
                                while ($obj = $db->fetch_object($resql)) {
                                    $equipementWarehouseList[$obj->rowid] = $obj->ref;
                                }
                            }
                        }

                        // find all equipments in this shipment line
                        ...
                        if ($resql) {
                            while ($obj = $db->fetch_object($resql)) {
                                $equipementIdList[] = $obj->rowid;
                                $equipementList[] = $obj->ref;
                            }

                            // set equipment list to ship
                            if (GETPOST($equipementFieldName, 'array')) {
                                $equipementIdList = GETPOST($equipementFieldName, 'array');
                            }
                            $qtyEquipementToShip += count($equipementIdList);

                            if ($action == 'edit_equipement' && $user->rights->expedition->creer && $user->rights->equipement->creer) {
                                // expedition lines to modify
                                print '<input type="hidden" name="expedition_line_suffix_list[]" value="' . $expeditionLineSuffix . '" />';
                                $multiSelectEquipementList[$entrepotId] = Form::multiselectarray($equipementFieldName, $equipementWarehouseList, $equipementIdList, 0, 0, '', 0, 200);
                            } else {
                                $multiSelectEquipementList[$entrepotId] = implode("<br />", $equipementList);
                            }
                        }
                    }

                    if ($object->statut < Expedition::STATUS_VALIDATED) {
                        if ($qtyEquipementToShip != $lines[$i]->qty_shipped) {
                            $equipementLine .= img_warning($langs->trans('EquipementErrorQtyToShip'));
                            $equipementLine .= ' <span style="color: red;">' . $langs->trans('EquipementErrorQtyToShip') . '</span>';
                        }
                    }
                }

                print '<td>';
                foreach ($multiSelectEquipementList as $entrepotId => $multiSelectEquipement) {
                    print $entrepotList[$entrepotId]['entrepot']->getNomUrl(1) . '<br />' . $multiSelectEquipement . '<br /><br />';
                }
                print $equipementLine;
                print '</td>';
            }
        ...
        {
            if ($user->rights->expedition->creer && $user->rights->equipement->creer) {
