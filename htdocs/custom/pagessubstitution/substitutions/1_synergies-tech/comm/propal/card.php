...
if (empty($reshook))
{
	...
	{
		$result = $object->await($user);
		if ($result >= 0) {
			// Hierarchy
			$huser=new User($db);
            $huser->fetch($user->fk_user);
			if(!empty($huser->email)) {	//Send email
				require_once DOL_DOCUMENT_ROOT .'/core/class/CMailFile.class.php';

				$subject = $langs->trans("NotifyAwaiting",$object->ref);
				$sendto = $huser->email;
				$replyto = (! empty($conf->global->MAIN_MAIL_EMAIL_FROM)?$conf->global->MAIN_MAIL_EMAIL_FROM:'');
				$message = $langs->trans("MessageAwaiting");
				$message .= '<br/><br/>';
				$message .= '<a href="'.$dolibarr_main_url_root.'/comm/propal/card.php?id='.$object->id.'">';
                $message .= $langs->trans("ButtonEmailAwaiting",$object->ref);
                $message .='</a>';
    ...
    // -- Open DSI -- Add form confirm for setting status -- Begin
	// Close proposal
...
if ($action == 'create')
{
    ...
    // -- Open DSI -- Add form confirm for setting status -- End
	...
	//}
    // -- Open DSI -- Possibility to modify confirm form -- End
	...
	if ($action != 'presend') {
		...
		if (empty($reshook))
		{
            ...
            if ($action != 'editline')
			{
				...
				{
					if(empty($user->rights->propale->await) && !empty($conf->global->MAIN_USE_AWAITING_PROPAL)) {
						if ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->propal->creer))
						|| (! empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->propal->propal_advance->validate)))
						{
							print '<div class="inline-block divButAction"><a class="butAction" href="' . $_SERVER["PHP_SELF"] . '?id=' . $object->id . '&amp;action=await">' . $langs->trans('Await') . '</a></div>';
						}
						else
							print '<div class="inline-block divButAction"><a class="butActionRefused" href="#">' . $langs->trans('Await') . '</a></div>';
					} else {
						if ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->propal->creer))
						|| (! empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->propal->propal_advance->validate)))
						{
				...
				if (($object->statut == Propal::STATUS_VALIDATED && $user->rights->propal->creer)
					//|| ($object->statut == Propal::STATUS_AWAIT && ($user->id == $hierarchy_user->fk_user  || $user->admin == 1))) {
						|| ($object->statut == Propal::STATUS_AWAIT)) {
