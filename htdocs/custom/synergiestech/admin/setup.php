...
// Fiche inter validate protection settings

print '<tr class="liste_titre">';
print '<td>' . $langs->trans("SynergiesTechInterventionValidateProtection") . '</td>' . "\n";
print '<td align="center">&nbsp;</td>' . "\n";
print '<td align="right">' . $langs->trans("Value") . '</td>' . "\n";
print "</tr>\n";

// SYNERGIESTECH_FICHINTER_PROTECTVALIDATEFICHINTER
print '<tr ' . $bc[$var] . '>' . "\n";
print '<td>' . $langs->trans("SynergiesTechProtectValidateFichInter") . '</td>' . "\n";
print '<td align="center">&nbsp;' . $langs->trans("SynergiesTechProtectValidateFichInterDesc") .'</td>' . "\n";
print '<td align="right">' . "\n";
if (empty($conf->global->SYNERGIESTECH_FICHINTER_PROTECTVALIDATEFICHINTER)) {
    print '<a href="' . $_SERVER['PHP_SELF'] . '?action=set_SYNERGIESTECH_FICHINTER_PROTECTVALIDATEFICHINTER">' . img_picto($langs->trans("Disabled"), 'switch_off') . '</a>';
} else {
    print '<a href="' . $_SERVER['PHP_SELF'] . '?action=del_SYNERGIESTECH_FICHINTER_PROTECTVALIDATEFICHINTER">' . img_picto($langs->trans("Enabled"), 'switch_on') . '</a>';
}
print '</td></tr>' . "\n";

// SYNERGIESTECH_FICHINTER_INTERVENTIONPRODUCT
$var = !$var;
print '<tr ' . $bc[$var] . '>' . "\n";
print '<td>' . $langs->trans("SynergiesTechProtectValidateServiceList") . '</td>' . "\n";
print '<td align="center">&nbsp;</td>' . "\n";
print '<td align="right">' . "\n";
$listOfProduct = $form->select_produits_list("", 'SYNERGIESTECH_FICHINTER_INTERVENTIONPRODUCT', 1, null, 0, 0, 1, 2, 1, 0, 1, 0, "", 1, '');
$listOfFormatedProduct = array();
foreach ($listOfProduct as $productArray) {
...
if (!empty($conf->use_javascript_ajax)) {
    ...
    if (empty($conf->global->SYNERGIESTECH_SEND_MAIL_TO_THIRDPARTY_BENEFACTOR)) {
