...
/**
 *	Class to generate PDF proposal Azur with company relationships
 */
class pdf_ouvrage_expedition_cr_st extends ModelePdfExpedition
{
    use libPDFST;
	var $db;
	var $name;
	...
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "ouvrage_expedition_cr_st";
	...
	function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0)
	{
		...
		$realpatharray=array();
		if (! empty($conf->global->MAIN_GENERATE_EXPEDITION_WITH_PICTURE))
		{
		...
		if ($conf->expedition->dir_output)
		{
			...
			if (file_exists($dir))
			{
                ...
                $pdf->backgroundImagePath = dol_buildpath('/synergiestechcontrat/img/fond_3.jpg');
...
//				}
                ...
                $heightforsignature = empty($conf->global->EXPEDITION_DISABLE_SIGNATURE)?(pdfGetHeightForHtmlContent($pdf, $outputlangs->transnoentities("ProposalCustomerSignature"))+10):0;
				...
				for ($i = 0; $i < $nblignes; $i++)
				{
                    ...
                    // -- Open DSI -- Add serial number if exists -- Begin
                    $strTemp = $this->_getEquipementSerialDesc($object, $outputlangs, $i);
                    if ($strTemp) {
                        $object->lines[$i]->desc = dol_concatdesc($object->lines[$i]->desc, $strTemp);
                    }
                    // -- Open DSI -- Add serial number if exists -- End
...
//				if (empty($conf->global->EXPEDITION_DISABLE_SIGNATURE))
//				{
...
//				}
				...
				//If expedition merge product PDF is active
				if (!empty($conf->global->PRODUIT_PDF_MERGE_EXPEDITION))
				{
					require_once DOL_DOCUMENT_ROOT.'/product/class/expeditionmergepdfproduct.class.php';

					$already_merged = array ();
					foreach ( $object->lines as $line ) {
						if (! empty($line->fk_product) && ! (in_array($line->fk_product, $already_merged))) {
							// Find the desire PDF
							$filetomerge = new Expeditionmergepdfproduct($this->db);
...
//                    var_dump($file);die('');
	...
	function _tableau_info(&$pdf, $object, $posy, $outputlangs)
	{
		...
		if (empty($conf->global->EXPEDITION_PDF_HIDE_PAYMENTTERMCOND) && ($object->cond_reglement_code || $object->cond_reglement))
		{
			$pdf->SetFont('','B', $default_font_size - 2);
			$pdf->SetXY($this->marge_gauche, $posy);
		...
		if (empty($conf->global->EXPEDITION_PDF_HIDE_PAYMENTTERMCOND))
		{
		...
		*/
		...
		if ($deja_regle > 0)
		{
			...
			if ($object->close_code == 'discount_vat')
			{
				...
				$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
				$pdf->MultiCell($largcol2, $tab2_hl, price($object->total_ttc - $deja_regle, 0, $outputlangs, 1, -1, -1, 'auto'), $useborder, 'R', 1);
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0, $currency='')
	{
		...
		if (! empty($conf->global->MAIN_GENERATE_EXPEDITION_WITH_PICTURE))
		{
			...
			if (empty($hidetop))
			{
				//$pdf->SetXY($this->posxpicture-1, $tab_top - 8.5);
...
//		}
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
        ...
        //OSCSS BEGIN
//        $bgImage = dol_buildpath('/synergiestechcontrat/img/fond_3.jpg');
//        $this->setBackgroundImage($pdf, $bgImage);
        //OSCSS END

		//  Show Draft Watermark
		if($object->statut==0 && (! empty($conf->global->EXPEDITION_DRAFT_WATERMARK)) )
		{
            pdf_watermark($pdf,$outputlangs,$this->page_hauteur,$this->page_largeur,'mm',$conf->global->EXPEDITION_DRAFT_WATERMARK);
		}
...
//		$pdf->SetXY($posx,$posy);
//		$pdf->SetTextColor(0,0,60);
//		$pdf->MultiCell(100, 3, $outputlangs->transnoentities("DateEndexpedition")." : " . dol_print_date($object->fin_validite,"day",false,$outputlangs,true), '', 'R');
//
...
//		$posy = pdf_writeLinkedObjects($pdf, $object, $outputlangs, $posx, $posy, 100, 3, 'R', $default_font_size);
		...
		if ($showaddress)
		{
            ...
            $title = $outputlangs->transnoentities("expeditionMAJ");
            ...
            $pdf->SetTextColor(0, 0, 60);
            $this->addBullet($pdf);
            $pdf->MultiCell($w, 4, $outputlangs->transnoentities("Numberexpedition")." : ".$outputlangs->convToOutputCharset($object->ref), '', 'L');
...
//            }
//            if ($object->type == 1) {
//                $objectreplaced = new Propal($this->db);
...
//            }
            ...
            $pdf->SetTextColor(0, 0, 60);
            $this->addBullet($pdf);
            $pdf->MultiCell($w, 3, $outputlangs->transnoentities("DateLivraison")." : ".dol_print_date($object->date_delivery, "day", false, $outputlangs), '', 'L');
...
//            }
            ...
            if ($object->ref_customer) {
                ...
                $pdf->SetTextColor(0, 0, 60);
                $this->addBullet($pdf);
                $pdf->MultiCell($w, 3, $outputlangs->transnoentities("ObjetFAC")." : ".$outputlangs->convToOutputCharset($object->ref_customer), '', 'L');
            }
...
//			$pdf->MultiCell($widthrecbox, 4, $carac_client, 0, 'L');
	...
	function _pagefoot(&$pdf,$object,$outputlangs,$hidefreetext=0)
	{
		...
		return $this->pdf_pagefoot($pdf,$outputlangs,'PROPOSAL_FREE_TEXT',$this->emetteur,$this->marge_basse,$this->marge_gauche,$this->page_hauteur,$object,$showdetails,$hidefreetext);
	}
    ...
    /**
     * Get equipment serial description
     *
     * @param   Object          $object         Object to generate
     * @param   Translate       $outputlangs    Lang output object
    ...
    {
        global $db;
        $idprod = (!empty($object->lines[$i]->fk_product) ? $object->lines[$i]->fk_product : 0);
        $retStr = '';
        ...
        if ($idprod > 0) {
            ...
            $sql .= " INNER JOIN " . MAIN_DB_PREFIX . "equipementevt AS ee ON ee.fk_equipement = e.rowid";
            $sql .= " AND ee.fk_expedition = " . $object->id;
            $sql .= " AND ee.fk_expeditiondet = " . $object->lines[$i]->id;
            $sql .= " AND e.fk_product = " . $idprod;
            ...
            if ($result) {
                ...
                if ($num>0) {
                    $retStr = '<br />' . $outputlangs->trans("EquipRef") . ' : ';
                    for ($i=0; $i<$num; $i++) {
