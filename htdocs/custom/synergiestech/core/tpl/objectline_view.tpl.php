...
?></td>
	...
	<td class="linecoluht nowrap right"><?php $coldisplay++; ?><?php if ($user->rights->synergiestech->product_line_price->lire) print price($sign * $line->subprice); ?></td>

<?php if (!empty($conf->multicurrency->enabled) && $this->multicurrency_code != $conf->currency) { ?>
	<td class="linecoluht_currency nowrap right"><?php $coldisplay++; ?><?phpif ($user->rights->synergiestech->product_line_price->lire) print price($sign * $line->multicurrency_subprice); ?></td>
<?php }

if ($inputalsopricewithtax) { ?>
	<td class="linecoluttc nowrap right"><?php $coldisplay++; ?><?php if ($user->rights->synergiestech->product_line_price->lire) print (isset($line->pu_ttc) ? price($sign * $line->pu_ttc) : price($sign * $line->subprice)); ?></td>
<?php } ?>
...
if (!empty($line->remise_percent) && $line->special_code != 3 && $user->rights->synergiestech->product_line_discount->lire) {
...
<?php } else {
	...
	if (empty($conf->global->MAIN_OPTIMIZEFORTEXTBROWSER))
	{
		...
		if ($user->rights->synergiestech->product_line_price->lire) print '<span class="classfortooltip" title="'.dol_escape_htmltag($tooltiponprice).'">';
	}
	if ($user->rights->synergiestech->product_line_price->lire) print price($sign * $line->total_ht);
	if (empty($conf->global->MAIN_OPTIMIZEFORTEXTBROWSER))
	{
		print '</span>';
	}
	...
	if (!empty($conf->multicurrency->enabled) && $this->multicurrency_code != $conf->currency) {
		...
		if ($user->rights->synergiestech->product_line_price->lire) print price($sign * $line->multicurrency_total_ht);
		print '</td>';
		$coldisplay++;
	}
}
if ($outputalsopricetotalwithtax) {
	print '<td class="linecolht nowrap right">';
	if ($user->rights->synergiestech->product_line_price->lire) print price($sign * $line->total_ttc);
	print '</td>';
	$coldisplay++;
}
