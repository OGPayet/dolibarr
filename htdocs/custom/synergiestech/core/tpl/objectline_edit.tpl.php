...
<tr <?php echo $bc[$var]; ?>>
	...
	<?php
	$coldisplay++;
  if ($user->rights->synergiestech->product_line_price->creer) {
      ...
      echo '<td align="right"><input type="hidden" name="tva_tx" id="tva_tx" value="' . price($line->tva_tx) . '"></td>';
  }
  ...
  if ($user->rights->synergiestech->product_line_price->creer) {
      ...
      echo '<td align="right"><input type="hidden" name="price_ht" id="price_ht" value="' . (isset($line->pu_ht)?price($line->pu_ht,0,'',0):price($line->subprice,0,'',0)) . '"></td>';
  }
	...
	if (!empty($conf->multicurrency->enabled)) {
      if ($user->rights->synergiestech->product_line_price->creer) {
          ...
          echo '<td align="right"><input type="hidden" name="multicurrency_subprice" id="multicurrency_subprice" value="' . price($line->multicurrency_subprice) . '"></td>';
      }
  }
	...
	if ($inputalsopricewithtax) {
      ...
      if ($user->rights->synergiestech->product_line_price->creer) {
          ...
          echo '<td align="right"><input type="hidden" name="price_ttc" id="price_ttc" value="' . (isset($line->pu_ttc) ? price($line->pu_ttc, 0, '', 0) : '') . '"></td>';
      }
  }
	?>
	...
	<?php if (($line->info_bits & 2) != 2) {
      if ($user->rights->synergiestech->product_line_discount->creer) {
          print '<input size="1" type="text" class="flat right" name="remise_percent" id="remise_percent" value="' . $line->remise_percent . '"';
          if ($this->situation_counter > 1) print ' readonly';
          print '>%';
      } else {
          echo '<input type="hidden" name="remise_percent" id="remise_percent" value="' . $line->remise_percent . '">';
      }
  } else { ?>
		&nbsp;
	<?php } ?>
