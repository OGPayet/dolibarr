		...
		if (empty($senderissupplier))
		{
          ...
          if ($conf->global->SYNERGIESTECH_PRODUCT_CATEGORY_FOR_CONTRACT_FORMULE > 0) {
              ...
              if ($object->element == 'requestmanager') {
                  ...
                  $ajaxoptions['dynamic_data'] = array('tags_categories');
              }

              $ajaxoptions['update'] = array('remise_percent' => 'discount');	// html id tags that will be edited with which ajax json response key
              ...
              print '&nbsp;<span style="color: red;">' . $langs->trans('SynergiesTechErrorProductCategoryForFomuleContratNotDefined') . '</span>';
          }
      } else {
          if ($conf->global->ENTREPOT_EXTRA_STATUS) {
              // hide products in closed warehouse, but show products for internal transfer
