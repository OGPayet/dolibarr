...
SCRIPT;
	...
	public function addMoreToEmail($parameters = array(), &$object, &$action = '', $hookmanager)
	{
		...
		if (in_array("interventionmail", $contexts)) {
			...
			if (
				$conf->global->SYNERGIESTECH_SEND_MAIL_TO_THIRDPARTY_PRINCIPAL &&
				$object->array_options['options_companyrelationships_availability_principal'] == '1'
				&& !empty($object->socid)
			) {
				$thirdparty = new Societe($this->db);
				if ($thirdparty->fetch($object->socid) > 0 && !empty($thirdparty->email)) {
					array_push($emailList, $thirdparty->email);
				}
				$errors = array_merge($errors, $thirdparty->errors);
			}

			// Add benefactor to the emailList
			$benefactorId = $object->array_options['options_companyrelationships_fk_soc_benefactor'];
			if (
				$conf->global->SYNERGIESTECH_SEND_MAIL_TO_THIRDPARTY_BENEFACTOR
				&& $object->array_options['options_companyrelationships_availability_benefactor'] == '1'
				&& !empty($benefactorId)
			) {
				$benefactor = new Societe($this->db);
				if ($benefactor->fetch($benefactorId) > 0 && !empty($benefactor->email)) {
					array_push($emailList, $benefactor->email);
				}
				$errors = array_merge($errors, $thirdparty->errors);
			}

			// Add Watcher to the emailList
			$watcherId = $object->array_options['options_companyrelationships_fk_soc_watcher'];
			if (
				$object->array_options['options_companyrelationships_availability_watcher'] == '1'
				&& !empty($watcherId)
				&& $conf->global->SYNERGIESTECH_SEND_MAIL_TO_THIRDPARTY_OBSERVER
			) {
				$watcher = new Societe($this->db);
				if ($watcher->fetch($watcherId) > 0 && !empty($watcher->email)) {
					array_push($emailList, $watcher->email);
				}
				$errors = array_merge($errors, $thirdparty->errors);
			}
