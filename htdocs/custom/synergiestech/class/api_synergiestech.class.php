 ...
 */
class UsersSynergiesTech extends DolibarrApi
{
	/**
	 *
	...
	function getUsers($sortfield = "t.rowid", $sortorder = 'ASC', $limit = 0, $page = 0, $sqlfilters = '') {
		...
		$sql.= ")";
		$sql.= " AND ( t.statut=1 ) AND (t.fk_soc IS NULL)";
		$sql.= " GROUP BY rowid";
	    ...
	    if ($result)
	    {
	        ...
	        while ($i < $min)
	        {
	            ...
	            if($user_static->fetch($obj->rowid)) {
					...
					$finalItem = $this->_cleanObjectDatas($product_static);

						$result=array();
						$temp=array("id","firstname","lastname");
						foreach($temp as $tempkey){$result[$tempkey]=$finalItem->$tempkey;}
						$finalItem=$result;
