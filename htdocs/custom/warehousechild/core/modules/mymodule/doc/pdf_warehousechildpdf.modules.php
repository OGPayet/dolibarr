...
/**
 *	\file       htdocs/core/modules/warehousechild/doc/pdf_beluga.modules.php
 *	\ingroup    warehousechild
 *	\brief      Fichier de la classe permettant de generer les warehousechilds au modele beluga
...
/**
 *	Classe permettant de generer les warehousechilds au modele Baleine
 */

class pdf_warehousechildpdf extends Modelewarehousechild
{
	...
	/**
	 *	Fonction generant le warehousechild sur le disque
	 *
	 *	@param	warehousechild		$object   		Object warehousechild a generer
	...
	function write_file($object,$outputlangs)
	{
		...
		$formwarehousechild=new Formwarehousechild($this->db);
		...
		$outputlangs->load("warehousechild");

		if ($conf->warehousechild->dir_output)
		{
			//$nblignes = count($object->lines);  // This is set later with array of tasks

			$objectref = dol_sanitizeFileName($object->ref);
			$dir = $conf->warehousechild->dir_output;
			...
			if (file_exists($dir))
			{
				...
				$tasksarray = $task->getTasksArray('warehousechild',0,0,$object->id);
				...
				$pdf->SetSubject($outputlangs->transnoentities("warehousechild"));
				$pdf->SetCreator("Dolibarr ".DOL_VERSION);
				$pdf->SetAuthor($outputlangs->convToOutputCharset($user->getFullName($outputlangs)));
				$pdf->SetKeyWords($outputlangs->convToOutputCharset($object->ref)." ".$outputlangs->transnoentities("warehousechild"));
                ...
                $listofreferent=array(
                    'propal'=>array(
			'name'=>"Proposals",
			'title'=>"ListProposalsAssociatedwarehousechild",
			'class'=>'Propal',
			'table'=>'propal',
                        'datefieldname'=>'datep',
			...
			'title'=>"ListOrdersAssociatedwarehousechild",
			'class'=>'Commande',
			'table'=>'commande',
			...
			'title'=>"ListInvoicesAssociatedwarehousechild",
			'class'=>'Facture',
			'margin'=>'add',
			'table'=>'facture',
			'datefieldname'=>'datef',
			...
			'title'=>"ListPredefinedInvoicesAssociatedwarehousechild",
			'class'=>'FactureRec',
			'table'=>'facture_rec',
			'datefieldname'=>'datec',
			'test'=>$conf->facture->enabled && $user->rights->facture->lire,
			...
			'title'=>"ListSupplierOrdersAssociatedwarehousechild",
			...
			'name'=>"BillsSuppliers",
			'title'=>"ListSupplierInvoicesAssociatedwarehousechild",
			...
			'test'=>$conf->fournisseur->enabled && $user->rights->fournisseur->facture->lire,
                    ...
                    'contract'=>array(
			'name'=>"Contracts",
			'title'=>"ListContractAssociatedwarehousechild",
			'class'=>'Contrat',
			'table'=>'contrat',
			...
			'test'=>$conf->contrat->enabled && $user->rights->contrat->lire,
                    ...
                    'intervention'=>array(
			'name'=>"Interventions",
			'title'=>"ListFichinterAssociatedwarehousechild",
			'class'=>'Fichinter',
			'table'=>'fichinter',
			...
			'title'=>"ListExpenseReportsAssociatedwarehousechild",
			'class'=>'Deplacement',
			'table'=>'deplacement',
			'datefieldname'=>'dated',
			'margin'=>'minus',
			...
			'test'=>$conf->expensereport->enabled && $user->rights->expensereport->lire,
                    ...
                    'agenda'=>array(
			'name'=>"Agenda",
			'title'=>"ListActionsAssociatedwarehousechild",
			'class'=>'ActionComm',
			'table'=>'actioncomm',
			'datefieldname'=>'datep',
			'disableamount'=>1,
			...
			$langs->load($langstoload);
                    ...
                    if ($qualified)
                    {
                        ...
                        if ($num >= 0)
                        {
                            ...
                            $selectList = $formwarehousechild->select_element($tablename, $warehousechild->thirdparty->id);
                            $nexY = $pdf->GetY() + 1;
                            $curY = $nexY;
                            $pdf->SetXY($this->posxref, $curY);
	 ...
	 *
	 *  @param	PDF			$pdf     		Object PDF
	 *  @param  warehousechild		$object     	Object to show
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell(100, 4, $outputlangs->transnoentities("warehousechild")." ".$outputlangs->convToOutputCharset($object->ref), '', 'R');
	...
	function _pagefoot(&$pdf,$object,$outputlangs,$hidefreetext=0)
	{
		...
		return pdf_pagefoot($pdf,$outputlangs,'WAREHOUSECHILD_FREE_TEXT',$this->emetteur,$this->marge_basse,$this->marge_gauche,$this->page_hauteur,$object,$showdetails,$hidefreetext);
	}
}
