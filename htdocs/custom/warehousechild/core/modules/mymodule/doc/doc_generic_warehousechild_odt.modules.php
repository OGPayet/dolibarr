...
/**
 *	\file       htdocs/core/modules/warehousechild/doc/doc_generic_warehousechild_odt.modules.php
 *	\ingroup    warehousechild
 *	\brief      File of class to build ODT documents for third parties
 */
dol_include_once('/warehousechild/core/modules/warehousechild/modules_warehousechild.php');
 ...
 */
class doc_generic_warehousechild_odt extends Modelewarehousechild
{
	...
	function __construct($db)
	{
		...
		$langs->load("warehousechild");

		$this->db = $db;
		$this->name = "ODT templates";
		$this->description = $langs->trans("DocumentModelOdt");
		$this->scandir = 'WAREHOUSECHILD_ADDON_PDF_ODT_PATH';	// Name of constant that is used to save list of directories to scan
	...
	function get_substitutionarray_tasks($task,$outputlangs)
	{
		...
		return array(
		'task_ref'=>$task->ref,
		'task_fk_warehousechild'=>$task->fk_warehousechild,
		'task_warehousechildref'=>$task->warehousechildref,
		'task_warehousechildlabel'=>$task->warehousechildlabel,
	 ...
	 *  @return	array								Return a substitution array
	 */
	function get_substitutionarray_warehousechild_contacts($contact,$outputlangs)
	{
		global $conf;
	 ...
	 *  @return	array								Return a substitution array
	 */
	function get_substitutionarray_warehousechild_file($file,$outputlangs)
	{
		global $conf;
	 ...
	 *  @return	array								Return a substitution array
	 */
	function get_substitutionarray_warehousechild_reference($refdetail,$outputlangs)
	{
		global $conf;
	...
	function info($langs)
	{
		...
		$texte.= '<input type="hidden" name="param1" value="WAREHOUSECHILD_ADDON_PDF_ODT_PATH">';
		$texte.= '<table class="nobordernopadding" width="100%">';

		// List of directories area
		$texte.= '<tr><td>';
		$texttitle=$langs->trans("ListOfDirectories");
		$listofdir=explode(',',preg_replace('/[\r\n]+/',',',trim($conf->global->WAREHOUSECHILD_ADDON_PDF_ODT_PATH)));
		$listoffiles=array();
		...
		$texte.=$conf->global->WAREHOUSECHILD_ADDON_PDF_ODT_PATH;
		$texte.= '</textarea>';
		...
		if (! empty($conf->global->WAREHOUSECHILD_ADDON_PDF_ODT_PATH))
		{
	...
	function write_file($object,$outputlangs,$srctemplatepath)
	{
		...
		$outputlangs->load("warehousechilds");
		...
		if ($conf->projet->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				// If WAREHOUSECHILDLEADER contact defined on warehousechild, we use it
				$usecontact=false;
				$arrayidcontact=$object->getIdContact('external','WAREHOUSECHILDLEADER');
				...
				if (! empty($usecontact))
				{
				// if we have a WAREHOUSECHILDLEADER contact and we dont use it as recipient we store the contact object for later use
                ...
                // retrieve contact information for use in warehousechild as contact_xxx tags
			$array_warehousechild_contact = array();
			if ($usecontact)
				$array_warehousechild_contact=$this->get_substitutionarray_contact($contactobject,$outputlangs,'contact');

				$tmparray = array_merge($array_user,$array_soc,$array_thirdparty,$array_objet,$array_other,$array_warehousechild_contact);
				...
				try
				{
					...
					foreach ($tasksarray as $task)
					{
						...
						// Replace tags of warehousechild files
				...
				try
				{
					$listlines = $odfHandler->setSegment('warehousechildfiles');
					...
					foreach ($filearray as $filedetail)
					{
						...
						$tmparray=$this->get_substitutionarray_warehousechild_file($filedetail,$outputlangs);
				...
				if ((is_array($contact_arrray) && count($contact_arrray) > 0))
				{
					try
					{
						$listlines = $odfHandler->setSegment('warehousechildcontacts');
						...
						foreach ($contact_arrray as $contact)
						{
							...
							$tmparray=$this->get_substitutionarray_warehousechild_contacts($contact,$outputlangs);
							foreach($tmparray as $key => $val)
							{
								try
								{
				...
				try
				{
					$listlines = $odfHandler->setSegment('warehousechildrefs');
						...
						if ($qualified)
						{
							...
							if (count($elementarray)>0 && is_array($elementarray))
							{
								...
								for ($i = 0; $i < $num; $i++)
								{
									...
									$tmparray=$this->get_substitutionarray_warehousechild_reference($ref_array,$outputlangs);
