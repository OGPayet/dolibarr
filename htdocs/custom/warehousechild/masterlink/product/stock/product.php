        ...
        if (is_numeric(GETPOST("nbpiece")) && $id) {
            ...
            if ($object->hasbatch()) {
                $result = $object->correct_stock_batch(
                    $user, GETPOST("id_entrepot"), GETPOST("nbpiece"), GETPOST("mouvement"), GETPOST("label"), // label movement
                    $priceunit, $d_eatby, $d_sellby, $batchnumber, GETPOST('inventorycode'), $origin_element, $origin_id
                );  // We do not change value of stock for a correction
            } else {
                $result = $object->correct_stock(
                    $user, GETPOST("id_entrepot"), GETPOST("nbpiece"), GETPOST("mouvement"), GETPOST("label"), $priceunit, GETPOST('inventorycode'), $origin_element, $origin_id
                );  // We do not change value of stock for a correction
            }
...
if ($action == "transfert_stock" && !$cancel) {
    ...
    // Open-DSI Patch 2018.05.16 FJ - BEGIN.
    // Bypass ErrorSrcAndTargetWarehouseMustDiffers error:
    // So we can transfer from 1 compartment to another in the same warehouse.
    ...
    // Open-DSI Patch 2018.05.16 FJ - END.
    if (!empty($conf->productbatch->enabled)) {
        $object = new Product($db);
        $result = $object->fetch($id);
            ...
            if ($object->hasbatch()) {
                ...
                if (!$error) {
                    ...
                    $result1 = $object->correct_stock_batch(
                        $user, $srcwarehouseid, GETPOST("nbpiece", 'int'), 1, GETPOST("label", 'san_alpha'), $pricesrc, $eatby, $sellby, $batch, GETPOST('inventorycode')
                    );
                    if ($result1 < 0) $error++;
                }
                if (!$error) {
                    ...
                    $result2 = $object->correct_stock_batch(
                        $user, GETPOST("id_entrepot_destination", 'int'), GETPOST("nbpiece", 'int'), 0, GETPOST("label", 'san_alpha'), $pricedest, $eatby, $sellby, $batch, GETPOST('inventorycode')
                    );
                    if ($result2 < 0) $error++;
                }
            }
                ...
                if (!$error) {
                    ...
                    $result1 = $object->correct_stock(
                        $user, GETPOST("id_entrepot"), GETPOST("nbpiece"), 1, GETPOST("label"), $pricesrc, GETPOST('inventorycode')
                    );
                    if ($result1 < 0) $error++;
                }
                if (!$error) {
                    ...
                    $result2 = $object->correct_stock(
                        $user, GETPOST("id_entrepot_destination"), GETPOST("nbpiece"), 0, GETPOST("label"), $pricedest, GETPOST('inventorycode')
                    );
                    if ($result2 < 0) $error++;
                }
            }
...
if ($id > 0 || $ref) {
    ...
    if ($result > 0) {
        ...
        // Desired stock
        print '<tr><td>'.$form->editfieldkey($form->textwithpicto($langs->trans("DesiredStock"), $langs->trans("DesiredStockDesc"), 1), 'desiredstock', $object->desiredstock, $object,
    ...
    if ($action == "correction") {
        include dol_buildpath('/warehousechild/tpl/stockcorrection.tpl.php');
        print '<br><br>';
    }

    // Transfer of units
    if ($action == "transfert") {
        include dol_buildpath('/warehousechild/tpl/stocktransfer.tpl.php');
        print '<br><br>';
    }
} else {
    dol_print_error();
