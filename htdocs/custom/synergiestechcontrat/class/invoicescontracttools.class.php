...
class InvoicesContractTools
{
	...
	public function getInvoiceAmount(&$contract, $watching_period, $billing_period, $test_mode = 0, $disable_revaluation = 0)
	{
		...
		switch (intval($frequency_of_billing)) {
				// Monthly
			case 1:
				$number_billing_period_in_year = 12;
				break;
				// Quarterly
			case 2:
				$number_billing_period_in_year = 4;
				break;
				// Semi
			case 3:
				$number_billing_period_in_year = 2;
				break;
				// Annual
			case 4:
				$number_billing_period_in_year = 1;
				break;
			default:
				$this->errors[] = $langs->trans('STCErrorBadFieldValue', $langs->transnoentitiesnoconv('STCContractFrequencyBilling'), $frequency_of_billing);
				return null;
		...
		if ($revaluation_index == 2 || $revaluation_index == 3) {
			...
			if (
				isset($potential_revaluation_date) &&
				...
				$second_billing_period_lenght = $second_billing_period_begin->copy()->diffInDays($second_billing_period_end->copy()->addMinutes(1));
	...
	public function generateInvoiceForTheContract(&$contract, $watching_date, $payment_condition = 0, $payment_deadline_date = 0, $ref_customer = '', $use_customer_discounts = 0, $test_mode = 0, $disable_revaluation = 0)
	{
		...
		if ($contract->statut == $contract::STATUS_DRAFT) {
			$error++;
			$this->errors[] = $langs->trans('STCErrorContractInDraftMode', $contract->ref);
		}

		// Get watching period
		$watching_period = $this->getWatchingPeriod($contract, $watching_date);
		if ($error && !isset($watching_period)) {
			$error++;
		}
			...
			if (!isset($invoices)) {
				...
				foreach ($invoices as $invoice) {
					$this->errors[] = $langs->trans(
						'STCErrorInvoiceOnBillingPeriodInfo',
						$invoice->ref,
	...
	public function generateInvoicesForTheContractInPeriod(&$contract, $watching_period_begin, $watching_period_end, $payment_condition = 0, $payment_deadline_date = 0, $ref_customer = '', $use_customer_discounts = 0, $test_mode = 0, $disable_revaluation = 0)
	{
		...
		switch (intval($frequency_of_billing)) {
			...
			case 1:
				...
				// Quarterly
			case 2:
				$number_billing_period_in_year = 3;
				break;
				// Semi
			case 3:
				$number_billing_period_in_year = 6;
				break;
				// Annual
			case 4:
				$number_billing_period_in_year = 12;
	...
	public function getRevaluationDate(&$contract)
	{
		...
		switch (intval($revaluation_date)) {
				// Anniversary date
			case 1:
				return Carbon::create(0, $effective_date->month, $effective_date->day, 0, 0, 0);
				break;
				// 1st of January
			case 2:
				return Carbon::create(0, 1, 1, 0, 0, 0);
				// 1st of April
			case 3:
				return Carbon::create(0, 4, 1, 0, 0, 0);
				// 1st of July
			case 4:
				return Carbon::create(0, 7, 1, 0, 0, 0);
				// 1st of October
			case 5:
				return Carbon::create(0, 10, 1, 0, 0, 0);
			default:
	...
	public function getRevaluationIndexInfo(&$contract, $potential_revaluation_date)
	{
		...
		if (
			!is_numeric($month_for_new_revaluation_index) ||
