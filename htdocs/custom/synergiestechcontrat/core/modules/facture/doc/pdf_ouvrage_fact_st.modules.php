 ...
 */
class pdf_ouvrage_fact_st extends ModelePDFFactures
{
    use libPDFST;
    var $db;
    var $name;
    ...
    var $main_color = array(68, 125, 189);
...
//            }
        ...
        if ($conf->facture->dir_output) {
            ...
            if (file_exists($dir)) {
                ...
                $pdf->backgroundImagePath = dol_buildpath('/synergiestechcontrat/img/fond.jpg');
    ...
    function _tableau_versements(&$pdf, $object, $posy, $outputlangs)
    {
        ...
        if ($resql) {
            ...
            while ($i < $num) {
                ...
                $pdf->MultiCell(20, 3, price($obj->amount_ttc, 0, $outputlangs, 1, -1, -1, 'auto'), 0, 'L', 0);
        ...
        if ($resql) {
            ...
            while ($i < $num) {
                ...
                $pdf->SetXY($tab3_posx + 21, $tab3_top + $y);
                $pdf->MultiCell(20, 3, price($sign * $row->amount, 0, $outputlangs, 1, -1, -1, 'auto'), 0, 'L', 0);
                $pdf->SetXY($tab3_posx + 40, $tab3_top + $y);
    ...
    function _tableau_info(&$pdf, $object, $posy, $outputlangs)
    {
        ...
        if ($object->type != 2 && ($object->cond_reglement_code || $object->cond_reglement)) {
            ...
            $lib_condition_paiement = $outputlangs->transnoentities("PaymentCondition".$object->cond_reglement_code) != ('PaymentCondition'.$object->cond_reglement_code) ? $outputlangs->transnoentities("PaymentCondition".$object->cond_reglement_code)
                    : $outputlangs->convToOutputCharset($object->cond_reglement_doc);
        ...
        if ($object->type != 2) {
            ...
            elseif (($object->mode_reglement_code == 'CHQ' && empty($conf->global->FACTURE_CHQ_NUMBER) && empty($object->fk_account) && empty($object->fk_bank)) || ($object->mode_reglement_code == 'VIR'
                && empty($conf->global->FACTURE_RIB_NUMBER) && empty($object->fk_account) && empty($object->fk_bank))) {
            ...
            if ($object->mode_reglement_code && $object->mode_reglement_code != 'CHQ' && $object->mode_reglement_code != 'VIR') {
                ...
                $pdf->SetFont('', '', $default_font_size - 2);
                $pdf->SetXY($posxval, $posy);
                $lib_mode_reg = $outputlangs->transnoentities("PaymentType".$object->mode_reglement_code) != ('PaymentType'.$object->mode_reglement_code) ? $outputlangs->transnoentities("PaymentType".$object->mode_reglement_code)
    ...
    function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
    {
        ...
        $pdf->MultiCell($col2x - $col1x, $tab2_hl, $outputlangs->transnoentities("TotalHTST"), 0, 'L', 1);
            ...
            if (!empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_IFNULL) && $tvaisnull) {
                ...
                foreach ($this->tva as $tvakey => $tvaval) {
                    if ($tvakey != 0) {    // On affiche pas taux 0
                        ...
                        $totalvat = $outputlangs->transnoentities("TotalVATST").' ';
                ...
                $pdf->MultiCell($col2x - $col1x, $tab2_hl, $outputlangs->transnoentities("TotalTTCST"), $useborder, 'L', 0);

                $total_ttc = ($conf->multicurrency->enabled && $object->multiccurency_tx != 1) ? $object->multicurrency_total_ttc : $object->total_ttc;
                $pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index +1);
                $pdf->MultiCell($largcol2, $tab2_hl, price($sign * $total_ttc, 0, $outputlangs, 1, -1, -1, 'auto'), $useborder, 'R', 0);
            }
        }
    ...
    function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop = 0, $hidebottom = 0, $currency = '')
    {
        ...
        if (empty($hidetop)) {
            ...
            if (!empty($conf->global->MAIN_PDF_TITLE_BACKGROUND_COLOR))
                    $pdf->Rect($this->marge_gauche, $tab_top, $this->page_largeur - $this->marge_droite - $this->marge_gauche, 5, 'F', null,
        ...
        if ($this->situationinvoice) {
            ...
            if (empty($hidetop)) {
                ...
                $pdf->SetXY($this->posxprogress, $tab_top - 8.5);
    ...
    function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
    {
        ...
        //OSCSS BEGIN
        //$bgImage = dol_buildpath('/synergiestechcontrat/img/fond.jpg');
...
//            $pdf->MultiCell($widthrecbox, $hautcadre, "", 0, 'R', 1);
        ...
        if ($showaddress) {
            ...
            $title = $outputlangs->transnoentities("InvoiceMAJ");
            ...
            $pdf->MultiCell($w-$bulletWidth, 4, $outputlangs->transnoentities("NumberInvoice")." : ".$outputlangs->convToOutputCharset($object->ref), $multiCellBorder, 'L');
            ...
            if ($object->type == 0 && $objectidnext) {
                ...
                $pdf->SetTextColor(0, 0, 60);
                $this->addBullet($pdf, $bulletSize);
                $pdf->MultiCell($w-$bulletWidth, 3, $outputlangs->transnoentities("ReplacementByInvoice").' : '.$outputlangs->convToOutputCharset($objectreplacing->ref), $multiCellBorder, 'L');
            }
            if ($object->type == 1) {
                ...
                $pdf->SetTextColor(0, 0, 60);
                $this->addBullet($pdf, $bulletSize);
                $pdf->MultiCell($w-$bulletWidth, 3, $outputlangs->transnoentities("ReplacementInvoice").' : '.$outputlangs->convToOutputCharset($objectreplaced->ref), $multiCellBorder, 'L');
            }
            if ($object->type == 2 && !empty($object->fk_facture_source)) {
                ...
                $pdf->SetTextColor(0, 0, 60);
                $this->addBullet($pdf, $bulletSize);
                $pdf->MultiCell($w-$bulletWidth, 3, $outputlangs->transnoentities("CorrectionInvoice").' : '.$outputlangs->convToOutputCharset($objectreplaced->ref), $multiCellBorder, 'L');
            }
            ...
            $pdf->SetTextColor(0, 0, 60);
            $this->addBullet($pdf, $bulletSize);
            $pdf->MultiCell($w-$bulletWidth, 3, $outputlangs->transnoentities("DateInvoice")." : ".dol_print_date($object->date, "day", false, $outputlangs), $multiCellBorder, 'L');
            ...
            if ($object->type != 2) {
                ...
                $pdf->SetTextColor(0, 0, 60);
                $this->addBullet($pdf, $bulletSize);
                $pdf->MultiCell($w-$bulletWidth, 3, $outputlangs->transnoentities("DateDue")." : ".dol_print_date($object->date_lim_reglement, "day", false, $outputlangs, true), $multiCellBorder, 'L');
            }
    ...
    function _pagefoot(&$pdf, $object, $outputlangs, $hidefreetext = 0)
    {
        ...
        return $this->pdf_pagefoot($pdf, $outputlangs, 'INVOICE_FREE_TEXT', $this->emetteur, $this->marge_basse, $this->marge_gauche, $this->page_hauteur, $object, $showdetails, $hidefreetext);
    }
