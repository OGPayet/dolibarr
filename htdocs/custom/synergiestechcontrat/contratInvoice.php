...
$contextpage='contractfacturerec';
...
$arrayfields=array(
    ...
    'facturerec'=>array('label'=>$langs->trans("FactureRec"), 'checked'=>1, 'position'=>1000),
);
// Extra fields
...
if (empty($reshook))
{
    ...
    include dol_buildpath('/synergiestechcontrat/class/massactions.inc.php');
}
...
$facturerectmp = new FactureRec($db);

llxHeader('', $langs->trans("FactureContracts"));
...
$sql.= ' max(ee.fk_target) as facturerec';
...
// ADD OSCSS
$sql.= " LEFT JOIN ".MAIN_DB_PREFIX."element_element as ee on c.rowid = ee.fk_source AND ee.sourcetype='contrat' AND ee.targettype='facturerec'";
$sql.= " WHERE c.fk_soc = s.rowid ";
$sql.= ' AND c.entity IN ('.getEntity('contract').')';
//$sql.= " AND ee.sourcetype='contrat' AND ee.targettype='facturerec' ";
...
if ($resql)
{
    ...
    $arrayofmassactions =  array(
        ...
        'factureanterieur'=>"Facturer dans le passé",
    );
    ...
    print_barre_liste($langs->trans("ListOfContractsToFacturerec"), $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $totalnboflines, 'object_invoice', 0, '', '', $limit);
    ...
    // facturerec
    if (! empty($arrayfields['facturerec']['checked']))
    {
    ...
    if (! empty($arrayfields['facturerec']['checked']))       print_liste_field_titre($arrayfields['facturerec']['label'],$_SERVER["PHP_SELF"],"facturerec","",$param,'align="center" class="nowrap"',$sortfield,$sortorder);
    ...
    while ($i < min($num,$limit))
    {
        ...
        if (! empty($arrayfields['facturerec']['checked'])){
            print '<td align="center">';
            if($obj->facturerec!=NULL){
                $facturerectmp->fetch($obj->facturerec);
                print $facturerectmp->getNomUrl(1);
            }else {
