...
$contextpage = 'invoicescontractlist';
...
$arrayfields = array(
    ...
    'invoices' => array('label' => $langs->trans('STCInvoicesDraft'), 'checked' => 1, 'position' => 1000),
    'invoices_total_ht' => array('label' => $langs->trans('STCTotalInvoicesHT'), 'checked' => 1, 'position' => 1000),
    'invoices_total_vat' => array('label' => $langs->trans('STCTotalInvoicesVAT'), 'checked' => 1, 'position' => 1000),
    'invoices_total_ttc' => array('label' => $langs->trans('STCTotalInvoicesTTC'), 'checked' => 1, 'position' => 1000),
);
// Extra fields
...
if (!GETPOST('confirmmassaction') && $massaction != 'generate_invoices' && $massaction != 'confirm_generate_invoices') {
    $massaction = '';
}
...
if (empty($reshook)) {
    ...
    include dol_buildpath('/synergiestechcontrat/core/actions_massactions.inc.php');
}
...
if ($resql) {
    ...
    if ($massaction == 'generate_invoices' && $user->rights->facture->creer) {
        ...
        if ($search_code_client != '') $formquestion[] = array('type' => 'hidden', 'name' => 'search_code_client', 'value' => $search_code_client);
    ...
    if ($search_code_client != '')          $param .= '&search_name=' . urlencode($search_code_client);
    ...
    //Email
    if (!empty($arrayfields['s.email']['checked'])) print '<td class="liste_titre"><input class="flat" type="text" name="search_email" value="' . $search_email . '"></td>';
    ...
    while ($i < min($num, $limit)) {
        ...
        if (!empty($arrayfields['invoices']['checked'])) {
            print '<td align="center">';
            if (isset($invoices_draft_list[$obj->rowid])) {
                $invoices_toprint = array();
                foreach ($invoices_draft_list[$obj->rowid]['invoices'] as $invoice) {
                    $facturetmp->id = $invoice['id'];
                    $facturetmp->ref = $invoice['ref'];
                    $facturetmp->type = $invoice['type'];
                    $facturetmp->ref_client = $invoice['ref_client'];
                    $facturetmp->note_private = $invoice['note_private'];
                    $facturetmp->note_public = $invoice['note_public'];
        ...
        if (!empty($arrayfields['invoices_total_ht']['checked'])) {
            ...
            if (isset($invoices_draft_list[$obj->rowid])) {
                print price($invoices_draft_list[$obj->rowid]['total_ht']);
                $totalarray['totalht'] += $invoices_draft_list[$obj->rowid]['total_ht'];
            }
            print "</td>\n";
            if (!$i) $totalarray['nbfield']++;
        ...
        if (!empty($arrayfields['invoices_total_vat']['checked'])) {
            print '<td align="right">';
            if (isset($invoices_draft_list[$obj->rowid])) {
                print price($invoices_draft_list[$obj->rowid]['total_vat']);
                $totalarray['totalvat'] += $invoices_draft_list[$obj->rowid]['total_vat'];
            }
            print "</td>\n";
            if (!$i) $totalarray['nbfield']++;
        ...
        if (!empty($arrayfields['invoices_total_ttc']['checked'])) {
            print '<td align="right">';
            if (isset($invoices_draft_list[$obj->rowid])) {
                print price($invoices_draft_list[$obj->rowid]['total_ttc']);
                $totalarray['totalttc'] += $invoices_draft_list[$obj->rowid]['total_ttc'];
            }
            print "</td>\n";
            if (!$i) $totalarray['nbfield']++;
    ...
    if ($massaction == 'confirm_generate_invoices' || $action == 'remove_file' || $show_files) {
        /*
         * Show list of available documents
         */
        $urlsource = $_SERVER['PHP_SELF'] . '?formfilteraction=list&sortfield=' . $sortfield . '&sortorder=' . $sortorder . '&page=' . $page;
