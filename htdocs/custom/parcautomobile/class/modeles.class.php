...
class modeles extends Commonobject{ 
	...
	public $element='modeles';
	public $table_element='modeles';
	...
	public function select_with_filter($selected=0,$name='model',$showempty=1,$val="rowid",$opt="label",$id='',$attr=''){
	    ...
	    $moreforfilter.='<select width="100%" '.$attr.' class="flat minwidth200 maxwidth200" onchange=get_logo(this) id="select_'.$id.'" name="'.$name.'" required>';
		...
		if ($resql) {
			...
			while ($obj = $this->db->fetch_object($resql)) {
				...
				if($obj->marque){
                    $marque  = new marques($this->db);
                    $marque->fetch($obj->marque);
                     if(!empty($marque->logo)){
                        $minifile = getImageFileNameForSize($marque->logo, '');
                        $urlfile = DOL_URL_ROOT.'/viewimage.php?modulepart=parcautomobile&entity='.(!empty($obj->entity)?$obj->entity:$conf->entity).'&file=marques/'.$marque->rowid.'/'.$minifile.'&perm=download';
                    }
                }

	            $marque->fetch($obj->marque);
				$moreforfilter.='<option value="'.$obj->$val.'"';
	            $moreforfilter.=' tag_img="'.$urlfile.'"';
	            if ($obj->$val == $selected) $moreforfilter.=' selected ';
	            $moreforfilter.='>'.$marque->label.'/'.$obj->label.'</option>';
