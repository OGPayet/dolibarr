...
class statut extends Commonobject{
	...
	public $element='statut';
	public $table_element='statut';
	...
	public function select_with_filter($selected=0,$name='statut',$showempty=1,$val="rowid",$opt="label",$id='',$attr=''){
	...
	$resql = $this->db->query($sql);
		...
		if ($resql) {
			...
			while ($obj = $this->db->fetch_object($resql)) {
	            ...
	            $moreforfilter.='>'.$langs->trans($obj->$opt).'</option>';
			}
			$this->db->free($resql);
		}
