...
/**
 * \file        core/dictionaries/eventdefaultconfidentiality.dictionary.php
 * \ingroup     eventconfidentiality
 * \brief       Class of the dictionary default confidentiality
 */

dol_include_once('/advancedictionaries/class/dictionary.class.php');

/**
 * Class for eventdefaultconfidentialityDictionary
 */
class eventdefaultconfidentialityDictionary extends Dictionary
{
    /**
     ...
     */
    public $familyLabel = 'Module163092Name';
     ...
     */
    public $moduleLabel = 'Module163092Name';

    /**
     * @var string      Name of this dictionary for show in the list, translated if key found
     */
    public $nameLabel = 'EventDefaultConfidentialityDictionaryLabel';
    ...
    /**
     * @var Dictionary    Cache of the list of tags
     */
    public $tags_cache = null;

    /**
	 * Load the cache of the list of tags
	 *
     * @return  void
	 */
    public function load_tags()
    {
        if (!isset($this->tags_cache)) {
            $this->tags_cache = Dictionary::getDictionary($this->db, 'eventconfidentiality', 'eventconfidentialitytags');
            $this->tags_cache->fetch_lines();
        }
    }
	...
	protected function initialize()
    {
        ...
        $this->fields['type_action'] = array(
            'name' => 'type',
            'label' => 'EventConfidentialityTypeAction',
            'type' => 'select',
            'options' => array(
        ...
        $this->fields['element_origin'] = array(
            ...
            'type' => 'select',
            'options' => array(
                'order' => $langs->trans('Commande'),
                'propal' => $langs->trans('Proposition Commerciale'),
            ),
            'is_require' => true,
        );
        ...
        $this->fields['type'] = array(
            'name' => 'type',
            'label' => 'EventConfidentialityType',
            'type' => 'select',
            'options' => array(
                EventConfidentiality::STATUS_INTERNE => $langs->trans('EventConfidentialityTypeInterne'),
                EventConfidentiality::STATUS_EXTERNE => $langs->trans('EventConfidentialityTypeExterne'),
            ),
            'is_require' => true,
        );
        $this->fields['level'] = array(
            'name' => 'level',
            'label' => 'EventConfidentialityLevel',
            'type' => 'select',
            'options' => array(
                EventConfidentiality::LEVEL_SHOW => $langs->trans('EventConfidentialityLevelShow'),
                EventConfidentiality::LEVEL_BLURRED => $langs->trans('EventConfidentialityLevelBlurred'),
                EventConfidentiality::LEVEL_HIDDEN => $langs->trans('EventConfidentialityLevelHidden'),
            ),
            'is_require' => true,
        );
    }
