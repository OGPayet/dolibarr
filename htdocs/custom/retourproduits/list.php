...
// Security check
$retourid = GETPOST('id','int');
if ($user->societe_id) $socid=$user->societe_id;
$result = restrictedArea($user, 'expedition',$retourid,'');

$diroutputmassaction=$conf->expedition->dir_output . '/temp/massgeneration/'.$user->id;

$search_ref_retour = GETPOST("search_ref_retour");
...
// Initialize technical object to manage hooks of page. Note that conf->hooks_modules contains array of hook context
$hookmanager->initHooks(array('returnproductslist'));
...
$checkedtypetiers=0;
$arrayfields=array(
    'rp.ref'=>array('label'=>$langs->trans("Ref"), 'checked'=>1),
    'rp.ref_customer'=>array('label'=>$langs->trans("RefCustomer"), 'checked'=>1),
    ...
    'rp.date_delivery'=>array('label'=>$langs->trans("DateDeliveryPlanned"), 'checked'=>1),
    'rp.datec'=>array('label'=>$langs->trans("DateCreation"), 'checked'=>0, 'position'=>500),
    'rp.tms'=>array('label'=>$langs->trans("DateModificationShort"), 'checked'=>0, 'position'=>500),
    'rp.fk_statut'=>array('label'=>$langs->trans("Status"), 'checked'=>1, 'position'=>1000),
    ...
    'rp.billed'=>array('label'=>$langs->trans("Billed"), 'checked'=>1, 'position'=>1000, 'enabled'=>(!empty($conf->global->WORKFLOW_BILL_ON_SHIPMENT)))
);
...
$return=new RetourProduits($db);
$formcompany=new FormCompany($db);

$helpurl='EN:Module_Shipments|FR:Module_Exp&eacute;ditions|ES:M&oacute;dulo_Expediciones';
llxHeader('',$langs->trans('ListOfReturnProducts'),$helpurl);

$sql = "SELECT rp.rowid, rp.ref, rp.ref_customer, rp.date_expedition as date_expedition, rp.date_delivery as date_livraison, l.date_delivery as date_reception, rp.fk_statut, rp.billed,";
$sql.= ' s.rowid as socid, s.nom as name, s.town, s.zip, s.fk_pays, s.client, s.code_client, ';
$sql.= " typent.code as typent_code,";
$sql.= " state.code_departement as state_code, state.nom as state_name,";
$sql.= ' rp.date_creation as date_creation, rp.tms as date_update';
...
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."expedition_extrafields as ef on (rp.rowid = ef.fk_object)";
$sql.= " LEFT JOIN ".MAIN_DB_PREFIX."societe as s ON s.rowid = rp.fk_soc";
$sql.= " LEFT JOIN ".MAIN_DB_PREFIX."c_country as country on (country.rowid = s.fk_pays)";
$sql.= " LEFT JOIN ".MAIN_DB_PREFIX."c_typent as typent on (typent.id = s.fk_typent)";
$sql.= " LEFT JOIN ".MAIN_DB_PREFIX."c_departements as state on (state.rowid = s.fk_departement)";
$sql.= " LEFT JOIN ".MAIN_DB_PREFIX."element_element as ee ON rp.rowid = ee.fk_source AND ee.sourcetype = 'retourproduits' AND ee.targettype = 'delivery'";
$sql.= " LEFT JOIN ".MAIN_DB_PREFIX."livraison as l ON l.rowid = ee.fk_target";
if (!$user->rights->societe->client->voir && !$socid)	// Internal user with no permission to see all
{
	$sql.= ", ".MAIN_DB_PREFIX."societe_commerciaux as sc";
}
$sql.= " WHERE rp.entity IN (".getEntity('expedition').")";
if (!$user->rights->societe->client->voir && !$socid)	// Internal user with no permission to see all
{
	$sql.= " AND rp.fk_soc = sc.fk_soc";
...
if ($search_ref_exp) $sql .= natural_search('rp.ref', $search_ref_exp);
...
if ($resql)
{
	...
	$expedition = new RetourProduits($db);
	...
	// Ref
	if (! empty($arrayfields['rp.ref']['checked']))
	{
	...
	// Ref customer
	if (! empty($arrayfields['rp.ref_customer']['checked']))
	{
	    print '<td class="liste_titre">';
	...
	if (! empty($arrayfields['rp.ref']['checked']))            print_liste_field_titre($arrayfields['rp.ref']['label'], $_SERVER["PHP_SELF"],"rp.ref","",$param,'',$sortfield,$sortorder);
	if (! empty($arrayfields['rp.ref_customer']['checked']))   print_liste_field_titre($arrayfields['rp.ref_customer']['label'], $_SERVER["PHP_SELF"],"rp.ref_customer","",$param,'',$sortfield,$sortorder);
	...
	if (! empty($arrayfields['rp.date_delivery']['checked']))  print_liste_field_titre($arrayfields['rp.date_delivery']['label'], $_SERVER["PHP_SELF"],"rp.date_delivery","",$param, 'align="center"',$sortfield,$sortorder);
	...
	print '<tr class="oddeven">';
		...
		if (! empty($arrayfields['rp.date_delivery']['checked']))
		{
		print '<td align="center">';
        ...
        {
		    $return->fetchObjectLinked($return->id,$return->element);
            $receiving='';
            if (count($return->linkedObjects['delivery']) > 0) $receiving=reset($return->linkedObjects['delivery']);
		...
		print $hookmanager->resPrint;
		// Date creation
		if (! empty($arrayfields['rp.datec']['checked']))
		{
		...
		// Date modification
		if (! empty($arrayfields['rp.tms']['checked']))
		{
		    print '<td align="center" class="nowrap">';
		...
		// Status
		if (! empty($arrayfields['rp.fk_statut']['checked']))
		{
		    print '<td align="right" class="nowrap">'.$return->LibStatut($obj->fk_statut,5).'</td>';
		    if (! $i) $totalarray['nbfield']++;
		}
		// Billed
		if (! empty($arrayfields['rp.billed']['checked']))
		{
