...
dol_include_once("/retourproduits/core/modules/retourproduits/modules_retourproduits.php");
 ...
 */
class pdf_retourproduits_rouget extends ModelePdfExpedition
{
	...
	function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		$outputlangs->load("retourproduits@retourproduidts");
		...
		if ($conf->retourproduits->dir_output) {
			// Definition de $dir et $file
			if ($object->specimen) {
				$dir = $conf->retourproduits->dir_output;
				$file = $dir . "/SPECIMEN.pdf";
			} else {
				$expref = dol_sanitizeFileName($object->ref);
				$dir = $conf->retourproduits->dir_output . '/' . $expref;
				$file = $dir . "/" . $expref . ".pdf";
			}
			...
			if (file_exists($dir)) {
				...
				for ($i = 0; $i < $nblignes; $i++) {
					...
					$pdf->MultiCell(($this->posxpuht - $this->posxqtytoship), 3, $object->lines[$i]->qty_return, '', 'C');


					$pdf->SetXY($this->posxqtytoship, $curY);
					$entrepot = new Entrepot($this->db);

					$entrepot->fetch($object->lines[$i]->fk_entrepot_dest);
					$pdf->MultiCell(($this->posxqtytoship - $this->posxqtyordered), 3, $entrepot->get_full_arbo(), '', 'C');
					if (!empty($conf->global->MAIN_PDF_SHIPPING_DISPLAY_AMOUNT_HT)) {
				...
				// Pied de page
				/** Commanditaire **/
				if ($object->array_options['options_expd']) {
					$dims = $pdf->getPageDimensions();
					$pdf->SetFont('', 'B', 10);
					$pdf->SetXY($dims['lm'], $posy + 5);
					$pdf->MultiCell($dims['wk'] - $dims['rm'] - $dims['lm'], 2, $langs->trans("commanditaire"), 0, 'C', 0);
				}
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop = 0, $hidebottom = 0)
	{
		...
		if (empty($hidetop)) {
			...
			$pdf->MultiCell(($this->posxqtytoship - $this->posxqtyordered), 2, $outputlangs->transnoentities("RetourProduitQtyReturned"), '', 'C');
		}

		$pdf->line($this->posxqtytoship - 1, $tab_top, $this->posxqtytoship - 1, $tab_top + $tab_height);
		if (empty($hidetop)) {
			$pdf->SetXY($this->posxqtytoship, $tab_top + 1);
			$pdf->MultiCell(($this->posxpuht - $this->posxqtytoship), 2, $outputlangs->transnoentities("RetourProduitWarehouseReturned"), '', 'C');
		}
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		$pdf->SetXY($posx, $posy);
		$pdf->SetTextColor(0, 0, 60);
		$title = $outputlangs->transnoentities("RetourProduitDocumentTitle");
		...
		$pdf->SetXY($posx, $posy);
		$pdf->SetTextColor(0, 0, 60);
		$pdf->MultiCell($w, 4, $outputlangs->transnoentities("RetourProduitDocumentRefTitle") . " : " . $object->ref, '', 'R');
