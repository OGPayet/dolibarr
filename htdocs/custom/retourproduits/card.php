...
$origin		= GETPOST('origin','alpha')?GETPOST('origin','alpha'):'retourproduits';
...
// !!!!! ==> A VOIR LA RESTRICTION A CE NIVEAU
/*if ($origin == 'retourproduits') $result=restrictedArea($user, $origin, $id);
else {
	$result=restrictedArea($user, 'retourproduits');
...
$hookmanager->initHooks(array('retourproduitscard','globalcard'));
...
/* *************************************************************************** */
{
	...
	if ($object->id > 0)
	{
		...
		$head=retourproduits_prepare_head($object);
		dol_fiche_head($head, 'retourproduits', $langs->trans("returnProducts"), 0, "retourproduits@retourproduits");
		...
		if ($action == 'valid')
		{
			$objectref = substr($object->ref, 1, 6);
			if ($objectref == 'RETURN')
			{
			...
			$text = $langs->trans("ConfirmValidateReturn",$numref);
	...
	print '<div class="underbanner clearboth"></div>';
		...
		print '<td align="center">'.$langs->trans("QtyOrdered").'</td>';
		print '<td align="center">'.$langs->trans("QtyShipped").'</td>';
		print '<td align="center">'.$langs->trans("QtyToReturn").'</td>';
...
/*		if ($origin && $origin_id > 0)
		...
		for ($i = 0 ; $i < $num_prod ; $i++)
		{
			...
			// Qty to return
			print '<td align="center">'.$lines[$i]->qty_return.'</td>';
			...
			{
				print '<td align="left">';
				if ($lines[$i]->fk_entrepot_dest > 0)
				{
					$entrepot = new Entrepot($db);
					$entrepot->fetch($lines[$i]->fk_entrepot_dest);
