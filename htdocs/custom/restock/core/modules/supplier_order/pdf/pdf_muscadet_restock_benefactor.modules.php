...
/**
 *	\file       htdocs/core/modules/supplier_order/pdf/pdf_muscadet_restock_benefactor.modules.php
 *	\ingroup    restock
 *	\brief      File of class to generate suppliers orders from muscadet restock benefactor model
 */
...
/**
 *	Class to generate the supplier orders with the muscadet_restock_benefactor model
 */
class pdf_muscadet_restock_benefactor extends ModelePDFSuppliersOrders
{
	var $db;
	var $name;
	var $description;
	...
	function __construct($db, $object = null)
	{
		...
		$this->db = $db;
		$this->name = "muscadet_restock_benefactor";
...
//�		global $conf;
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		if ($showaddress) {
			...
			// Delivery address (Benefactor)
            $object->fetch_thirdparty($object->array_options['options_companyrelationships_fk_soc_benefactor']);
            $carac_client_name = $outputlangs->convToOutputCharset($object->thirdparty->nom);
            $carac_destinataire = pdf_build_address($outputlangs, $this->emetteur, $object->thirdparty, '',  0, 'target');

            // Show delivery address frame
            $pdf->SetTextColor(0, 0, 0);
            $pdf->SetFont('', '', $default_font_size - 2);
            $pdf->SetXY($posx, $posy-5);
