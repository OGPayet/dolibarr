...
public function setVarsFromFetchObj(&$obj){
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/*if (! ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->surveyblocstatus->create))
		 || (! empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->surveyblocstatus->surveyblocstatus_advance->validate))))
		 {
		...
		{
			// Call trigger
			$result = $this->call_trigger('SURVEYBLOCSTATUS_VALIDATE', $user);
			if ($result < 0) $error++;
			// End call triggers
		}
		...
		if (!$error)
		{
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref))
			{
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'surveyblocstatus/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'surveyblocstatus/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				...
				$dirsource = $conf->interventionsurvey->dir_output.'/surveyblocstatus/'.$oldref;
				$dirdest = $conf->interventionsurvey->dir_output.'/surveyblocstatus/'.$newref;
				if (!$error && file_exists($dirsource))
				{
					...
					if (@rename($dirsource, $dirdest))
					{
						...
						$listoffiles = dol_dir_list($conf->interventionsurvey->dir_output.'/surveyblocstatus/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'SURVEYBLOCSTATUS_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'SURVEYBLOCSTATUS_CLOSE');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'SURVEYBLOCSTATUS_REOPEN');
	}
    ...
    public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
    {
        ...
        $label = '<u>'.$langs->trans("surveyBlocStatus").'</u>';
        $label .= '<br>';
        ...
        $url = dol_buildpath('/interventionsurvey/surveyblocstatus_card.php', 1).'?id='.$this->id;
        ...
        if (empty($notooltip))
        {
            ...
            {
                $label = $langs->trans("ShowsurveyBlocStatus");
		...
		$hookmanager->initHooks(array('surveyblocstatusdao'));
	...
	public function getLinesArray()
	{
	    ...
	    $objectline = new surveyBlocStatusLine($this->db);
	    $result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_surveyblocstatus = '.$this->id));
	...
	{
		global $langs, $conf;
		$langs->load("interventionsurvey@surveyblocstatus");

		if (empty($conf->global->INTERVENTIONSURVEY_SURVEYBLOCSTATUS_ADDON)) {
			$conf->global->INTERVENTIONSURVEY_SURVEYBLOCSTATUS_ADDON = 'mod_mymobject_standard';
		}
		...
		if (!empty($conf->global->INTERVENTIONSURVEY_SURVEYBLOCSTATUS_ADDON))
		{
			...
			$file = $conf->global->INTERVENTIONSURVEY_SURVEYBLOCSTATUS_ADDON.".php";
			$classname = $conf->global->INTERVENTIONSURVEY_SURVEYBLOCSTATUS_ADDON;
		...
		{
			print $langs->trans("Error")." ".$langs->trans("Error_INTERVENTIONSURVEY_SURVEYBLOCSTATUS_ADDON_NotDefined");
			return "";
		}
	}
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			...
			} elseif (!empty($conf->global->SURVEYBLOCSTATUS_ADDON_PDF)) {
				$modele = $conf->global->SURVEYBLOCSTATUS_ADDON_PDF;
			}
		}
