...
class SurveyAnswer extends CommonObject
{
    ...
    static public $API_WHITELIST_OF_PROPERTIES = array(
        'id' => '', 'label' => '', 'predefined_texts' => '', 'fk_chosen_predefined_texts' => '', 'color' => '', 'mandatory_justification' => '', 'consider_as_positive'=>''
    );
    ...
    public function setVarsFromFetchObj(&$obj, &$parent = null, bool $forceId = false)
    {
        ...
        if(!$this->fk_surveyquestion && $this->surveyQuestion){
            $this->fk_surveyquestion = $this->surveyQuestion->id;
        }
        ...
        if ($obj->predefined_texts) {
            foreach ($obj->predefined_texts as $predefinedTextObj) {
                ...
                $predefined_text->setVarsFromFetchObj($predefinedTextObj, $this, $forceId);
     ...
     *
     */
    public function save(&$user, $fk_surveyquestion = NULL, $noSurveyReadOnlyCheck = false, $notrigger = true)
    {
        global $langs;
    ...
    /**
     *
     * Merge current SurveyAnswer with a given SurveyAnswer
     *
     */
    ...
    public static function fillCacheFromParentObjectIds($arrayOfSurveyQuestionIds) {
        ...
        commonLoadCacheIdForLinkedObject(self::$DB_CACHE_FROM_SURVEYQUESTION, 'fk_surveyquestion', self::$DB_CACHE, $arrayOfSurveyQuestionIds);
