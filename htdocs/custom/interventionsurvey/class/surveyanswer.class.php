...
/**
 * \file        class/surveyanswer.class.php
 * \ingroup     interventionsurvey
 * \brief       This file is a CRUD class file for surveyAnswer (Create/Read/Update/Delete)
 */
 ...
 */
class SurveyAnswer extends CommonObject
{
    /**
     * @var string ID to identify managed object
     */
    public $element = 'surveyanswer';

    /**
     * @var string Name of table without prefix where object is stored
     */
    public $table_element = 'interventionsurvey_surveyanswer';

    /**
     * @var int  Does surveyanswer support multicompany module ? 0=No test on entity, 1=Test with field entity, 2=Test with link by societe
     */
    ...
    /**
     * @var string String with name of icon for surveyanswer. Must be the part after the 'object_' into object_surveyanswer.png
     */
    public $picto = 'surveyanswer@interventionsurvey';
     ...
     */
    //public $table_element_line = 'interventionsurvey_surveyanswerline';

    /**
     * @var int    Field with ID of parent key if this field has a parent
     */
    //public $fk_element = 'fk_surveyanswer';

    /**
     * @var int    Name of subtable class that manage subtable lines
     */
    //public $class_element_line = 'surveyAnswerline';
     ...
     */
    //protected $childtablesoncascade=array('interventionsurvey_surveyanswerdet');

    /**
     * @var surveyAnswerLine[]     Array of subtable lines
     */
    //public $lines = array();
    ...
    public function __construct(DoliDB $db)
    {
        ...
        /*if ($user->rights->interventionsurvey->surveyanswer->read) {
    ...
    public function validate($user, $notrigger = 0)
    {
        ...
        /*if (! ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->surveyanswer->create))
		 || (! empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->surveyanswer->surveyanswer_advance->validate))))
		 {
        ...
        if (!$error && !$notrigger) {
            // Call trigger
            $result = $this->call_trigger('SURVEYANSWER_VALIDATE', $user);
            if ($result < 0) $error++;
            // End call triggers
        }
        ...
        if (!$error) {
            ...
            if (preg_match('/^[\(]?PROV/i', $this->ref)) {
                ...
                $sql = 'UPDATE ' . MAIN_DB_PREFIX . "ecm_files set filename = CONCAT('" . $this->db->escape($this->newref) . "', SUBSTR(filename, " . (strlen($this->ref) + 1) . ")), filepath = 'surveyanswer/" . $this->db->escape($this->newref) . "'";
                $sql .= " WHERE filename LIKE '" . $this->db->escape($this->ref) . "%' AND filepath = 'surveyanswer/" . $this->db->escape($this->ref) . "' and entity = " . $conf->entity;
                $resql = $this->db->query($sql);
                if (!$resql) {
                    $error++;
                    $this->error = $this->db->lasterror();
                ...
                $dirsource = $conf->interventionsurvey->dir_output . '/surveyanswer/' . $oldref;
                $dirdest = $conf->interventionsurvey->dir_output . '/surveyanswer/' . $newref;
                if (!$error && file_exists($dirsource)) {
                    ...
                    if (@rename($dirsource, $dirdest)) {
                        ...
                        $listoffiles = dol_dir_list($conf->interventionsurvey->dir_output . '/surveyanswer/' . $newref, 'files', 1, '^' . preg_quote($oldref, '/'));
    ...
    public function setDraft($user, $notrigger = 0)
    {
        ...
        return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'SURVEYANSWER_UNVALIDATE');
    }
    ...
    public function cancel($user, $notrigger = 0)
    {
        ...
        return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'SURVEYANSWER_CLOSE');
    }
    ...
    public function reopen($user, $notrigger = 0)
    {
        ...
        return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'SURVEYANSWER_REOPEN');
    }
    ...
    public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
    {
        ...
        $label = '<u>' . $langs->trans("surveyAnswer") . '</u>';
        $label .= '<br>';
        ...
        $url = dol_buildpath('/interventionsurvey/surveyanswer_card.php', 1) . '?id=' . $this->id;
        ...
        if (empty($notooltip)) {
            if (!empty($conf->global->MAIN_OPTIMIZEFORTEXTBROWSER)) {
                $label = $langs->trans("ShowsurveyAnswer");
        ...
        $hookmanager->initHooks(array('surveyanswerdao'));
    ...
    public function getLinesArray()
    {
        ...
        $objectline = new surveyAnswerLine($this->db);
        $result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql' => 'fk_surveyanswer = ' . $this->id));
    ...
    {
        global $langs, $conf;
        $langs->load("interventionsurvey@surveyanswer");

        if (empty($conf->global->INTERVENTIONSURVEY_SURVEYANSWER_ADDON)) {
            $conf->global->INTERVENTIONSURVEY_SURVEYANSWER_ADDON = 'mod_mymobject_standard';
        }

        if (!empty($conf->global->INTERVENTIONSURVEY_SURVEYANSWER_ADDON)) {
            $mybool = false;

            $file = $conf->global->INTERVENTIONSURVEY_SURVEYANSWER_ADDON . ".php";
            $classname = $conf->global->INTERVENTIONSURVEY_SURVEYANSWER_ADDON;
            ...
            print $langs->trans("Error") . " " . $langs->trans("Error_INTERVENTIONSURVEY_SURVEYANSWER_ADDON_NotDefined");
            return "";
        }
    }
    ...
    public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
    {
        ...
        if (!dol_strlen($modele)) {
            ...
            } elseif (!empty($conf->global->SURVEYANSWER_ADDON_PDF)) {
                $modele = $conf->global->SURVEYANSWER_ADDON_PDF;
            }
        }
     ...
     *
     */
    public function save($user, $fk_surveyquestion = NULL, $noSurveyReadOnlyCheck = false)
    {
        global $langs;
    ...
    {
        $this->fetchParentCommon("SurveyQuestion", $this->fk_surveyquestion, $this->surveyQuestion);
    }
    ...
    {
        $this->fetchParent();
        return $this->surveyQuestion->is_survey_read_only();
    }
}
