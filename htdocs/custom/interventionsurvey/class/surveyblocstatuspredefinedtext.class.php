...
class SurveyBlocStatusPredefinedText extends CommonObject
{
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/*if (! ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->surveyblocstatuspredefinedtext->create))
		 || (! empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->surveyblocstatuspredefinedtext->surveyblocstatuspredefinedtext_advance->validate))))
		 {
		...
		{
			// Call trigger
			$result = $this->call_trigger('SURVEYBLOCSTATUSPREDEFINEDTEXT_VALIDATE', $user);
			if ($result < 0) $error++;
			// End call triggers
		}
		...
		if (!$error)
		{
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref))
			{
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'surveyblocstatuspredefinedtext/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'surveyblocstatuspredefinedtext/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				...
				$dirsource = $conf->interventionsurvey->dir_output.'/surveyblocstatuspredefinedtext/'.$oldref;
				$dirdest = $conf->interventionsurvey->dir_output.'/surveyblocstatuspredefinedtext/'.$newref;
				if (!$error && file_exists($dirsource))
				{
					...
					if (@rename($dirsource, $dirdest))
					{
						...
						$listoffiles = dol_dir_list($conf->interventionsurvey->dir_output.'/surveyblocstatuspredefinedtext/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'SURVEYBLOCSTATUSPREDEFINEDTEXT_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'SURVEYBLOCSTATUSPREDEFINEDTEXT_CLOSE');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'SURVEYBLOCSTATUSPREDEFINEDTEXT_REOPEN');
	}
    ...
    public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
    {
        ...
        $label = '<u>'.$langs->trans("SurveyBlocStatusPredefinedText").'</u>';
        $label .= '<br>';
        ...
        $url = dol_buildpath('/interventionsurvey/surveyblocstatuspredefinedtext_card.php', 1).'?id='.$this->id;
        ...
        if (empty($notooltip))
        {
            ...
            {
                $label = $langs->trans("ShowSurveyBlocStatusPredefinedText");
		...
		$hookmanager->initHooks(array('surveyblocstatuspredefinedtextdao'));
	...
	public function getLinesArray()
	{
	    ...
	    $objectline = new SurveyBlocStatusPredefinedTextLine($this->db);
	    $result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_surveyblocstatuspredefinedtext = '.$this->id));
	...
	{
		global $langs, $conf;
		$langs->load("interventionsurvey@surveyblocstatuspredefinedtext");

		if (empty($conf->global->INTERVENTIONSURVEY_SURVEYBLOCSTATUSPREDEFINEDTEXT_ADDON)) {
			$conf->global->INTERVENTIONSURVEY_SURVEYBLOCSTATUSPREDEFINEDTEXT_ADDON = 'mod_mymobject_standard';
		}
		...
		if (!empty($conf->global->INTERVENTIONSURVEY_SURVEYBLOCSTATUSPREDEFINEDTEXT_ADDON))
		{
			...
			$file = $conf->global->INTERVENTIONSURVEY_SURVEYBLOCSTATUSPREDEFINEDTEXT_ADDON.".php";
			$classname = $conf->global->INTERVENTIONSURVEY_SURVEYBLOCSTATUSPREDEFINEDTEXT_ADDON;
		...
		{
			print $langs->trans("Error")." ".$langs->trans("Error_INTERVENTIONSURVEY_SURVEYBLOCSTATUSPREDEFINEDTEXT_ADDON_NotDefined");
			return "";
		}
	}
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			...
			} elseif (!empty($conf->global->SURVEYBLOCSTATUSPREDEFINEDTEXT_ADDON_PDF)) {
				$modele = $conf->global->SURVEYBLOCSTATUSPREDEFINEDTEXT_ADDON_PDF;
			}
		}
...
public function save($user, $fk_surveyblocstatus=NULL, $noSurveyReadOnlyCheck)
{
    global $langs;
    ...
    public function fetchParent(){
        $this->fetchParentCommon("SurveyBlocStatus", $this->fk_surveyblocstatus, $this->surveyBlocStatus);
    }
