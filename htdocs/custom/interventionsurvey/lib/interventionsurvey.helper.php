...
/**
 *
 * Get an array of file information thanks to an array of attached files information and an array of filename
 *
 * @param array $arrayOfFilename            List of filename to find
 * @param array $arrayOfFileInformation     List of attached files information
 * @param array $mimeTypeFilter             List of mimetype in with return files must have - ignored if empty
 * @return array                            List of file path
 */
function getListOfWantedFilesInformation($arrayOfFilename, $arrayOfFileInformation, $mimeTypeFilter = array())
{
    $result = array();
    $isMimeTypeFilterEmpty = empty($mimeTypeFilter);
    foreach ($arrayOfFilename as $fileName) {
        $fileObject = getItemFromThisArray($arrayOfFileInformation, array("name" => $fileName));
        if ($fileObject) {
