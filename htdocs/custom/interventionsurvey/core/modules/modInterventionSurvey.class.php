...
class modInterventionSurvey extends DolibarrModules
{
    ...
    public function __construct($db)
    {
        ...
        */
        ...
        $r++;
        $this->rights[$r][0] = $this->numero + $r; // Permission id (must not be already used)
        $this->rights[$r][1] = 'Ability to choose to no check survey data when cloturing intervention'; // Permission label
        $this->rights[$r][4] = 'survey'; // In php code, permission will be checked by test if ($user->rights->interventionsurvey->level1->level2)
        ...
        $r++;
        $this->rights[$r][0] = $this->numero + $r; // Permission id (must not be already used)
        $this->rights[$r][1] = 'Ability to reopen intervention from api'; // Permission label
        $this->rights[$r][4] = 'survey'; // In php code, permission will be checked by test if ($user->rights->interventionsurvey->level1->level2)
        $this->rights[$r][5] = 'reopenApi'; // In php code, permission will be checked by test if ($user->rights->interventionsurvey->level1->level2)
        $r++;
    ...
    public function init($options = '')
    {
        ...
        $extrafields->addExtraField('scheduled_stakeholder', $langs->trans('InterventionSurveyScheduledUserLabel'), 'chkbxlst', 10,  NULL, 'fichinter',   0, 0, NULL, array('options'=>array('user:firstname|lastname:rowid::statut = 1 AND fk_soc IS NULL'=>null)), 1, '', 1, 0, '', '', 'interventionsurvey@interventionsurvey', '$conf->interventionsurvey->enabled');
        ...
        $extrafields->addExtraField('send_fichinter_by_mail', $langs->trans('InterventionSurveyThirdPartiesToSendFichinterToLabel'), 'boolean', 305, NULL, 'fichinter', 0, 0, NULL, '', -1, '', -1, 0, '', '', 'interventionsurvey@interventionsurvey', '$conf->interventionsurvey->enabled && $conf->global->INTERVENTIONSURVEY_SEND_FICHINTER_BY_MAIL');
        // Permissions
        $this->remove($options);
