...
/**
 * \file /mailchimp/mailchimp/mailchimp.class.php
 * \ingroup mailchimp
 */
 ...
 */
class DolMailchimp extends CommonObject
{
	var $db; // !< To store db handler
	var $error; // !< To return error code (or message)
	var $errors = array(); // !< To return several error codes (or messages)
	var $element = 'mailchimp'; // !< Id that identify managed objects
	var $table_element = 'mailchimp'; // !< Name of table without prefix where object is stored
	var $mailchimp; // API Object
	...
	var $mailchimp_id;
	var $mailchimp_webid;
	var $mailchimp_listid;
	var $mailchimp_segmentid;
	var $mailchimp_sender_name;
	var $fk_user_author;
	var $datec = '';
	var $fk_user_mod;
	var $tms = '';
	...
	function create($user, $notrigger = 0) {
		...
		if (isset($this->mailchimp_id))
			$this->mailchimp_id = trim($this->mailchimp_id);
		if (isset($this->mailchimp_webid))
			$this->mailchimp_webid = trim($this->mailchimp_webid);
		if (isset($this->mailchimp_listid))
			$this->mailchimp_listid = trim($this->mailchimp_listid);
		if (isset($this->mailchimp_segmentid))
			$this->mailchimp_segmentid = trim($this->mailchimp_segmentid);
		if (isset($this->mailchimp_sender_name))
			$this->mailchimp_sender_name = trim($this->mailchimp_sender_name);

			// Check parameters
			// Put here code to add control on parameters values

		// Insert request
		$sql = "INSERT INTO " . MAIN_DB_PREFIX . "mailchimp(";

		$sql .= "entity,";
		$sql .= "fk_mailing,";
		$sql .= "mailchimp_id,";
		$sql .= "mailchimp_webid,";
		$sql .= "mailchimp_listid,";
		$sql .= "mailchimp_segmentid,";
		$sql .= "mailchimp_sender_name,";
		$sql .= "fk_user_author,";
		$sql .= "datec,";
		$sql .= "fk_user_mod";
		$sql .= ") VALUES (";
		...
		$sql .= " " . (! isset($this->mailchimp_id) ? 'NULL' : "'" . $this->mailchimp_id . "'") . ",";
		$sql .= " " . (! isset($this->mailchimp_webid) ? 'NULL' : "'" . $this->mailchimp_webid . "'") . ",";
		$sql .= " " . (! isset($this->mailchimp_listid) ? 'NULL' : "'" . $this->db->escape($this->mailchimp_listid) . "'") . ",";
		$sql .= " " . (! isset($this->mailchimp_segmentid) ? 'NULL' : "'" . $this->db->escape($this->mailchimp_segmentid) . "'") . ",";
		$sql .= " " . (! isset($this->mailchimp_sender_name) ? 'NULL' : "'" . $this->db->escape($this->mailchimp_sender_name) . "'") . ",";
		$sql .= " '" . $user->id . "',";
		...
		if (! $error) {
			$this->id = $this->db->last_insert_id(MAIN_DB_PREFIX . "mailchimp");
	...
	function fetch($id) {
		...
		$sql .= " t.fk_mailing,";
		$sql .= " t.mailchimp_id,";
		$sql .= " t.mailchimp_webid,";
		$sql .= " t.mailchimp_listid,";
		$sql .= " t.mailchimp_segmentid,";
		$sql .= " t.mailchimp_sender_name,";
		$sql .= " t.fk_user_author,";
		$sql .= " t.datec,";
		$sql .= " t.fk_user_mod,";
		$sql .= " t.tms";

		$sql .= " FROM " . MAIN_DB_PREFIX . "mailchimp as t";
		...
		if ($resql) {
			if ($this->db->num_rows($resql)) {
				...
				$this->mailchimp_id = $obj->mailchimp_id;
				$this->mailchimp_webid = $obj->mailchimp_webid;
				$this->mailchimp_listid = $obj->mailchimp_listid;
				$this->mailchimp_segmentid = $obj->mailchimp_segmentid;
				$this->mailchimp_sender_name = $obj->mailchimp_sender_name;
		...
		if ($resql) {
			if ($this->db->num_rows($resql)) {
				...
				while ( $obj = $this->db->fetch_object($resql) ) {
					...
					$line = new DolMailchimpLine();

					$line->id = $obj->rowid;

					$line->entity = $obj->entity;
					$line->fk_mailing = $obj->fk_mailing;
					$line->mailchimp_id = $obj->mailchimp_id;
					$line->mailchimp_webid = $obj->mailchimp_webid;
					$line->mailchimp_listid = $obj->mailchimp_listid;
					$line->mailchimp_segmentid = $obj->mailchimp_segmentid;
					$line->mailchimp_sender_name = $obj->mailchimp_sender_name;
	 ...
	 * @return int <0 if KO, >0 if OK
	 */
	function fetch_by_mailchimpid($id) {
		global $langs;
		$sql = "SELECT";
		$sql .= " t.rowid,";
		...
		$sql .= " FROM " . MAIN_DB_PREFIX . "mailchimp as t";
		$sql .= " WHERE t.mailchimp_id = '" . $this->db->escape($id) . "'";

		dol_syslog(get_class($this) . "::fetch_by_mailchimpid sql=" . $sql, LOG_DEBUG);
		$resql = $this->db->query($sql);
		if ($resql) {
			if ($this->db->num_rows($resql)) {
				$obj = $this->db->fetch_object($resql);
			...
			$this->error = "Error " . $this->db->lasterror();
			dol_syslog(get_class($this) . "::fetch_by_mailchimpid " . $this->error, LOG_ERR);
			return - 1;
		}
	}
	...
	function update($user = 0, $notrigger = 0) {
		...
		// Update request
		$sql = "UPDATE " . MAIN_DB_PREFIX . "mailchimp SET";

		$sql .= " entity=" . $conf->entity . ",";
		$sql .= " fk_mailing=" . (isset($this->fk_mailing) ? $this->fk_mailing : "null") . ",";
		$sql .= " mailchimp_id=" . (isset($this->mailchimp_id) ? "'" . $this->mailchimp_id . "'" : "null") . ",";
		$sql .= " mailchimp_webid=" . (isset($this->mailchimp_webid) ? "'" . $this->mailchimp_webid . "'" : "null") . ",";
		$sql .= " mailchimp_listid=" . (isset($this->mailchimp_listid) ? "'" . $this->db->escape($this->mailchimp_listid) . "'" : "null") . ",";
		$sql .= " mailchimp_segmentid=" . (isset($this->mailchimp_segmentid) ? "'" . $this->db->escape($this->mailchimp_segmentid) . "'" : "null") . ",";
		$sql .= " mailchimp_sender_name=" . (isset($this->mailchimp_sender_name) ? "'" . $this->db->escape($this->mailchimp_sender_name) . "'" : "null") . ",";
	...
	function delete($user, $notrigger = 0) {
		...
		if (! $error) {
			$sql = "DELETE FROM " . MAIN_DB_PREFIX . "mailchimp";
			$sql .= " WHERE rowid=" . $this->id;
	 ...
	 *
	 * return MailChimp Object of rise error
	 */
	private function getInstanceMailChimp() {
		global $conf, $langs;

		if (! is_object($this->mailchimp)) {
			if (empty($conf->global->MAILCHIMP_MAIL_SMTPS_ID)) {
				$langs->load("mailchimp@mailchimp");
				$this->error = $langs->trans("MailChimpAPIKeyNotSet");
				dol_syslog(get_class($this) . "::getInstanceMailChimp " . $this->error, LOG_ERR);
				return - 1;
			}
			if (empty($conf->global->MAILCHIMP_MAIL_SMTPS_PW)) {
				$langs->load("mailchimp@mailchimp");
				$this->error = $langs->trans("MailChimpSecretKeyNotSet");
				dol_syslog(get_class($this) . "::getInstanceMailChimp " . $this->error, LOG_ERR);
				return - 1;
			}
			...
			if (! empty($conf->global->MAILCHIMP_MAIL_EMAIL_TLS)) {
				...
				// TODO: Need to specify ssl_cainfo or PATH check Mailchimp.php
			} else {
				$param = array(
						'ssl_verifypeer' => false
				);
			}
			$mailchimp = new Mailchimp($conf->global->MAILCHIMP_MAIL_SMTPS_PW, $param);
	...
	/**
	 * Retreive MailChimp Contact List
	 *
	...
	function getListDestinaries($filters = array(), $start = 0, $limit = 100, $sort_field = 'created', $sort_dir = 'DESC') {
		...
		$result = $this->getInstanceMailChimp();
		if ($result < 0) {
		...
		try {
			...
			if (! empty($filters['list_id'])) {
				$response = $this->mailchimp->get('lists/' . $filters['list_id']);
			} else {
				$response = $this->mailchimp->get('lists',array('count'=>$conf->global->MAILCHIMP_NB_MAX_LIST));
			}
		} catch ( Exception $e ) {
	...
	/**
	 * Retreive MailChimp Contact for a list and a status
	 *
	 * @param entre midi et deux
	 *
	 */
	function updateMembersStatusForList($listid, $status="unsuscribed", $getinstance=true) {
	    global $user;

	    if($getinstance) {
	    $result = $this->getInstanceMailChimp();
	    if ($result < 0) {
	        dol_syslog(get_class($this) . "::updateMembersStatusForList " . $this->error, LOG_ERR);

	        if(!empty($this->debug)) {
	            echo get_class($this) . "::updateMembersStatusForList " . $this->error."\n";
	        }
	    ...
	    if($status=='cleaned') $fk_status = 5; //hardbounce (can't distinct from soft)
	    else $fk_status=3; //unsuscribed;
		...
		if(!empty($this->debug)) {
                    echo get_class($this) . "::api get list 'lists/". $listid . "/members' $status offset  " . $offset."\n";

                }

	        $response = $this->mailchimp->get('lists/' . $listid . '/members',array('status'=>$status,'count'=>100,'offset'=>$offset));

	        $count = count($response['members']);

	        if(!empty($this->debug)) {
	            echo get_class($this) . "::api get list 'lists/". $listid . "/members' $status count  " . $count."\n";
	        ...
	        if($count > 0) {
	            ...
	            foreach($response['members'] as $member) {
	                $this->updateTargetMailingStatus($user, $fk_status, $member['email_address']);
	            }
	...
	/**
	 * Retreive MailChimp segment List
	 *
	...
	function getListSegmentDestinaries($id) {
		...
		// Call
		try {
			$response = $this->mailchimp->get('lists/' . $id . '/segments');
		} catch ( Exception $e ) {
			$this->error = $e->getMessage();
			$error ++;
		}
	...
	/**
	 * Retreive MailChimp list for email
	 *
	...
	function getListForEmail($email) {
		...
		// Call
		try {
			$response = $this->mailchimp->get('lists', array(
					'email' => $email
			));
		} catch ( Exception $e ) {
			if (get_class($e) != 'Mailchimp_List_NotSubscribed') {
				$this->error = $e->getMessage();
				$error ++;
			}
		}
	...
	/**
	 * Retraive email from mailchimp segment
	 *
	 * @return int <0 if KO, >0 if OK
	 */
	function getEmailSegment() {
		...
		$url_string = "&segment[match]=all&segment[conditions][0][field]=static_segment&segment[conditions][0][op]=eq&segment[conditions][0][value]=" . $this->mailchimp_segmentid;

		$chunk_size = 4096; // in bytes
		$url = 'http://' . $dc . '.api.mailchimp.com/export/1.0/list?apikey=' . $conf->global->MAILCHIMP_MAIL_SMTPS_PW . '&id=' . $this->mailchimp_listid . $url_string;
		// print $url.'<BR>';
		/**
	...
	/**
	 * Retraive email from mailchimp List
	 *
	 * @return int <0 if KO, >0 if OK
	 */
	function getEmailList() {
		...
		$chunk_size = 4096; // in bytes
		$url = 'http://' . $dc . '.api.mailchimp.com/export/1.0/list?apikey=' . $conf->global->MAILCHIMP_MAIL_SMTPS_PW . '&status=subscribed&id=' . $this->mailchimp_listid;
		// print $url.'<BR>';
		/**
		 * a more robust client can be built using fsockopen *
		 */
	...
	function getCampaignActivity() {
		...
		// $chunk_size = 4096; //in bytes
		$url = 'http://' . $dc . '.api.mailchimp.com/export/1.0/campaignSubscriberActivity?apikey=' . $conf->global->MAILCHIMP_MAIL_SMTPS_PW . '&id=' . $this->mailchimp_id . '&include_empty=true';
	...
	/**
	 * Create MailChimp segment on List
	 *
	...
	function createSegment($listid, $segmentname) {
		...
		// Call
		try {
			$response = $this->mailchimp->post('lists/' . $listid . '/segments', array(
					'name' => $segmentname,
	...
	/**
	 * update MailChimp segment on List
	 *
	 * @param string $listid the list id to connect to. Get by calling lists()
	...
	function updateSegment($listid, $segmentid, $emailtoadd, $resetseg = 0) {
		...
		if (! empty($resetseg)) {
			try {
				$myMembers = $this->mailchimp->get('/lists/' . $listid . '/segments/' . $segmentid . '/members');
				...
				$response = $this->mailchimp->post('lists/' . $listid . '/segments/' . $segmentid, array(
		...
		foreach ( $batch as $groupbatch ) {
			...
			try {
				...
				$_SESSION['MAILCHIMP_MEMBERS_LAST_SEGMENT'] = gzdeflate(serialize($response));
			...
			if ($this->db->num_rows($resql)) {
				while ( $obj = $this->db->fetch_object($resql) ) {
					...
					if (! empty($obj->custemail) && isValidEMail($obj->custemail)) {
						$custemail = new DolMailchimpeMailLine($this->db);
	...
	function getListCampaign() {
		...
		// Call
		try {
			$response = $this->mailchimp->get('campaigns', array('count'=>$conf->global->MAILCHIMP_NB_MAX_LIST));
		} catch ( Exception $e ) {
			$this->error = $e->getMessage();
	...
	function getBatchInforamtion($batchnum='') {
		...
		// Call
		try {
			$response = $this->mailchimp->get('batches/'.$batchnum);
		} catch ( Exception $e ) {
			$this->error = $e->getMessage();
			dol_syslog(get_class($this) . "::getListCampaign " . $this->error, LOG_ERR);
			return - 1;
	...
	function addEmailToList($listid = 0, $array_email = array()) {
		...
		foreach ( $array_email as $email ) {
			...
			if (! empty($tmp_array[0]) && isValidEmail($tmp_array[0]) && ! in_array($tmp_array[0], $email_added)) {
				...
				$email_to_add[$idx_tbl][] = array (
						...
						'status'=>'subscribed', // subscribed (to add an address right away) | pending (to send a confirmation email) | unsubscribed or cleaned (to archive unused addresses)
						'email_type' => 'html',
		...
		foreach ( $email_to_add as $key_batch=>$batch_email_to_add ) {
			...
			foreach ( $batch_email_to_add as $key_mail=>$emails ) {
				...
				//var_dump($email_object);exit;
				$attributes_object = new stdClass();
				$attributes_object->method='PUT';
				$attributes_object->path='lists/' . $listid . '/members/'.$this->mailchimp->subscriberHash($email_object->email_address);
			...
			try {
				...
				$response = $this->mailchimp->post('batches/', $attributes);
				$idDolCamp = GETPOST('id');
				$_SESSION['MAILCHIMP_ID_BATCH'][$idDolCamp]=$response['id'];
//				while($response['status'] != 'finished'){
//					$response = $this->mailchimp->get('batches/'.$response['id']);
//
//				}

				$_SESSION['MAILCHIMP_LAST_BATCHES_ID'] = $response['id'];
				if (!empty($conf->global->MAILCHIMP_SHOW_API_MESSAGE)) setEventMessage($langs->trans('statusCode_'.$response['status']), 'warnings');
	...
	function getLastBatchesInfo()
	{
		...
		$response = $this->mailchimp->get('batches/'.$_SESSION['MAILCHIMP_LAST_BATCHES_ID']);
		return $response;
	}
	function getBatchesInfo($fk_camp)
	{
		$result = $this->getInstanceMailChimp();
		if ($result < 0) {
			dol_syslog(get_class($this) . "::getListDestinaries " . $this->error, LOG_ERR);
		...
		$response = $this->mailchimp->get('batches/'.$_SESSION['MAILCHIMP_ID_BATCH'][$fk_camp]);
		return $response;
	}
	...
	function deleteEmailFromList($listid = 0, $array_email = array()) {
		...
		foreach ( $array_email as $mail ) {
			try {
				...
				$response = $this->mailchimp->delete('lists/' . $listid . '/members/' . $this->mailchimp->subscriberHash($mail));
	...
	function unsubscribeEmail($listid, $email, $delete_member = false, $send_goodbye = true, $send_notify = true) {
		...
		$succes_count = 0;
		try {
			$response = $this->mailchimp->put('lists/' . $listid . '/members/' . $this->mailchimp->subscriberHash($email), array(
					'status' => 'unsubscribed'
			));
	...
	static function getLibStatus($status, $mode = 1) {
		...
		if ($mode == 0) {
			return $langs->trans('MailChimp' . $status);
		}
		if ($mode == 1) {

			if ($status == 'save') {
				return img_picto($langs->trans('MailChimp' . $status), 'stcomm0') . ' ' . $langs->trans('MailChimp' . $status);
			}
			if ($status == 'paused') {
				return img_picto($langs->trans('MailChimp' . $status), 'stcomm1_grayed') . ' ' . $langs->trans('MailChimp' . $status);
			}
			if ($status == 'schedule') {
				return img_picto($langs->trans('MailChimp' . $status), 'stcomm0_grayed') . ' ' . $langs->trans('MailChimp' . $status);
			}
			if ($status == 'sent') {
				return img_picto($langs->trans('MailChimp' . $status), 'stcomm3') . ' ' . $langs->trans('MailChimp' . $status);
			}
			if ($status == 'sending') {
				return img_picto($langs->trans('MailChimp' . $status), 'stcomm2') . ' ' . $langs->trans('MailChimp' . $status);
			}
		}
	}
	 ...
	 * @return string status
	 */
	function getMailChimpCampaignStatus($mode = 1) {
		$result = $this->getInstanceMailChimp();
		if ($result < 0) {
			dol_syslog(get_class($this) . "::getMailChimpCampaignStatus " . $this->error, LOG_ERR);
			return - 1;
		}

		$opts['campaign_id'] = $this->mailchimp_id;

		// Call
		try {
			$response = $this->mailchimp->get('campaigns/' . $this->mailchimp_id);
		} catch ( Exception $e ) {
			$this->error = $e->getMessage();
			dol_syslog(get_class($this) . "::getListCampaign " . $this->error, LOG_ERR);
			return - 1;
		...
		if ($mode == 1) {
			return DolMailchimp::getLibStatus($response['status']);
		} elseif ($mode == 0) {
	 ...
	 * @return int <0 if KO, >0 if OK
	 */
	function sendMailChimpCampaign() {
		$result = $this->getInstanceMailChimp();
		if ($result < 0) {
			dol_syslog(get_class($this) . "::sendMailChimpCampaign " . $this->error, LOG_ERR);
			return - 1;
		}

		try {
			$response = $this->mailchimp->post('campaigns/' . $this->mailchimp_id . '/actions/send');
		} catch ( Exception $e ) {
			$this->error = $e->getMessage();
			dol_syslog(get_class($this) . "::sendMailChimpCampaign " . $this->error, LOG_ERR);
			return - 1;
	...
	function importSegmentDestToDolibarr($segment_id) {
		...
		$this->mailchimp_segmentid = $segment_id;
	...
	/**
	 * TEST : methode pour récupérer le contenu d'un segment pour s'assurer qu'elle est vide ou non
	 * @param type $listid
	 * @param type $segmentid
	 * @return type
	 */
	function getMembersOfSegment($listid, $segmentid)
	{
		$result = $this->getInstanceMailChimp();
		$response = $this->mailchimp->get('lists/' . $listid . '/segments/' . $segmentid.'/members', array());

		return $response;
	}

	/**
	 * Export to list and segments mailchimp only segment from dolibarr email
	 *
	 ...
	 * @return int <0 if KO, >0 if OK
	 */
	function exportSegmentDesttoMailChimp($segmentid, $newsegmentname, $resetseg = 0) {
		global $conf;
		...
		if (count($this->email_lines)) {
			...
			$this->mailchimp_segmentid = $segmentid;
			if (! empty($newsegmentname)) {
				$result = $this->createSegment($this->mailchimp_listid, $newsegmentname);
				if ($result < 0) {
					return - 1;
				}
				$this->mailchimp_segmentid = $result;
			}

			$result = $this->updateSegment($this->mailchimp_listid, $this->mailchimp_segmentid, $this->email_lines, $resetseg);
			if ($result < 0) {
				return - 1;
			}
		}
	 ...
	 * @return int <0 if KO, >0 if OK
	 */
	function exportListDesttoMailChimp() {
		global $conf;
		...
		if (count($this->email_lines)) {
			...
			$result_add_to_list = $this->addEmailToList($this->mailchimp_listid, $this->email_lines);
	...
	/**
	 * Export to mailchimp only segment from dolibarr email
	 *
	 * @param int $segmentid segment id
	 * @param string $newsegmentname segment name
	 * @param int $resetseg segment
	 * @return int <0 if KO, >0 if OK
	 */
	function exportSegmentOnlyDesttoMailChimp($segmentid, $newsegmentname, $resetseg = 0) {
		global $conf;
	...
	/**
	 * Create the capaign on MailChimp
	 *
	 * @param user $user
	 * @return int <0 if KO, >0 if OK
	 */
	function createMailChimpCampaign($user) {
		$result = $this->getInstanceMailChimp();
		if ($result < 0) {
			dol_syslog(get_class($this) . "::createMailChimpCampaign " . $this->error, LOG_ERR);
			return - 1;
		}
		...
		$settings->from_name = $this->mailchimp_sender_name;
		...
		if (empty($this->mailchimp_id)) {
			try {
				...
				$response = $this->mailchimp->post('campaigns', array(
						'type' => $type,
			...
			$this->mailchimp_id = $response['id'];
			$r = $this->mailchimp->put('campaigns/' . $this->mailchimp_id . '/content', $content);
			...
			$this->mailchimp_webid = $newcampaign['web_id'];
	 ...
	 * @return int <0 if KO, >0 if OK
	 */
	public function updateMailChimpAllCampaignStatus($user,$month_filter=0) {
		$mailchimpstatic = new DolMailchimp($this->db);

		$result=$mailchimpstatic->fetch_all($month_filter);
		if ($result < 0) {
			dol_syslog(get_class($mailchimpstatic) . "::updateMailChimpAllCampaignStatus " . $mailchimpstatic->error, LOG_ERR);
			return - 1;
		}

		$error = 0;

		if (is_array($mailchimpstatic->lines) && count($mailchimpstatic->lines) > 0) {
			foreach ( $mailchimpstatic->lines as $line ) {
				if ($line->mailchimp_id != null) {

					$this->mailchimp_id = $line->mailchimp_id;
					$this->fk_mailing = $line->fk_mailing;
					$this->mailchimp_listid = $line->mailchimp_listid;
					$this->updateMailChimpCampaignStatus($user);
					if ($result < 0) {
	 ...
	 * @return int <0 if KO, >0 if OK
	 */
	public function updateMailChimpCampaignStatus($user) {
		global $conf;

		$result = $this->getInstanceMailChimp();
		if ($result < 0) {
			dol_syslog(get_class($this) . "::updateMailChimpCampaignStatus " . $this->error, LOG_ERR);
			return - 1;
		}
		...
		try {
			...
			$response = $this->mailchimp->get('campaigns/' . $this->mailchimp_id . '/content');
		...
		// Set Dolibarr campaign with this information from mailchimp
		...
		// Call
		try {
			$responsecampaigndt = $this->mailchimp->get('campaigns/' . $this->mailchimp_id);
		} catch ( Exception $e ) {
			$this->error = $e->getMessage();
			dol_syslog(get_class($this) . "::getListCampaign " . $this->error, LOG_ERR);
			return - 1;
		...
		dol_syslog(get_class($this) . "::updateMailChimpCampaignStatus" . dol_print_date(dol_now(), 'standard'), LOG_DEBUG);
		try {
			$response = $this->mailchimp->get('reports/' . $this->mailchimp_id . '/sent-to');
		} catch ( Exception $e ) {
			$this->error = $e->getMessage();
			dol_syslog(get_class($this) . "::updateMailChimpCampaignStatus " . $this->error, LOG_ERR);
			$error ++;
		...
		dol_syslog(get_class($this) . "::updateMailChimpCampaignStatus start unsubscribe " . dol_print_date(dol_now(), 'standard'), LOG_DEBUG);
		// Unsubscribe
		try {
			$response = $this->mailchimp->get('reports/' . $this->mailchimp_id . '/unsubscribed');
		} catch ( Exception $e ) {
			$this->error = $e->getMessage();
			dol_syslog(get_class($this) . "::updateMailChimpCampaignStatus " . $this->error, LOG_ERR);
			$error ++;
				...
				foreach ( $this->email_lines as $email ) {
					...
					if ($result < 0) {
						...
						if (is_array($this->contactemail_activity) && count($this->contactemail_activity) > 0) {
							require_once 'mailchimpactivites.class.php';
							$contact_activites = new MailchimpActivites($this->db);
							$contact_activites->fk_mailing = $this->fk_mailing;
							$contact_activites->mailchimp_id = $this->mailchimp_id;
		...
		$this->updateMembersStatusForList($this->mailchimp_listid,'cleaned',false);

		// Commit or rollback
		if ($error) {
			foreach ( $this->errors as $errmsg ) {
				dol_syslog(get_class($this) . "::updateMailChimpCampaignStatus " . $errmsg, LOG_ERR);
				$this->error .= ($this->error ? ', ' . $errmsg : $errmsg);
			}
			return - 1;
		} else {
	...
	public function isUnsubscribed($idList, $email) {
		global $conf, $langs;
		$response = $this->mailchimp->get('lists/' . $idList . '/members', array(
				'status' => 'unsubscribed'
		));
	...
	public function updateTargetMailingStatus($user, $status, $email = '', $notrigger = 0, $send_dt = '') {
		...
		if(empty($email) && empty($this->fk_mailing)) {
		    $this->errors[] = "updateTargetMailingStatus Error no mail or fk_mailing";
		    return -1;
		}
		...
		$sql.=" AND fk_mailing = " . $this->fk_mailing;
		...
		if(!empty($this->debug)) {
		    echo get_class($this) . "::updateTargetMailingStatus " . $email." $status\n";

		}

		/*if (! $error)
		 {
		 $object = new DolMailchimpTargetLine($this->db);
		 $object->id = $this->id;
		 $object->entity = $this->entity;
		 $object->fk_mailing = $this->fk_mailing;
		 $object->mailchimp_id = $this->mailchimp_id;
		 $object->mailchimp_webid = $this->mailchimp_webid;
		 $object->mailchimp_listid = $this->mailchimp_listid;
		 $object->mailchimp_segmentid = $this->mailchimp_segmentid;
		 $object->mailchimp_sender_name = $this->mailchimp_sender_name;
	...
	static public function libStatutDest($statut, $mode = 0) {
		...
		if ($mode == 0) {
			...
			if ($statut == 2)
				return $langs->trans("MailchimpStatusOpen");
			if ($statut == 3)
				return $langs->trans("MailchimpUnsucscribe");
			if ($statut == 4)
				return $langs->trans("MailchimpClick");
			if ($statut == 5)
				return $langs->trans("MailchimpHardBounce");
			if ($statut == 6)
				return $langs->trans("MailchimpSoftBounce");
		}
		if ($mode == 1) {
			if ($statut == - 1)
				return $langs->trans("MailingStatusError");
			...
			if ($statut == 1)
				return $langs->trans("MailingStatusSent");
			if ($statut == 2)
				return $langs->trans("MailchimpOpen");
			if ($statut == 3)
				return $langs->trans("MailchimpUnsucscribe");
			if ($statut == 4)
				return $langs->trans("MailchimpClick");
		...
		if ($mode == 2) {
			...
			if ($statut == 2)
				return $langs->trans("MailchimpOpen") . ' ' . img_picto($langs->trans("MailingStatusRead"), 'statut6');
			if ($statut == 3)
				return $langs->trans("MailchimpUnsucscribe") . ' ' . img_picto($langs->trans("MailchimpUnsucscribe"), 'statut8');
			if ($statut == 4)
				return $langs->trans("MailchimpClick") . ' ' . img_picto($langs->trans("MailchimpClick"), 'statut6');
			if ($statut == 5)
				return $langs->trans("MailchimpHardBounce") . ' ' . img_error();
			if ($statut == 6)
				return $langs->trans("MailchimpSoftBounce") . ' ' . img_error();
		}
		if ($mode == 3) {
			if ($statut == - 1)
				return $langs->trans("MailingStatusError") . ' ' . img_error();
	...
	public function getEmailcontactActivites($email = '') {
		...
		if (! empty($email)) {
			...
			foreach ( $res['lists'] as $lists ) {
				...
				try {
					$res_array=$this->mailchimp->get('campaigns', array(
							'list_id' => $lists['id']
					));
				...
				if (! empty($campaign['campaigns'])) {
					foreach ( $campaign['campaigns'] as $c ) {
						...
						if (is_array($c) && count($c) > 0) {
							...
							$staticmailchimp = new DolMailchimp($this->db);

							$line = new DolMailchimpActivitesLine();
							$line->campaign = $c['settings']['title'];
							$line->campaignid = $c['id'];
							$result = $staticmailchimp->fetch_by_mailchimpid($c['id']);

							if ($result < 0) {
								$this->error = $staticmailchimp->error;
								return - 1;
							}
							$line->fk_mailing = $staticmailchimp->fk_mailing;
							$succes_count = 0;

							if (! empty($line->campaignid) && ! empty($line->fk_mailing)) {
								// Call
								try {
									$responseact = $this->mailchimp->get('reports/' . $line->campaignid . '/email-activity/' . $this->mailchimp->subscriberHash($email));
									...
									if ($responseact['data'][0]['member']['status'] == 'unsubscribed') {
										...
										if ($resql) {
											if ($this->db->num_rows($resql)) {
												...
												$objstatuslibel = DolMailchimp::libStatutDest($objstatus->statut);
												$line->activites = array(
														0 => array(
	...
	function getNomUrl() {
		...
		$result = '<a href="' . dol_buildpath('/mailchimp/mailchimp/mailchimp.php', 1) . '?id=' . $object->id . '">';
		$result .= $object->titre;
		$result .= '</a>';
...
class DolMailchimpTargetLine extends DolMailchimp
{
	var $email;
	var $status;
...
class DolMailchimpActivitesLine
{
	public $campaign;
	public $campaignid;
	public $fk_mailing;
...
class DolMailchimpLine
{
	public $id;
	public $entity;
	public $fk_mailing;
	public $mailchimp_id;
	public $mailchimp_webid;
	public $mailchimp_listid;
	public $mailchimp_segmentid;
	public $mailchimp_sender_name;
