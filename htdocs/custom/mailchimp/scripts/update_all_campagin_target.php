...
if(!empty($argv[4])) $company_code = $argv[4];
...
if (isset($argv[3])) {
	...
	$result=$mailchimp->fetch_by_mailing($mailingid);
	if ($result<0) {
		echo '--- end error message=' . $mailchimp->error . "\n";
		$error++;
	} else {
		$result=$mailchimp->updateMailChimpCampaignStatus($user);
		if ($result<0) {
			echo '--- updateMailChimpCampaignStatus message=' . $mailchimp->error . "\n";
			$error++;
		}
	}
} else {
	...
	if ($result<0) {
		echo '--- updateMailChimpAllCampaignStatus error message=' . $mailchimp->error . "\n";
		$error++;
	}
}
