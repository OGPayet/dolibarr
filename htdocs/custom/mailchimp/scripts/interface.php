...
elseif ($action == 'getLastBatchesInfo')
{
	$mailchimp = new DolMailchimp($db);

	echo json_encode($mailchimp->getLastBatchesInfo());
	exit;
}
elseif ($action == 'getMembersSegmentInfo')
{
	$data = unserialize(gzinflate($_SESSION['MAILCHIMP_MEMBERS_LAST_SEGMENT']));
	echo json_encode($data);
	exit;
}elseif ($action == 'getBatchesInfo')
{
	$mailchimp = new DolMailchimp($db);
	$fk_camp=GETPOST('fk_camp');
	$response= $mailchimp->getBatchesInfo($fk_camp);
	if($response['status']=="finished"){
		$_SESSION['MAILCHIMP_ID_BATCH'][$fk_camp]="";
	}
