...
/**
 *       \file       /mailchimp/mailchimp/target.php
 *       \ingroup    mailchiùmp
 *       \brief      Page to define emailing targets link with mailchimp
 */
$res=false;
...
if ($object->fetch($id) >= 0)
{
	...
	dol_fiche_head($head, 'tabMailChimpTarget', $langs->trans("Mailing"), 0, 'email');
		...
		foreach ($modulesdir as $dir)
		{
            ...
            foreach($modulenames as $modulename)
            {
				...
				if ($qualified)
				{
					...
					print '<td data-lang-key="MailingModuleDesc'.$obj->name.'">';
	...
	if ($resql)
	{
		...
		print $formmailchimp->selectDestinariesStatus($search_dest_status,'search_dest_status',1);
		print '</td>';
		//Search Icon
		print '<td class="liste_titre" align="right">';
		...
		if ($num)
		{
			...
			include_once DOL_DOCUMENT_ROOT.'/contact/class/contact.class.php';
						...
						$m->fetch_thirdparty();
						print '<td align="center">';
						print '<a href="'.dol_buildpath('/mailchimp/mailchimp/contact_activites.php',1).'?id='.$m->id.'">'.$m->getFullName($langs).'</a>';
						print '</td>';
						print '<td align="center">';
			...
			print $obj->source_url;
				...
				else
				{
					...
					print $mailchimp::libStatutDest($obj->statut,2);
					print '</td>';
				}
