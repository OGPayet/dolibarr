...
class modBuyPriceHistory extends DolibarrModules
{
    ...
    public function init($options = '')
    {
        ...
        if ($this->_init($sql, $options) > 0) {
            ...
            $errors = array_merge($errors, $extrafieldsToolBox->cloneExtrafields('product_fournisseur_price', $staticBuyPriceHistory->table_element, true));
            if (empty($errors)) {
                $staticBuyPriceHistory->archiveAllPrice();
                $errors = $staticBuyPriceHistory->errors;
            }
            if (!empty($errors)) {
