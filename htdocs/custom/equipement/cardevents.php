<?php
/* Copyright (C) 2012-20174	Charlie BENKE	<charlie@patas-monkey.com>
 *
...
/**
 *	\file		htdocs/equipement/cardevents.php
...
if ($action == "addline" && $user->rights->equipement->creer) {
	...
	$tblEquipementRef=explode(($separatorlist ? $separatorlist : ";"),
			GETPOST("listEquipementRef"));

	//$db->begin();
	$error=0;
	foreach ($tblEquipementRef as $keyEquipement) {
		// pour g�rer le cas du dernier ; foireux
		if ($keyEquipement) {
...
if ( $user->rights->equipement->creer ) {
	...
	print '<th colspan=7>'.$langs->trans('ListEquipementToAddEvents').'</th></tr>';

	// Ajout ligne d'intervention
	$var=false;

	print "<tr >\n";
	print '<td colspan=7 valign=top>';
	print '<textarea name="listEquipementRef" cols="120" rows="'.ROWS_3.'"></textarea>';
	print '</td></tr>';
	...
	$doleditor=new DolEditor(
					'np_desc', GETPOST('np_desc'), '', 100, 'dolibarr_details', '',
					false, true, $conf->global->FCKEDITOR_ENABLE_DETAILS,ROWS_6, 60
	);
	$doleditor->Create();
	print '</td>';
	//
