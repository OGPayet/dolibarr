			...
			if (file_exists($dir)) {
				...
				for ($i = 0; $i < $nblignes; $i++) {
					...
					$pdf->setPageOrientation(
									'', 1,
									$heightforfooter+$heightforfreetext+$heightforinfotot
					);
					...
					if (isset($imglinesize['width']) && isset($imglinesize['height'])) {
						...
						$pdf->Image(
										...
										$curY, $imglinesize['width'], $imglinesize['height'], '', '', '',
										2, 300
						);	// Use 300 dpi
					...
					if ($pageposafter > $pageposbefore) {
						...
						pdf_writelinedesc(
										...
										$this->posxpicture-$curX, 3, $curX, $curY, $hideref, $hidedesc
						);
					...
					$pdf->MultiCell(
									($this->page_largeur - $this->marge_droite - $this->posxqtyleft), 3,
									$qty_left_to_ship, '', 'C'
					);
					...
					if ($conf->equipement->enabled) {
						$strTemp = $this->_categcompositionserial(
										$object->lines[$i]->fk_product, $object->id, $outputlangs
						);
						if ($strTemp) {
					...
					$strTemp = $this->_categcomposition(
									$object->lines[$i]->fk_product, $object->id, $outputlangs,
...
//		global $conf;
		...
		if (empty($hidetop)) {
			...
			$pdf->MultiCell(
							($this->posxqtyleft - $this->posxqtyyetship), 2,
		...
		if (empty($hidetop)) {
			...
			$pdf->MultiCell(
							($this->page_largeur - $this->marge_droite - $this->posxqtyleft), 2,
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		if (! empty($object->thirdparty->code_client)) {
			$textout = $outputlangs->transnoentities("CustomerCode")." : ";
			$textout.= $outputlangs->transnoentities($object->thirdparty->code_client);
			$posy+=4;
			$pdf->SetXY($posx, $posy);
			$pdf->SetTextColor(0, 0, 60);
			$pdf->MultiCell(100, 3, $textout, '', 'R');
		}
		...
		if (! empty($conf->$origin->enabled)) {
			...
			if ($result >= 0) {
				...
				$posYoff+= 8;
				$pdf->SetXY($this->page_largeur - $this->marge_droite - 100, $posYoff);
				$pdf->MultiCell(
								100, 2,
				...
				$posYoff+= 3;
				$pdf->SetXY($this->page_largeur - $this->marge_droite - 60, $posYoff);
				$pdf->MultiCell(
								60, 2,
		...
		if ($showaddress) {
			...
			$carac_client=pdf_build_address(
							...
							(!empty($object->contact)?$object->contact:null), $usecontact, 'targetwithdetails'
			);
	...
	function _Categcomposition($fk_product, $fk_expedition, $outputlangs, $qtypere)
	{
		...
		if ( $this->productinCateg($fk_product, $categproduct)>0) {
			...
			$sql  = 'SELECT p.rowid, p.ref, p.label, pa.qty';
			$sql .= ' FROM '.MAIN_DB_PREFIX.'product_association AS pa, '.MAIN_DB_PREFIX.'product AS p';
...
//		global $conf, $object;
		...
		if ( $this->productinCateg($fk_product, $categproduct)>0) {
			if ($fk_product_pere > 0) {
				...
				$sql  = 'SELECT eqfils.rowid, eqfils.ref';
				$sql .= ' FROM '.MAIN_DB_PREFIX.'equipement AS eqfils, '.MAIN_DB_PREFIX.'equipementassociation AS ea';
