 ...
 */
class pdf_expedition_rouget_equipement extends ModelePdfExpedition
{
	...
	function __construct($db=0)
	{
		...
		$this->db = $db;
		$this->name = "rouget equipement";
		$this->description = $langs->trans("DocumentModelEquipementSimple");
		...
		$this->posxqtytoship = $this->page_largeur - $this->marge_droite - 45;
		$this->posxqtyyetship = $this->page_largeur - $this->marge_droite - 30;
		$this->posxstilltoship = $this->page_largeur - $this->marge_droite - 15;
		...
		$this->posxpicture = $this->posxweightvol
				- (empty($conf->global->MAIN_DOCUMENTS_WITH_PICTURE_WIDTH)?20:$conf->global->MAIN_DOCUMENTS_WITH_PICTURE_WIDTH);

		if ($this->page_largeur < 210) { // To work with US executive format
			$this->posxweightvol-=20;
			$this->posxpicture-=20;
			$this->posxqtyordered-=20;
			$this->posxqtyyetship-=20;
		...
		if (! empty($conf->global->MAIN_GENERATE_SHIPMENT_WITH_PICTURE)) {
			...
			for ($i = 0 ; $i < $nblignes ; $i++) {
				...
				if (! empty($conf->global->PRODUCT_USE_OLD_PATH_FOR_PHOTO)) {
					$pdir[0] = get_exdir(
									$objphoto->id, 2, 0, 0, $objphoto, 'product'
					).$objphoto->id ."/photos/";
					$pdir[1] = get_exdir(
									0, 0, 0, 0, $objphoto, 'product'
					).dol_sanitizeFileName($objphoto->ref).'/';
				} else {
					$pdir[0] = get_exdir(
									0, 0, 0, 0, $objphoto, 'product'
					).dol_sanitizeFileName($objphoto->ref).'/';	// default
					$pdir[1] = get_exdir(
									$objphoto->id, 2, 0, 0, $objphoto, 'product'
					). $objphoto->id ."/photos/";	// alternative
				}
				...
				foreach ($pdir as $midir) {
					if (! $arephoto) {
						...
						foreach ($objphoto->liste_photos($dir, 1) as $key => $obj) {
							// If CAT_HIGH_QUALITY_IMAGES not defined, we use thumb if defined
		...
		if ($conf->expedition->dir_output) {
			...
			if (file_exists($dir)) {
				...
				if (	empty($conf->global->MAIN_DISABLE_FPDI)
					...
					$pdf->setSourceFile($conf->mycompany->dir_output.'/'.$conf->global->MAIN_ADD_PDF_BACKGROUND);
				...
				if ($conf->incoterm->enabled) {
					...
					if ($desc_incoterms) {
						...
						$pdf->SetFont('', '', $default_font_size - 1);
						$pdf->writeHTMLCell(
										190, 3, $this->posxdesc-1, $tab_top-1,
						...
						$pdf->Rect(
										...
										$this->page_largeur-$this->marge_gauche-$this->marge_droite, $height_incoterms+1
						);
				...
				if (	!empty($object->note_public)
					||	!empty($object->tracking_number)) {
					$tab_top = 88 + $height_incoterms;
					$tab_top_alt = $tab_top;

					$pdf->SetFont('', 'B', $default_font_size - 2);
					$pdf->writeHTMLCell(
									60, 4, $this->posxdesc-1, $tab_top-1,
									$outputlangs->transnoentities("TrackingNumber")." : ".$object->tracking_number,
									0, 1, false, true, 'L'
					);
					...
					if (! empty($object->tracking_number)) {
						...
						if (! empty($object->tracking_url)) {
							if ($object->shipping_method_id > 0) {
								...
								$code=$outputlangs->getLabelFromKey(
												$this->db, $object->shipping_method_id,
												'c_shipment_mode', 'rowid', 'code'
								);
								$label='';
					...
					if (! empty($object->note_public)) {
						...
						$pdf->writeHTMLCell(
										190, 3, $this->posxdesc-1, $tab_top_alt,
				...
				for ($i = 0; $i < $nblignes; $i++) {
					...
					if (isset($imglinesize['width'])
						&&	isset($imglinesize['height'])
					...
					if (isset($imglinesize['width']) && isset($imglinesize['height'])) {
						...
						$pdf->Image(
										$realpatharray[$i],
										$curX + (($this->posxqtyordered-$this->posxpicture-$imglinesize['width'])/2),
										$curY, $imglinesize['width'], $imglinesize['height'], '', '', '', 2, 300
						);
					...
					$posybefore= $curY;
					pdf_writelinedesc(
									$pdf, $object, $i, $outputlangs, $this->posxpicture-$curX,
									3, $curX, $curY, $hideref, $hidedesc
					);
					$posyafterprint=$pdf->GetY();
					...
					$strTemp = $this->_getEquipementSerialDesc($object, $outputlangs, $i);
					if ($strTemp) {
						$ytoadd = $posyafterprint - $posybefore;
						$pdf->writeHTMLCell(
										140, 3, $this->posxdesc+10, $curY + 4 + $ytoadd,
										$outputlangs->convToOutputCharset($strTemp), 0, 1
						);
						$curY = $curY+$ytoadd;
					}
					...
					if ($pageposafter > $pageposbefore) {
						...
						if ($strTemp) {
							$ytoadd=$posyafterprint - $posybefore;
							$pdf->writeHTMLCell(
											140, 3, $this->posxdesc+10, $curY+$ytoadd,
											$outputlangs->convToOutputCharset($strTemp), 0, 1
							);
							$curY = $curY+$ytoadd;
						}
					...
					if ($object->lines[$i]->fk_product_type == 0 && $object->lines[$i]->weight) {
						$weighttxt=$object->lines[$i]->weight*$object->lines[$i]->qty_shipped;
						$weighttxt.=' '.measuring_units_string($object->lines[$i]->weight_units, "weight");
					}
					$voltxt='';
					if ($object->lines[$i]->fk_product_type == 0 && $object->lines[$i]->volume) {
						$voltxt=$object->lines[$i]->volume*$object->lines[$i]->qty_shipped;
						$voltxt.=' '.measuring_units_string(
										$object->lines[$i]->volume_units?$object->lines[$i]->volume_units:0,
										"volume"
						);
					}

					$pdf->MultiCell(
									($this->posxqtyordered - $this->posxweightvol), 3,
									$weighttxt.(($weighttxt && $voltxt)?', ':'').$voltxt, '', 'C'
					);

					if (empty($conf->global->SHIPPING_PDF_HIDE_ORDERED)) {
						$pdf->SetXY($this->posxqtyordered, $curY);
						$pdf->MultiCell(
										($this->posxqtytoship - $this->posxqtyordered), 3,
										$object->lines[$i]->qty, '', 'C'
						);
					}

					$qtyshipped =0;
					$qty_left_to_ship =0;
					// si il y a déjà eu une expédition
					...
					// détermination de la quantité déjà envoyé et restante à envoyer
					...
					$pdf->MultiCell(($this->posxstilltoship - $this->posxqtyyetship), 3, $qty_yet_shipped, '', 'C');
					$pdf->SetFont('', '', $default_font_size - 1);
					$pdf->SetXY($this->posxstilltoship, $curY);
					$pdf->MultiCell(($this->marge_droite - $this->posxstilltoship), 2, $qty_left_to_ship, '', 'C');

					if (!empty($conf->global->MAIN_PDF_SHIPPING_DISPLAY_AMOUNT_HT)) {
						$pdf->SetXY($this->posxpuht, $curY);
						$pdf->MultiCell(
										($this->posxtotalht - $this->posxpuht-1), 3,
										price($object->lines[$i]->subprice, 0, $outputlangs), '', 'R'
						);

						$pdf->SetXY($this->posxtotalht, $curY);
						$pdf->MultiCell(
										($this->page_largeur - $this->marge_droite - $this->posxtotalht), 3,
										price($object->lines[$i]->total_ht, 0, $outputlangs), '', 'R'
						);
					}
					...
					while ($pagenb < $pageposafter) {
						...
						if ($pagenb == 1)
							...
							$this->_tableau(
											$pdf, $tab_top_newpage,
											$this->page_hauteur - $tab_top_newpage - $heightforfooter, 0,
											$outputlangs, 1, 1
							);
				...
				if ($pagenb == 1)
					$this->_tableau(
									$pdf, $tab_top,
									$this->page_hauteur - $tab_top - $heightforfooter - $heightforfreetext - $heightforinfotot,
									0, $outputlangs, 0, 0
					);
				else
					$this->_tableau(
									$pdf, $tab_top_newpage,
									$this->page_hauteur - $tab_top_newpage - $heightforfooter - $heightforfreetext - $heightforinfotot,
									0, $outputlangs, 1, 0
					);
...
//		$sign=1;
		...
		// Load dim data
		// attention cette fonction n'est dispo que depuis la 4.0
		...
		if ($object->trueWidth && $object->trueHeight && $object->trueDepth) {
			$object->trueVolume = price(
							($object->trueWidth * $object->trueHeight * $object->trueDepth),
							0, $outputlangs, 0, 0
			);
		...
		if (!empty($conf->global->MAIN_PDF_SHIPPING_DISPLAY_AMOUNT_HT)) {
			...
			$pdf->MultiCell(
							$this->page_largeur - $this->marge_droite - $this->posxtotalht, $tab2_hl,
							price($object->total_ht, 0, $outputlangs), 0, 'C', 1
			);
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
		...
		if (empty($hidetop)) {
			...
			$pdf->MultiCell(
							$this->posxweightvol - $this->posxdesc, 2,
							$outputlangs->transnoentities("Description"), '', 'L'
			);
		}
		...
		if (empty($hidetop)) {
			...
			$pdf->MultiCell(
							($this->posxqtyordered - $this->posxweightvol), 2,
							$outputlangs->transnoentities("WeightVolShort"), '', 'C'
			);
		}
		...
		if (empty($conf->global->SHIPPING_PDF_HIDE_ORDERED)) {
			...
			if (empty($hidetop)) {
				...
				$pdf->MultiCell(
								($this->posxqtytoship - $this->posxqtyordered), 2,
								$outputlangs->transnoentities("QtyOrdered"), '', 'C'
				);
			}
		}
		...
		if (empty($hidetop)) {
			...
			$pdf->MultiCell(
							($this->posxqtyyetship - $this->posxqtytoship), 2,
							$outputlangs->transnoentities("QtyToShip"), '', 'C'
			);
		}
...
//			$pdf->SetFont('', 'B', $default_font_size);
			$pdf->MultiCell(
							($this->posxstilltoship - $this->posxqtyyetship), 2,
							$outputlangs->transnoentities("QtyShipped"), '', 'C'
			);
//			$pdf->SetFont('', '', $default_font_size);
		}

		$pdf->line($this->posxstilltoship-1, $tab_top, $this->posxstilltoship-1, $tab_top + $tab_height);
		if (empty($hidetop)) {
			$pdf->SetXY($this->posxstilltoship, $tab_top+1);
			$pdf->MultiCell(
							($this->marge_droite - $this->posxstilltoship), 2,
							$outputlangs->transnoentities("KeepToShip"), '', 'C'
			);
		}
		...
		if (!empty($conf->global->MAIN_PDF_SHIPPING_DISPLAY_AMOUNT_HT)) {
			...
			if (empty($hidetop)) {
				...
				$pdf->MultiCell(
								($this->posxtotalht - $this->posxpuht), 2,
								$outputlangs->transnoentities("PriceUHT"), '', 'C'
				);
			}
			...
			if (empty($hidetop)) {
				...
				$pdf->MultiCell(
								($this->page_largeur - $this->marge_droite - $this->posxtotalht), 2,
								$outputlangs->transnoentities("TotalHT"), '', 'C'
				);
			}
		}
	}
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		if ($object->statut==0  && (! empty($conf->global->SHIPPING_DRAFT_WATERMARK)))
			pdf_watermark(
							$pdf, $outputlangs, $this->page_hauteur,
							$this->page_largeur, 'mm', $conf->global->SHIPPING_DRAFT_WATERMARK
			);
		...
		if (! empty($object->date_delivery)) {
			...
			$pdf->MultiCell(
							$w, 4,
							$outputlangs->transnoentities("DateDeliveryPlanned")." : ".dol_print_date(
											$object->date_delivery, "day", false, $outputlangs, true
							), '', 'R'
			);
		}
		...
		if (! empty($object->thirdparty->code_client)) {
			...
			$pdf->SetXY($posx, $posy);
			$pdf->SetTextColor(0, 0, 60);
			$textcell =$outputlangs->transnoentities("CustomerCode")." : ";
			$textcell.=$outputlangs->transnoentities($object->thirdparty->code_client);
			$pdf->MultiCell($w, 3, $textcell, '', 'R');
		}
		...
		if (! empty($conf->$origin->enabled)) {
			...
			if ($result >= 0) {
				...
				$pdf->MultiCell(
								$w, 2,
								$outputlangs->transnoentities("RefOrder") ." : ".$outputlangs->transnoentities($text),
								0, 'R'
				);
				$yoff = $yoff+3;
				$pdf->SetXY($this->page_largeur - $this->marge_droite - $w, $yoff);
				$datetoprint=dol_print_date($linkedobject->date, "day", false, $outputlangs, true);
				$pdf->MultiCell($w, 2, $outputlangs->transnoentities("OrderDate")." : ".$datetoprint, 0, 'R');
			}
		}
		...
		if ($showaddress) {
			...
			$pdf->MultiCell(
							$widthrecbox-2, 4,
							$outputlangs->convToOutputCharset($this->emetteur->name), 0, 'L'
			);
			$posy=$pdf->getY();
			...
			$carac_client=pdf_build_address(
							...
							(!empty($object->contact)?$object->contact:null),
							$usecontact, 'targetwithdetails', $object
			);
	...
	function _pagefoot(&$pdf, $object, $outputlangs, $hidefreetext=0)
	{
		...
		return pdf_pagefoot(
						$pdf, $outputlangs, 'SHIPPING_FREE_TEXT',
	...
	function _getEquipementSerialDesc($object, $outputlangs, $i)
	{
		...
		if ($idprod) {
			...
			dol_syslog("PDF_Expedition_Rouget_Equipement::_getEquipementSerialDesc - SQL = ".$sql);

			$result=$db->query($sql);
			if ($result) {
				$num = $db->num_rows($result);
				if ($num>0) {
					$retStr = $outputlangs->trans("EquipRef").' ='.($num>1?'<br/>':'');
					for ($i=0; $i<$num; $i++) {
						$objp = $db->fetch_object($result);
						$retStr .= ($num>1?'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;':'&nbsp;').$objp->ref.'<br/>';
					}
				}
			} else {
