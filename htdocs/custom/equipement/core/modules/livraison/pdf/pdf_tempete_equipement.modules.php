...
/**
 *	\file	   htdocs/core/modules/livraison/pdf/pdf_tempete.modules.php
 *	\ingroup	livraison
 *	\brief	  File of class to manage receving receipts with template tempete
...
/**
 *	Classe permettant de generer les bons de livraison au modele Tempete
 */
class pdf_tempete_equipement extends ModelePDFDeliveryOrder
{
	var $db;
	var $name;
	var $description;
	...
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "tempete equipement";
		$this->description = $langs->trans("DocumentModeltempeteEquipement");
	...
	function write_file($object, $outputlangs, $srctemplatepath='', $hidedetails=0, $hidedesc=1, $hideref=0)
	{
		...
		if ($conf->expedition->dir_output) {
			...
			if (file_exists($dir)) {
				...
				$pdf->SetKeyWords($object->ref." ".$outputlangs->transnoentities("DeliveryOrder"));
				...
				*/
				...
				for ($i = 0 ; $i < $nblines ; $i++) {
					...
					pdf_writelinedesc(
									...
									$this->posxcomm-$curX, 3, $curX, $curY, $hideref, $hidedesc
					);

					// Ajout du numéro de série, s'il existe...
					dol_syslog("PDF_Livraison_Tempete::write_file : searching Serial = ".$object->lines[$i]->fk_product);
					$strTemp = $this->_getEquipementSerialDesc($object, $outputlangs, $i);
					$pdf->writeHTMLCell(
									140, 3, $this->posxdesc+10, $curY+4,
					...
					if ($pageposafter > $pageposbefore) {
						...
						pdf_writelinedesc(
										$pdf, $object, $i, $outputlangs,
										$this->posxcomm-$curX, 4, $curX, $curY, $hideref, $hidedesc
						);
						$posyafter=$pdf->GetY();
					...
					$pdf->MultiCell(
									$this->page_largeur-$this->marge_droite-$this->posxqty,
									3, $object->lines[$i]->qty_shipped, 0, 'R'
					);
					/*
					 // Remise sur ligne
					...
					while ($pagenb < $pageposafter) {
						...
						if ($pagenb == 1)
							$this->_tableau(
											$pdf, $tab_top, $this->page_hauteur - $tab_top - $heightforfooter,
											0, $outputlangs, 0, 1
							);
						else
							$this->_tableau(
				...
				$pdf->MultiCell(
								30, 3,
								$outputlangs->transnoentities("Page").' '.$pdf->getAliasNumPage().'/'.$pdf->getAliasNbPages(),
								 0, 'L', 0);
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
		...
		if (empty($hidetop)) {
			...
			$pdf->MultiCell(
							$this->page_largeur-$this->marge_droite-$this->posxqty,
							2, $outputlangs->transnoentities("QtyShipped"), '', 'R'
			);
		}
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		$pdf->MultiCell(
						100, 3,
						$outputlangs->transnoentities("DeliveryOrder")." ".$outputlangs->convToOutputCharset($object->ref),
						'', 'R'
		);
		...
		if ($object->date_valid) {
			$pdfcelltext = $outputlangs->transnoentities("Date")." : ";
			$pdfcelltext.= dol_print_date(
							($object->date_delivery?$object->date_delivery:$date->valid),
							"%d %b %Y", false, $outputlangs, true
			);
			$pdf->MultiCell( 100, 4, $pdfcelltext, '', 'R');
		} else {
		...
		if ($object->thirdparty->code_client) {
			...
			$pdf->MultiCell(
							100, 3,
							$outputlangs->transnoentities("CustomerCode")." : " . $outputlangs->transnoentities($object->thirdparty->code_client),
							'', 'R'
			);
		}
		...
		if ($object->origin == 'expedition' || $object->origin == 'shipping') {
			...
			if ($conf->$origin->enabled) {
				...
				if ($result >= 0) {
					...
					$pdf->MultiCell(
									60, 2,
									$outputlangs->transnoentities("OrderDate")." : ".dol_print_date(
													$linkedobject->date, "day", false, $outputlangs, true
									), 0, 'R'
					);
				}
			}
		...
		if ($showaddress) {
			...
			$carac_client=pdf_build_address(
							...
							($usecontact?$object->contact:''), $usecontact,'target'
			);
	...
	{
		return pdf_pagefoot(
						$pdf, $outputlangs,'DELIVERY_FREE_TEXT',
	...
	/**
	 * Récupère la ligne de description à ajouter avec le n° de série de l'équipement pour la ligne $i
	 *
	 * @return String contenant le texte à afficher si un n° de série associé est trouvé.
	 * @author
	 */
	function _getEquipementSerialDesc($object, $outputlangs, $i) {
		...
		dol_syslog("PDF_Livraison_Tempete::_getEquipementSerialDesc - Object line = ".$i." fk_product = ".$object->lines[$i]->fk_product."  idProd = '".$idprod."'");
		$retStr = '';

		if ($idprod){
			// shipping / delivery link
			$object->fetchObjectLinked('', '', $object->id, $object->element);
			if (! empty($object->linkedObjectsIds['shipping'][0]))		// If there is a delivery
				$expeditionid =$object->linkedObjectsIds['shipping'][0];
			...
			$sql .= ' AND eqv.fk_expedition = '.$expeditionid;
			$sql .= ' AND eq.fk_product = '.$idprod;
			dol_syslog("PDF_Livraison_Tempete::_getEquipementSerialDesc - SQL = ".$sql);
