...
/**
 *	\file	   htdocs/equipement/tabs/contratAdd.php
 *	\brief	  List of all Equipement for join Events with a contract
 *	\ingroup	equipement
 */
...
// le client est toujours celui du contrat
...
dol_fiche_head($head, 'eventadd',  $langs->trans("Contract"), 0, 'contract');
...
if ($result) {
	...
	while ($i < $num) {
		...
		// ici la case � cocher de s�lection pour ajouter un �v�nement
		print "<td width=15px>";
	...
	if ($num > 0 ) {
		...
		$doleditor=new DolEditor(
						...
						$conf->global->FCKEDITOR_ENABLE_DETAILS, ROWS_3, 60
		);
		$doleditor->Create();
		print '</td>';
