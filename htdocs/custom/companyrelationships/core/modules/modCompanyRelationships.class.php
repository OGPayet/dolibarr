...
/**
 * 	\defgroup   companyrelationships     Module Company Relationships
 *  \brief      Example of a module descriptor.
 *				Such a file must be copied into htdocs/companyrelationships/core/modules directory.
 *  \file       htdocs/companyrelationships/core/modules/modCompanyRelationships.class.php
 *  \ingroup    companyrelationships
 *  \brief      Description and activation file for module Company Relationships
 */
include_once DOL_DOCUMENT_ROOT . '/core/modules/DolibarrModules.class.php';

/**
 *  Description and activation class for module Company Relationships
 */
class modCompanyRelationships extends DolibarrModules
{
	/**
	...
	public function __construct($db)
    {
        ...
        $this->db = $db;
        $langs->load('opendsi@companyrelationships');

        // Id for module (must be unique).
        // Use here a free id (See in Home -> System information -> Dolibarr for list of used modules id).
        $this->numero = 163020;        // TODO Go on page http://wiki.dolibarr.org/index.php/List_of_modules_id to reserve id number for your module
        // Key text used to identify module (for permissions, menus, etc...)
        $this->rights_class = 'companyrelationships';
        ...
        $this->picto = 'opendsi@companyrelationships';
        ...
        $this->module_parts = array(
            'dictionaries' => 1,
            'substitutions' => 1,
            'hooks' => array('globalcard', 'propallist', 'orderlist', 'invoicelist', 'shipmentlist', 'interventionlist', 'contractlist', 'digitalsignaturemanager', 'interventionmail'),
            'triggers' => 1
        );

        // Data directories to create when module is enabled.
        // Example: this->dirs = array("/mymodule/temp");
        $this->dirs = array('/companyrelationships');

        // Config pages. Put here list of php page, stored into mymodule/admin directory, to use to setup module.
        $this->config_page_url = array("dictionaries.php@companyrelationships");
        ...
        $this->langfiles = array("companyrelationships@companyrelationships", "opendsi@companyrelationships");
        ...
        $this->tabs = array(
            'thirdparty:+companyrelationships:SUBSTITUTION_COMPANIESRELATIONSHIPLABEL:companyrelationships@companyrelationships:$user->rights->societe->lire:/companyrelationships/companyrelationships.php?socid=__ID__',
        );

        if (!isset($conf->companyrelationships) || !isset($conf->companyrelationships->enabled)) {
            $conf->companyrelationships = new stdClass();
            $conf->companyrelationships->enabled = 0;
        }
	...
	public function init($options='')
	{
        ...
        foreach($publicSpaceAvailibilityElementList as $elementType) {
            ...
            $result = $extrafields->addExtraField($extrafieldName, 'CompanyRelationshipsBenefactorCompany', 'link', $extrafieldToKeepList[$extrafieldName]['pos'], '', $elementType, 0, 1, '0', array('options'=>array("Societe:societe/class/societe.class.php" => null)), 1, '', 1, 0, '', '', 'companyrelationships@companyrelationships', '$conf->companyrelationships->enabled');
            $result = $extrafields->update($extrafieldName, 'CompanyRelationshipsBenefactorCompany', 'link', '', $elementType, 0, 1, $extrafieldToKeepList[$extrafieldName]['pos'], array('options'=>array("Societe:societe/class/societe.class.php" => null)), 1, '', 1, 0, '0', '', '', 'companyrelationships@companyrelationships', '$conf->companyrelationships->enabled');

            // public space availability principal and benefactor
            $extrafieldName = $extrafieldToKeepArray[1];
            $result = $extrafields->addExtraField($extrafieldName, 'CompanyRelationshipsPublicSpaceAvailabilityPrincipal', 'boolean', $extrafieldToKeepList[$extrafieldName]['pos'], '', $elementType, 0, 0, '', '', 1, '$user->rights->companyrelationships->update_md->element', 0, 0, '', '', 'companyrelationships@companyrelationships', '$conf->companyrelationships->enabled');
			$result = $extrafields->update($extrafieldName, 'CompanyRelationshipsPublicSpaceAvailabilityPrincipal', 'boolean', '', $elementType, 0, 0, $extrafieldToKeepList[$extrafieldName]['pos'], '', 1, '$user->rights->companyrelationships->update_md->element', 1, 0, '', '', '', 'companyrelationships@companyrelationships', '$conf->companyrelationships->enabled');

			//
			$extrafieldName = $extrafieldToKeepArray[2];
            $result = $extrafields->addExtraField($extrafieldName, $langs->trans('CompanyRelationshipsPublicSpaceAvailabilityBenefactor'), 'boolean', $extrafieldToKeepList[$extrafieldName]['pos'], '', $elementType, 0, 0, '', '', 1, '$user->rights->companyrelationships->update_md->element', 0, 0, '', '', 'companyrelationships@companyrelationships', '$conf->companyrelationships->enabled');
            $result = $extrafields->update($extrafieldName, 'CompanyRelationshipsPublicSpaceAvailabilityBenefactor', 'boolean', '', $elementType, 0, 0, $extrafieldToKeepList[$extrafieldName]['pos'], '', 1, '$user->rights->companyrelationships->update_md->element', 1, 0, '', '', '', 'companyrelationships@companyrelationships', '$conf->companyrelationships->enabled');

            // watcher
            $extrafieldName = $extrafieldToKeepArray[3];
            $result = $extrafields->addExtraField($extrafieldName, $langs->trans('CompanyRelationshipsWatcherCompany'), 'link', $extrafieldToKeepList[$extrafieldName]['pos'], '', $elementType, 0, 0, '', array('options'=>array('societe:nom:rowid::(client = 1 OR client = 2 OR client = 3) AND status=1:Societe:societe/class/societe.class.php'=>null)), 1, '', 0, 0, '', '', 'companyrelationships@companyrelationships', '$conf->companyrelationships->enabled');
            $result = $extrafields->update($extrafieldName, 'CompanyRelationshipsWatcherCompany', 'link', '', $elementType, 0, 0, $extrafieldToKeepList[$extrafieldName]['pos'], array('options'=>array("Societe:societe/class/societe.class.php" => null)), 1, '', 1, 0, '0', '', '', 'companyrelationships@companyrelationships', '$conf->companyrelationships->enabled');
			// public space availability for watcher
            $extrafieldName = $extrafieldToKeepArray[4];
            $result = $extrafields->addExtraField($extrafieldName, $langs->trans('CompanyRelationshipsPublicSpaceAvailabilityWatcher'), 'boolean', $extrafieldToKeepList[$extrafieldName]['pos'],'', $elementType, 0, 0, '', '', 1, '$user->rights->companyrelationships->update_md->element', 1, 0, '', '', 'companyrelationships@companyrelationships', '$conf->companyrelationships->enabled');
            $result = $extrafields->update($extrafieldName, 'CompanyRelationshipsPublicSpaceAvailabilityWatcher', 'boolean', '', $elementType, 0, 0, $extrafieldToKeepList[$extrafieldName]['pos'], '', 1, '$user->rights->companyrelationships->update_md->element', 1, 0, '', '', '', 'companyrelationships@companyrelationships', '$conf->companyrelationships->enabled');
