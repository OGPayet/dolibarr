...
/**
 *  \file		htdocs/core/menus/standard/owntheme7-0-1.lib.php
 *  \brief		Library for file OwnTheme menus
 */
require_once DOL_DOCUMENT_ROOT.'/core/class/menubase.class.php';
global $langs;

/**
 * Core function to output top menu owntheme
 *
 ...
 * @return	void
 */
function print_owntheme_menu($db,$atarget,$type_user,&$tabMenu,&$menu,$noout=0,$mode='')
{
    ...
    print '<div class="slidemenu-btn">'."\n";
    print '  <a>'."\n";
    print '	  <div class="icon-plus-menu"></div>'."\n";
    print '  </a>'."\n";
    print '</div>'."\n";
	...
	$showmode=dol_owntheme_showmenu($type_user, $tmpentry, $listofmodulesforexternal);
	if ($showmode)
	{
	...
	$menuArbo = new Menubase($db,'owntheme');
	$newTabMenu = $menuArbo->menuTopCharger('','',$type_user,'owntheme',$tabMenu);	// Return tabMenu with only top entries

	$num = count($newTabMenu);
	for($i = 0; $i < $num; $i++)	{
		$idsel=(empty($newTabMenu[$i]['mainmenu'])?'none':$newTabMenu[$i]['mainmenu']);
		$showmode=dol_owntheme_showmenu($type_user,$newTabMenu[$i],$listofmodulesforexternal);
		if ($showmode == 1)
		{
	...
	if ( $conf->global->DOL_VERSION >='3.4.0' && $conf->global->DOL_VERSION <'3.5.0') {
	...
	if ( $conf->global->DOL_VERSION >='3.6.0' ) {
	return 0;
	}
}
...
/**
 * Core function to output left menu owntheme
 ...
 */
function print_left_owntheme_menu($db,$menu_array_before,$menu_array_after,&$tabMenu,&$menu,$noout=0,$forcemainmenu='',$forceleftmenu='',$moredata=null)
{
	global $user,$conf,$langs,$dolibarr_main_db_name,$mysoc;
	...
	if ($mainmenu)
	{
		...
		if ($mainmenu == 'commercial')
		{
			...
			if (! empty($conf->propal->enabled))
			{
				...
				if ($usemenuhider || empty($leftmenu) || $leftmenu=="propals") $newmenu->add("/comm/propal/list.php?leftmenu=propals&viewstatut=5", $langs->trans("PropalsAwaiting"), 2, $user->rights->propale->lire);
   ...
   //  			}
		...
		*/
		...
		$menuArbo = new Menubase($db,'owntheme');
		$newmenu = $menuArbo->menuLeftCharger($newmenu,$mainmenu,$leftmenu,(empty($user->societe_id)?0:1),'owntheme',$tabMenu);
		//var_dump($newmenu->liste);    //
	...
	if (empty($noout))
	{
		...
		for ($i = 0; $i < $num; $i++)
		{
			...
			$current_url = explode("htdocs", $current_url);
			$current_url = $current_url[1];
			// if(strpos($url, '?') !== FALSE){
			// 	$url_vr = "&";
			// }
			// parse_str($_SERVER['QUERY_STRING'], $actif_menu);
			// $actif_nc = ($actif_menu['text_menu'] == "menu_".$i) ? "nc_actif_element" : "";

			$actif_nc = (str_replace("&amp;", "&", $menu_array[$i]['url']) == $current_url) ? "nc_actif_element" : "";
			
			// print_r(($menu_array[$i]['url']));
			// print_r("<br>");
			// print_r($current_url);
			// print_r("<br>");
			// print_r($actif_nc);
			...
			{
				// if(!empty($actif_menu['leftmenu']) && empty($actif_menu['text_menu']))
				// 	$actif_nc = ($actif_menu['leftmenu'] == $menu_array[$i]['leftmenu']) ? "nc_actif_element" : "";

				if ($menu_array[$i]['enabled']) {
					print '<li class="menu_titre li_'.$menu_array[$i]['leftmenu'].'">
					'.$tabstring.'
					<a class="vmenu" data-actif="'.$actif_nc.'" href="'.$url.'"'.($menu_array[$i]['target']?' target="'.$menu_array[$i]['target'].'"':'').'>
					'.(!empty($menu_array[$i]['leftmenu']) ?
						'
						<div class="icon-'.$menu_array[$i]['leftmenu']." " : 
						'<div class="').'mainvmenu"></div> <span class="mainmenuaspan">'.$menu_array[$i]['titre'].'</span></a>'."\n";
				}
				else if ($showmenu)
				{
					print '<li class="'.$menu_array[$i]['titre'].' li_'.$menu_array[$i]['leftmenu'].' menu_titre">'.$tabstring.'<font class="vmenudisabled">'.$text_menu.'</font>'."\n";
				}
			}

			// parse_str($_SERVER['QUERY_STRING'], $actif2_menu);
			// $actif2_nc = ($actif2_menu['text_menu'] == "menu_".$i ) ? "nc_actif_element" : "s";
			// print_r($current_url);
			// print_r("<br>");
			// print_r($menu_array[$i]['url']);
			// die();
			...
			if ($menu_array[$i]['level'] > 0)
			{
				if ($menu_array[$i]['enabled'])
				{	
					$actif2_nc = (str_replace("&amp;", "&", $menu_array[$i]['url']) == $current_url) ? "nc_actif_element" : "";
				    print '<div data-actif="'.$actif2_nc.'" class="menu_contenu'.$menu_array[$i]['level'].' mainmenuhspan">'.$tabstring;
				...
				{
					print '<div class="menu_contenu_disable">'.$tabstring.'<font class="vsmenudisabled vsmenudisabledmargin">'.$menu_array[$i]['titre'].'</font></div>'."\n";
				}
			}
		...
		<script type="text/javascript">
			$(window).on('load',function(){
				...
				$( '.icon-plus-filter.search_icons_container').parent("td").css("width","84px");
				$( '.icon-plus-filter.search_icons_container').parent().parent("td").css("width","84px");
 ...
 */
function dol_owntheme_showmenu($type_user, &$menuentry, &$listofmodulesforexternal)
{
	global $conf;
