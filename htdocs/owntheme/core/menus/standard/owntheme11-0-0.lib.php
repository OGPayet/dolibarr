<?php
/* Copyright (C) 2010-2014 Laurent Destailleur  <owntheme_@users.sourceforge.net>
...
/**
 *  \file		htdocs/core/menus/standard/owntheme_.lib.php
 *  \brief		Library for file owntheme_ menus
 */
require_once DOL_DOCUMENT_ROOT.'/core/class/menubase.class.php';


/**
 * Core function to output top menu owntheme_
 *
...
function print_owntheme_menu($db, $atarget, $type_user, &$tabMenu, &$menu, $noout = 0, $mode = '')
{
	...
	$menuArbo = new Menubase($db, 'owntheme_');
	$newTabMenu = $menuArbo->menuTopCharger('', '', $type_user, 'owntheme_', $tabMenu); // Return tabMenu with only top entries
...
/**
 * Core function to output left menu owntheme_
...
function print_left_owntheme_menu($db, $menu_array_before, $menu_array_after, &$tabMenu, &$menu, $noout = 0, $forcemainmenu = '', $forceleftmenu = '', $moredata = null)
{
	...
	if ($mainmenu)	// If this is empty, loading hard coded menu and loading personalised menu will fail
	{
		...
		*/
		...
		$menuArbo = new Menubase($db, 'owntheme_');
		$newmenu = $menuArbo->menuLeftCharger($newmenu, $mainmenu, $leftmenu, (empty($user->socid) ? 0 : 1), 'owntheme_', $tabMenu);
		//var_dump($newmenu->liste);    //
	...
	if (empty($noout))
	{
		...
		for ($i = 0; $i < $num; $i++)     // Loop on each menu entry
		{
			...
			if ($menu_array[$i]['level'] == 0)
			{
				...
				{
					print '<li class="menu_titre li_'.$menu_array[$i]['leftmenu'].'">'.$tabstring;
					if ($shorturlwithoutparam) {
						print '<a class="vmenu" data-actif="'.$actif_nc.'" href="'.$url.'"'.($menu_array[$i]['target'] ? ' target="'.$menu_array[$i]['target'].'"' : '').'>'.(!empty($menu_array[$i]['leftmenu']) ?
						'<div class="icon-'.$menu_array[$i]['leftmenu']." " : 
						'<div class="').'mainvmenu"></div> <span class="mainmenuaspan">'."\n";
					}

					else print '<span class="vmenu">';
					print ($menu_array[$i]['prefix'] ? $menu_array[$i]['prefxi'] : '').$menu_array[$i]['titre'];
				...
				{
					print '<li class="'.$menu_array[$i]['titre'].' li_'.$menu_array[$i]['leftmenu'].' menu_titre">'.$tabstring.'<font class="vmenudisabled vmenudisabledmargin">'.$menu_array[$i]['titre'].'</font></div>'."\n";
					$lastlevel0 = 'greyed';
				}
				else
				{
