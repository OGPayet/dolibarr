...
/**
 *  \file       htdocs/core/menus/standard/owntheme_menu.php
 *  \brief      Menu owntheme manager
 */
...
class MenuManager
{
    ...
    function loadMenu($forcemainmenu='',$forceleftmenu='')
    {
        ...
        else
       {
        ...
        $tabMenu=array();
        $menuArbo = new Menubase($this->db,'owntheme');
        $menuArbo->menuLoad($mainmenu, $leftmenu, $this->type_user, 'owntheme', $tabMenu);
    ...
    function showmenu($mode, $moredata=null)
    {
        ...
        {
            if ($mode == 'top')  print_owntheme_menu($this->db,$this->atarget,$this->type_user,$this->tabMenu,$this->menu,0,$mode);
            if ($mode == 'left') print_left_owntheme_menu($this->db,$this->menu_array,$this->menu_array_after,$this->tabMenu,$this->menu,0,'','',$moredata);
        }
        else
        {
            $conf->global->MAIN_SHOW_LOGO=0;
            if ($mode == 'top')  print_left_owntheme_menu($this->db,$this->menu_array,$this->menu_array_after,$this->tabMenu,$this->menu,0);
            if ($mode == 'left') print_owntheme_menu($this->db,$this->atarget,$this->type_user,$this->tabMenu,$this->menu,0,$mode);
        }

        if ($mode == 'topnb')
        {
            print_owntheme_menu($this->db,$this->atarget,$this->type_user,$this->tabMenu,$this->menu,1,$mode);  // no output
            return $this->menu->getNbOfVisibleMenuEntries();
        }

        if ($mode == 'jmobile')     // Used to get menu in xml ul/li
        {
            print_owntheme_menu($this->db,$this->atarget,$this->type_user,$this->tabMenu,$this->menu,1,$mode);      // Fill this->menu that is empty with top menu
            ...
            foreach($this->menu->liste as $key => $val)     // $val['url','titre','level','enabled'=0|1|2,'target','mainmenu','leftmenu'
            {
                ...
                if ($val['enabled'] == 1)
                {
                    ...
                    $tmpleftmenu='all';
                    $submenu=new Menu();
                    print_left_owntheme_menu($this->db,$this->menu_array,$this->menu_array_after,$this->tabMenu,$submenu,1,$tmpmainmenu,$tmpleftmenu);       // Fill $submenu (example with tmpmainmenu='home' tmpleftmenu='all', return left menu tree of Home)
