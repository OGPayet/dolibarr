 ...
 * @param  	array	$tabMenu        If array with menu entries already loaded, we put this array here (in most cases, it's empty). For owntheme menu, it contains menu entries loaded from database.
...
function print_left_owntheme_menu($db, $menu_array_before, $menu_array_after, &$tabMenu, &$menu, $noout = 0, $forcemainmenu = '', $forceleftmenu = '', $moredata = null)
{
	...
	if ($mainmenu)	// If this is empty, loading hard coded menu and loading personalised menu will fail
	{
		...
		*/
		...
		$menuArbo = new Menubase($db, 'owntheme');
		$newmenu = $menuArbo->menuLeftCharger($newmenu, $mainmenu, $leftmenu, (empty($user->socid) ? 0 : 1), 'owntheme', $tabMenu);
		//var_dump($newmenu->liste);    //
		...
		for ($i = 0; $i < $num; $i++)     // Loop on each menu entry
		{
			...
			if ($menu_array[$i]['level'] == 0)
			{
				if ($menu_array[$i]['enabled'])     // Enabled so visible
				{
					...
					if ($shorturlwithoutparam) print '<a class="vmenu" data-actif="'.$actif_nc.'" title="'.dol_escape_htmltag($menu_array[$i]['titre']).'" href="'.$url.'"'.($menu_array[$i]['target'] ? ' target="'.$menu_array[$i]['target'].'"' : '').'>';
				...
				{
					print '<li class="menu_titre">'.$tabstring.'<font class="vmenudisabled">'.$menu_array[$i]['titre'].'</font></li>'."\n";
					$lastlevel0 = 'greyed';
				}
				else
				{
